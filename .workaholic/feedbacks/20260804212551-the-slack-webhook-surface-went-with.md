---
type: Feedback
title: The Slack webhook surface went with the crate
kind: concern
source: development
...
supersedes:
severity: low
concern_id: the-slack-webhook-surface-went-with
owner: a@qmu.jp
...
`parse_event`/`verify_signature` verified `X-Slack-Signature` over HMAC-SHA256 and

## How to Fix

If inbound events are ever wanted, recover the module from history rather than
