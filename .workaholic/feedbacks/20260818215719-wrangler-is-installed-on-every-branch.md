---
type: Feedback
title: `wrangler` is installed on every branch push
kind: concern
source: development
...
supersedes:
severity: low
concern_id: wrangler-is-installed-on-every-branch
owner: a@qmu.jp
...
`docs-build` runs `npm install` on every push and pull request, and the new

## How to Fix

Give the publish its own `push: branches: [main]` job if the ~15s becomes visible.
