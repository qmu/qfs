...
- [ ] The serve read path evaluates SELECT against the endpoint's policy with the resolved actor; the inert-on-reads behavior (empty-commit-plan ⇒ Allow) is gone and the read-allowed test flips accordingly (#20260724013000-enforce-policy-on-the-serve-read-path.md)
- [ ] FOR / AT / WHERE / RoleGraph each admit and deny on reads in both directions, hermetically (#20260724013100-every-grant-axis-bites-on-reads.md)
