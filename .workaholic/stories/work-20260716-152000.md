...
- **When:** 2026-07-16T16:13:34+09:00
- **Target:** qfs GitHub Release (release-on-tag)
- **Method:** other (pre-merge readiness proof)
- **Status:** pass
- **Observed:** All seven pre-merge gates green on the final tree: cargo build --workspace, cargo test --workspace (150 suites ok, exit 0), cargo clippy --workspace --all-targets -D warnings, cargo fmt --all --check, gen-docs --check in sync, gen-skills --check in sync, check-migrations clean, version 0.0.73 ahead of main 0.0.72 and tag v0.0.73 free. Branch-safety scan: pass (0 findings). Both-directions proof: ledger legs disabled reproduce the pre-move zero-event behavior and the new QG tests fail exactly there.
