...
The concern judge proposed a compound over `the-api-policy-row-gates-mcp` + `owner-attended-live-verification-backlog` at severity **urgent**, on the argument that an unverified bearer gate is the sole control over an ungated statement-arbitrary read path whose sibling faces share one all-or-nothing policy row. The developer confirmed it on 2026-07-26 and it is now applied as `one-coarse-api-policy-row-for` (urgent), with both members archived as superseded.

The developer also ruled the open bridge question: `POST /api/run` gets its own **named** policy row, resolved and enforced like an endpoint's and failing closed when absent or dangling — not the parked super-admin posture, and not a reuse of the shared `api` row. That ruling is recorded on ticket `20260725110500-gate-the-statement-bridge-read-leg.md`.

The corpus is also over the triage threshold (20 active), which the same unattended constraint left for that pass.

## Deployment Evidence

- **When:** 2026-07-28T12:35:11+09:00
- **By:** a@qmu.jp
- **Target:** release-scan
- **Method:** override
- **Status:** bypassed
- **Observed:** size override accepted: commit be62a69 878 non-generated lines against the 500 threshold; zero secret and zero leak findings in the scan

## Deployment Evidence

- **When:** 2026-07-28T12:39:27+09:00
- **By:** a@qmu.jp
- **Target:** qfs GitHub Release (release-on-tag)
- **Method:** other (pre-merge readiness proof)
- **Status:** pass
- **Observed:** all six gate commands exited 0 on the caught-up branch: build, test --workspace with 151 suites ok and zero failures, clippy -D warnings, fmt --check, gen-docs --check, gen-skills --check; Cargo.toml version 0.0.90 is ahead of main 0.0.89 and tag v0.0.90 is unused
