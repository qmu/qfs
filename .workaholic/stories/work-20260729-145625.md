...
- **Nine empty `Resume a PR-unit` commits, and the stamp that caused them.** The unit was originally claimed on `work-20260729-145625`; the re-land moved it to `claude/existing-prs-issues-naiqyz`, but the archived ticket kept `claim: work-20260729-145625`. The claim scan attributes artifacts by that stamp, so it reported this claim with `artifacts: []` — which made the drained unit look `resumable` to every tick (hence the empty Resume commits) *and* left the base-side `todo/` copy of the ticket in the unclaimed backlog with no `excluded[]` row, inviting a fresh runner to re-implement finished work. This tick corrected the stamp to name the branch that actually holds the claim, and filed the underlying defect as `.workaholic/feedbacks/20260817014809-a-re-landed-claim-branch-loses-its-artifact-attribution.md`. That fixed the resume loop — the claim now reads `claim_active` instead of `heartbeat_lapsed`, so no further empty Resume commits — but **one layer of the same defect is left, and it is not repairable from this repository**: the reader resolves the artifact to its *legacy per-user* base-side path, `.workaholic/tickets/todo/a-qmu-jp/20260728085253-…md`, because that is where the ticket sat when it was claimed on 2026-07-29, before `migrate-todo-owners.sh` flattened the queue. The live base-side path has no `a-qmu-jp/` segment, so the survey's exclusion never matches it and the ticket is still offered as unclaimed backlog on every tick — an invitation to re-implement what this PR already contains. Merging #46 ends it; until then the offer should be ignored, and the repair belongs with the rename-mapping in `qmu/workaholic`.
