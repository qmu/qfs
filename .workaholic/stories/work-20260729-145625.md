...
- **The branch no longer merges with `main`, and an unattended run may not resolve it.** Catching up (`ship/scripts/catchup-main.sh`) now conflicts on five paths — `.claude-plugin/marketplace.json`, `packages/qfs/Cargo.lock`, `packages/qfs/crates/qfs/Cargo.toml`, and both `plugins/qfs/*/plugin.json`. Every one is a version/lockstep manifest, but the classifier returned `conflict_class: content` (its mechanical patterns do not match these nested paths), and a `content` conflict demotes to the PR path by rule. So the merge is aborted and the tree left clean. **What a human has to do here is two rulings, not one**: override the `size` finding above, *and* redo the four version bumps on top of current `main` (qfs patch + all four plugin `version` fields, per CLAUDE.md) with `Cargo.lock` regenerated. Until both happen this unit is re-resumed by every `[Implement]` tick and can only add empty `Resume a PR-unit` commits — eight of them so far.
