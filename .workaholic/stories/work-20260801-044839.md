...
### Installing a declaration is a manual step the release does not perform

- **Severity:** low
- **Description:** The live round only worked after the updated `chatwork.qfs` map was re-installed by
  hand: a declared driver's map body lives as a stored `/sys/drivers` row, so shipping a new binary
  does **not** update an operator's already-installed declaration (see
  [8ca09f4](https://github.com/qmu/qfs/commit/8ca09f4) in
  `packages/qfs/crates/skill/assets/examples/chatwork.qfs`). An operator who upgrades to `0.0.93` and
  does nothing else keeps the old JSON-body map and keeps getting `400`, with no signal that the fix
  exists.
- **How to Fix:** Decide how a shipped declaration reaches an installed one — a version stamp on the
  stored row plus a "your installed declaration is older than the shipped one" notice, or an explicit
  `qfs` reinstall path the release note points at. Note that the release note for `0.0.93` should say
  so explicitly in the meantime.
...
- Say in the release note that an operator with an already-installed `/chatwork` declaration must
  re-install the message map to pick the fix up — the stored `/sys/drivers` row does not follow the
  binary (section 6).
