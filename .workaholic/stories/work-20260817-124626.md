...
### Seventy-seven code spans still render as literal backticks

- **Severity:** low
- **Description:** The build-breaking subset is fixed, but 154 stray backticks survive into rendered output across 18 of 38 pages, and the new `docs-build` job cannot catch them because only an angle-bracketed payload breaks the compiler (see [84040bc](https://github.com/qmu/qfs/commit/84040bc) in `docs/blueprint.md`). Three of the four worst pages are generated by `gen-docs`.
- **How to Fix:** Try a renderer/markdown-it option before any reflow — half the affected spans are on generated pages where a reflow is not available. Ticketed at `20260817130410`.
