...
### Scope
New leaf crate `crates/driver-objstore` (`cfs-driver-objstore`) exposing two drivers over one
...
- `backend` — `ObjectBackend` trait (owned DTOs only), `HttpBackend` (real, over a local
  `HttpExchange` seam on `cfs-http-core` — the cf precedent, so NO dep on `cfs-driver-http`),
...
`HttpBackend` builds owned `cfs_http_core::HttpRequest`s, runs the **SigV4** signer over them
(injecting the `Authorization` + `x-amz-*` headers), and sends them over a local
`HttpExchange` seam — so reqwest stays confined to `cfs-driver-http` and this crate stays an
**independent runtime leaf** (no `cfs-driver-http` edge). The SigV4 signer and any `http::`/
...
Credentials are injected as a `cfs_secrets::Secret` (access-key-id is non-secret config; the
...
`cfs-http-core` redaction authority). Never logged, never stored in a DTO, never in an
...
`cfs-driver-objstore → { cfs-driver, cfs-plan, cfs-types, cfs-runtime, cfs-http-core,
cfs-secrets, serde, serde_json, thiserror, tracing, hex, sha2, hmac }`. It is a **runtime leaf**
(bridges its synchronous `PlanApplier` → async `ApplyDriver`); nothing depends back onto it, so
tokio dead-ends. Append `cfs-driver-objstore` to `dep_direction.rs`'s `runtime_consumers_allowed`
...
no regression on 552, plus cfs-plan purity + generic runtime-leaf confinement still green.
...
`wasm32-unknown-unknown` because it depends on `cfs-runtime`, which pulls `tokio` with
native-only features (`rt-multi-thread`) — the wasm tokio build rejects them. This is a
**workspace-wide constraint the t23 `cfs-driver-cf` driver shares** (no runtime-bridge driver
...
`wasm32-unknown-unknown`** in isolation (no `cfs-runtime`), proving the R2 binding type-checks and
