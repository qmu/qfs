...
### Topology decision — a dedicated leaf crate `cfs-cron`

Following the established E7 leaf-binding pattern (`cfs-http` is the precedent): the
scheduler is a **new leaf crate `cfs-cron`**, NOT a module inside `cfs-server`.

Rationale, weighed against every confinement guard:
- The scheduler must consume `cfs-exec` (`build_plan`: eval a DO `Statement` →
  `Plan`) — exactly the role `cfs-http` plays. `cfs-server` must stay OFF `cfs-exec`
  and runtime-free (CO-t30-1 / the t30/t32 rule), so the scheduler cannot live in
  `cfs-server`. `cfs-cmd` must stay off `cfs-exec`/`cfs-http`, so it cannot live there
  either.
- `cfs-cron` consumes `cfs-server` (the `Binding`/`ServerState`/`JobDef` registry +
  reconcile seam) AND `cfs-exec` (the evaluator) AND `cfs-core` (Plan/commit/Value/
  StatementSpec). It is the one crate that legitimately binds all three for the JOB
  cause — the same justification that made `cfs-http` a new leaf.
- `cfs-cron` is a LEAF: only the terminal `cfs` binary depends on it (the serve
...
- `cfs-cron` does NOT depend on `cfs-runtime`. The REAL commit path (the runtime
  ...
  interpreter never enters `cfs-cron`; its only tokio is the native daemon loop (HTTP/
  cron I/O domain), exactly mirroring `cfs-http`.

Guard impact (all must stay green):
- `dep_direction::exec_is_confined_above_the_spine_and_off_the_runtime`: extend the
  `allowed_exec_consumers` allowlist with `cfs-cron` (the 4th admitted leaf consumer of
  the read/eval executor, same role as `cfs-http`). Add the documented rationale.
- A new guard `cron_binding_is_a_leaf_serve_consumer` (twin of
  `http_binding_is_a_leaf_serve_consumer`): asserts `cfs-cron` depends on
  `cfs-server` + `cfs-exec`, is a leaf (only `cfs` depends on it), and does NOT depend
  on `cfs-runtime`.
- `binary_is_the_thin_entrypoint_plus_the_t28_shell_composition_root`: extend the
  binary allowlist with `cfs-cron`.
- `cfs-cmd` dep_direction guards untouched (cfs-cmd never gains a cfs-cron edge — the
...
`cfs-cron` mirrors `cfs-driver-slack`'s split:
...
- **run_id is DETERMINISTIC**: `run_id = stable_key(job_name, scheduled_for)` — a hash
  ...
  (`cfs-driver-objstore::sha256`); to avoid a cross-leaf dep we vendor the ~80-line FIPS
...
- `Scheduler<S: JobStore, C: Clock>` with the injected `Committer`:
  ...
  - `dispatch(&self, job, scheduled_for) -> RunRecord`: deterministic run_id →
    ...
    builds the Plan via cfs-exec build_plan and runs the REAL applier) → on success
...
Tests (in `cfs-cron`, `MockClock` + `MemJobStore` + `RecordingCommitter`, no live creds):
...
1. Create `cfs-cron` crate (Cargo.toml: native/default feature split; deps cfs-server,
   cfs-exec, cfs-core, cfs-parser, optional tokio). Register in workspace `members` (it
...
2. Pure modules: `hash` (vendored sha256), `schedule` (Schedule/CronExpr/next_after),
   ...
   dispatch/circuit-breaker), `binding` (CronBinding impl cfs_server::Binding kind Cron).
...
4. Binary wiring: extend `crates/cfs/src/serve.rs` to register `CronBinding` alongside
...
- **Guard regression**: extending two allowlists + one new guard is the only spine
  surface change; mirrors the t32 cfs-http precedent exactly, so the topology is already
  blessed.
