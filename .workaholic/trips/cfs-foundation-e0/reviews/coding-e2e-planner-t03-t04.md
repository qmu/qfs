...
Targets: t03 (`cfs-lang::lex`) + t04 (`cfs-parser::parse_statement`)
Method: throwaway external consumer crate in `/tmp/cfs-e2e-consumer` (own
`[workspace]` table; path-dep on `crates/parser`), calls **only** the public
surface `cfs_parser::parse_statement(&str) -> Result<Statement, ParseError>` and
...
(`cfs_lang::lex`) and parses the full t04 grammar; t03 is therefore validated
