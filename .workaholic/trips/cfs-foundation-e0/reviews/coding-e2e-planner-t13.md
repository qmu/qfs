...
party who has never seen cfs internals) implementing `cfs_driver::Driver` using **only the
public API**, plus an out-of-crate `MyApplier: PlanApplier`. This proves the contract is
implementable from outside the cfs workspace. The binary exercises every required scenario
...
- `mount() == "/my"`.
- `describe(/my/notes)` → `archetype=AppendLog`, typed 2-column schema (`cfs_types::Schema`).
