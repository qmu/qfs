...
|---|---|
| `cfs` (bin) | `cfs-cmd` |
| `cfs-cmd` | `cfs-core`, `cfs-server` |
| `cfs-core` | `cfs-driver`, `cfs-codec`, `cfs-lang`, `cfs-plan` |
| `cfs-server` | `cfs-core` |
| `cfs-codec` | `cfs-driver` |
| `cfs-driver` | `cfs-plan` |
| `cfs-lang` | — (leaf) |
| `cfs-plan` | — (leaf) |
| `cfs-parser` | `cfs-lang` |

Topological order exists: `cfs-lang, cfs-plan` (level 0) → `cfs-driver` → `cfs-codec` → `cfs-core` → `cfs-server` (and `cfs-cmd`) → `cfs-cmd` → `cfs` (bin). `cfs-parser` sits at level 1 (depends only on `cfs-lang`). **No back-edge, no cycle.** Nothing depends on `cfs-cmd` except the `cfs` binary. The spine matches model-v1 §4 and design-v1 §1.2, and is *mechanically* locked by `crates/cmd/tests/dep_direction.rs` (four `cargo metadata` assertions: cmd has no domain-crate edge; nothing-but-`cfs` depends on cmd; the binary depends on cmd only among workspace crates; core does not yet depend on parser and parser never depends on core).

Note: the implemented spine carries two edges the model-v1 §4 *diagram* did not draw as arrows — `cfs-driver → cfs-plan` and `cfs-codec → cfs-driver`. Both are acyclic and both are explicitly anticipated in the model-v1 §4 *prose* ("`cfs-driver` returning a `cfs-plan::Plan` node") and the design. They are RFD-justified (trait signatures reference `Plan`; the shared error). This is fidelity, not drift.

## 2. Decision D1 (CfsError + Path in cfs-driver) — STRUCTURALLY SOUND

design-v1 §2.3 placed `CfsError`/`Path` in `cfs-core`. The Constructor moved them to `cfs-driver` because the `Driver`/`Codec` trait signatures return `Result<_, CfsError>` and take `&Path`, while the spine mandates `cfs-core → cfs-driver`; keeping the error in `cfs-core` would force the back-edge `cfs-driver → cfs-core` — exactly the cycle model-v1 §4 forbids and which I flagged in my own Review Notes as a revision trigger. The fix:

- canonical `CfsError`/`Path` live in `cfs-driver` (the lowest crate the signatures need);
- `cfs-codec` takes a one-line `cfs-driver` dep purely for the shared error (acyclic, RFD-justified — both trait crates return `Result<_, CfsError>`);
- `cfs-core` *re-exports* both (`pub use cfs_driver::{… CfsError, …, Path, …}`), so the rest of the workspace still names `cfs_core::CfsError` / `cfs_core::Path`.

**Judgment: faithful to the model's "one error enum, acyclic spine" intent.** There is exactly one error enum; the spine stays acyclic; consumers above `cfs-core` are unaffected by where the type physically lives. The decision is documented in three places that agree verbatim (`crates/driver/src/error.rs` header, `cfs-driver` package description, `ARCHITECTURE.md` §"Decision D1"). This is the *correct* resolution of the design-v1/model-v1 tension, and it is the kind of decision the model explicitly delegated ("verify it faithfully encodes the RFD, not redesign it" — A2; the Constructor preserved the contract while removing a cycle the design hadn't noticed).

**One forward-looking observation (a minor suggestion, not a blocker).** Placing the *canonical* `CfsError`/`Path` in `cfs-driver` makes `cfs-driver` carry a second responsibility — "the driver contract" *and* "the home of the two universal primitives." For E0–E2 this is harmless. The latent risk surfaces if a future crate needs `CfsError`/`Path` but should **not** depend on the whole driver contract — e.g. an E7 server-config or E2 audit type that wants the error without pulling `Driver`/`Archetype`/`Capabilities` into scope. Today every consumer either is above `cfs-core` (gets the re-export for free) or is `cfs-codec` (legitimately needs the error). So no crate is forced into an unwanted edge *yet*.

- **Proposal (defer, do not act now):** if/when a crate needs `CfsError`/`Path` without the driver contract, introduce a leaf `cfs-types` (or `cfs-error`) crate holding *only* `CfsError` + `Path`, and have `cfs-driver`/`cfs-codec`/`cfs-core` re-export from it. Doing this **now** would be churn against a hypothetical: it is a pure move-plus-reexport, fully mechanical, and `cfs-core`'s re-export surface (`cfs_core::CfsError`, `cfs_core::Path`) is the stable public name everything else already uses — so the future extraction is *invisible* to every downstream crate. The current placement does **not** foreclose the leaf-crate option; it defers it at zero cost. I therefore do **not** request the leaf crate now. Recommend a one-line `// TODO` note in `crates/driver/src/error.rs` recording that `CfsError`/`Path` may later be extracted to a `cfs-types` leaf if a non-driver consumer appears, so the option is visibly reserved rather than rediscovered.
...
- **G5 / C4 (cmd logic-free) — PASS.** `cfs-cmd` depends on `cfs-core` + `cfs-server` only (manifest + `dep_direction.rs`). The dispatch arms (`dispatch_run`, `dispatch_shell`, `serve`) return `CfsError::NotImplemented`; the only "logic" present is argv parsing (clap), output-mode selection, error rendering, and tracing init — all command-boundary concerns, no engine/domain logic. Faithful to the "two faces of one engine" boundary B1.
- **G6 (parser behind owned ParseError) — PASS (skeleton).** `cfs-parser` exposes an owned `#[non_exhaustive] ParseError` and `parse_statement` with a library-free signature; no vendor type is reachable (no parser lib is even a dependency yet — t02 adds it). The reversibility contract is documented for t02. As a t01 skeleton this is exactly the reserved boundary B6 requires; the `pub use` audit lands with t02 when a real library is introduced.
...
- **E1 (grammar/AST in `cfs-lang`, parser in `cfs-parser`):** `cfs-lang` is explicitly the AST home ("AST sum types land here in E1") and is a leaf; `cfs-parser → cfs-lang` is wired and `cfs-parser`'s `parse_statement`/`ParseError` shapes are stable. E1 adds the AST enum to `cfs-lang`, fills `parse_statement` in `cfs-parser`, and wires the reserved `cfs-core → cfs-parser` edge — **no restructure**.
- **E2 (effect-plan/interpreter in `cfs-plan`):** `cfs-plan` owns `Effect`/`Plan`/`irreversible` (construction-only). The interpreter (`COMMIT : Plan -> World`) is reserved and *named* in three crates (`cfs-plan` docs, the `cfs-driver` marker, `Engine::audit_sink`/`Session::irreversible` seams). E2 adds the interpreter as a runtime over `cfs-plan` — **inside the seam**.
- **E7 (server DDL):** `cfs-server` is positioned as a `Driver` over `/server`; the DDL keywords (`CREATE ENDPOINT/TRIGGER/JOB/VIEW/MATERIALIZED VIEW/WEBHOOK/POLICY`, `DO/EVERY/ON`) are already in the frozen `KEYWORDS` set; the `TODO(E7)` anchor states the implementation path. E7 implements `Driver` for `cfs-server` and registers it — **no new "server is special" boundary**.
...
1. **D1 single-responsibility creep in `cfs-driver` (minor, forward-looking).** Covered in §2 — proposal: a `// TODO` note reserving a future `cfs-types` leaf extraction; do not extract now.
...
3. **`Session::irreversible` duplicates `Plan::is_irreversible` (minor).** `cfs-core::Session` carries an `irreversible: bool` that "mirrors `cfs_plan::Plan::is_irreversible`." Two sources of the same truth invite drift once E2 populates plans. *Proposal:* in E2, derive the session flag from the active `Plan` rather than storing it independently (or document it as a per-request override gate, distinct from the plan's own property). Recorded for E2; not a t01 defect.
