...
- Parsing alias bodies as real cfs source and purity-checking that the body is exactly one
...
`StdlibRegistry` prelude facility (`register_prelude` → parse cfs body → purity-check →
...
can hand the resolver an `AliasFn` that never passed the cfs-source purity gate, and there
...
prelude as `AliasDecl` cfs-source bodies and route them through
...
### Observation 3 — PlanNode is a stdlib-local DTO, not folded into `cfs_plan::EffectNode` (reconciliation debt for t10)
`stdlib::tablevalued::PlanNode { kind: Read | HttpGet }` is an owned, vendor-free DTO that
deliberately does **not** reference `cfs_plan::EffectNode` (ARCHITECTURE.md confirms
`cfs-plan` carries the effect DAG). For t08 (construct-only, no runtime) this is the correct
...
/`HttpGet` lifts into the `cfs_plan` DAG (a `Read` source node + the codec-decode chain the
...
`plain_source`). This is an acceptable deviation: parsing the body as a *real* cfs
`Statement::Query` is what makes the purity check by-construction rather than by-convention,
and a FROM-led pipeline is the genuine cfs surface form (a bare `d |>` is the desugar input,
...
- Dependency direction preserved: stdlib is in `cfs-core`, depends down on
  `cfs-types`/`cfs-parser`/`cfs-driver`; no new crate, no back-edge, no vendor types across
  the boundary (owned DTOs only). `cfs-plan` is untouched (PlanNode is local).
...
(lift PlanNode into `cfs_plan::EffectNode`) are the two to carry explicitly into E4 and t10
respectively.
