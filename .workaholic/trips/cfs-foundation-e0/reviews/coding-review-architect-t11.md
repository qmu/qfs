...
- **Owned ETag/version (no vendor leak)**: correct. `Version`/`Etag` are owned `String`
  ...
  token the golden test asserts. No `reqwest`/SDK type appears in any `cfs-txn` signature
...
- **`cfs-txn` is pure**: confirmed by reading every module — no `tokio`, no `async`, no
  ...
  (a') `cfs-txn`'s workspace deps ⊆ `{cfs-plan, cfs-types}` and (a) `cfs-runtime`'s ⊆
  `{cfs-plan, cfs-types, cfs-txn}`, plus the no-back-edge direction. This is the right
