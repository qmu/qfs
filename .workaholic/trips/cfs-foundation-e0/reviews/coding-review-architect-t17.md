...
- Target: t17 — `cfs-driver-sql`, commit `43c5af1`
...
**Is LIKE genuinely lossy?** Yes — and keeping it residual is correct. cfs `LIKE` is glob
...
territory):** a *glob→LIKE* translation could be pushed exactly per-dialect (escaping cfs glob
...
- Credential is a `cfs_secrets::Secret` resolved by `(DriverId "sql", AccountId <conn>)` in
...
`dep_direction.rs` confirms `cfs-driver-sql` is admitted as a **runtime-leaf consumer** in both the
generic leaf-confinement check (b) and the named allowlist (b'): it depends on `cfs-runtime` (to
