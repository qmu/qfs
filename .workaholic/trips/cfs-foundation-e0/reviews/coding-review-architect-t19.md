...
- **All four secret classes are `Secret` or wire-only.** `client_secret`, the refresh token, and the access
  token are `cfs_secrets::Secret` (no `Clone`, no `Serialize`/`Deserialize`, redacting `Debug`/`Display`,
...
- **Redacting `Debug` on every token-bearing DTO.** `AccessToken`, `GoogleAccount`, `BorrowedToken`,
  ...
  redaction or substitute `cfs_secrets::REDACTED` for sensitive header *values* (shared `SENSITIVE_HEADERS`
...
## 2. The two-seam decision — CORRECT for now; recommend extracting `cfs-http-core` before t20/t21/t41 land

**The call to NOT depend on `cfs-driver-http` is right.** `dep_direction.rs::runtime_is_confined_to_plan_and_types`
mechanically enforces that every `cfs-runtime` consumer is a leaf (test (b)) and pins the allowlist to
`{cfs-driver-local, cfs-driver-http, cfs}` (test (b')). `cfs-driver-http` depends on `cfs-runtime`; if
`cfs-google-auth` depended on `cfs-driver-http`, the latter would stop being a leaf and test (b) would fire.
Keeping `cfs-google-auth → cfs-secrets` only (secrets → types) keeps it entirely off the runtime and off the
...
| Element | t18 `cfs-driver-http` | t19 `cfs-google-auth` | Drift |
| --- | --- | --- | --- |
...
runtime-free** request/response/method/redaction layer into a new leaf crate `cfs-http-core` that BOTH
`cfs-driver-http` and `cfs-google-auth` depend on:

- `cfs-http-core` owns: `HttpMethod`, `HttpRequest`, `HttpResponse`, `SENSITIVE_HEADERS`,
  `is_sensitive_header`, and the redacting `Debug` impls. It depends on `cfs-secrets` (for `REDACTED`) →
...
- `cfs-driver-http` keeps `HttpClient` (sync trait), `ReqwestClient`, `HttpError` (the status/transport
  taxonomy), and the runtime bridge — and re-exports the DTOs from `cfs-http-core`.
- `cfs-google-auth` depends on `cfs-http-core` for the DTOs + redaction, and keeps **only** its `HttpExchange`
...
- `dep_direction.rs` gains one assertion: `cfs-http-core` is a leaf and is depended on by both
  `cfs-driver-http` and `cfs-google-auth`; neither re-defines the DTOs.
...
`cfs-google-auth` that asserts its `SENSITIVE_HEADERS` is a superset-or-equal of `cfs_driver_http::SENSITIVE_HEADERS`
(a `dev-dependency` on `cfs-driver-http` for tests only does not affect the production dep graph). That pins
...
1. **Duplicate HTTP DTO seam already drifting (the headline structural concern).** *Proposal*: extract
   `cfs-http-core` (leaf) owning `HttpMethod`/`HttpRequest`/`HttpResponse`/`SENSITIVE_HEADERS`/redacting
   `Debug`, depended on by both `cfs-driver-http` and `cfs-google-auth`, so the redaction set and DTOs cannot
   drift and the three driver adapters become zero-copy trait shims. Minimum fallback if deferred: a
   cross-crate test asserting t19's `SENSITIVE_HEADERS ⊇ cfs_driver_http::SENSITIVE_HEADERS`.
