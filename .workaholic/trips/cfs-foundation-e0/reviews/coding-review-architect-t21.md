...
- **Target**: t21 — `cfs-driver-gdrive`, commit `f7c1a51`
...
- No `reqwest` in `crates/driver-gdrive/Cargo.toml` — the driver rides the t19 `GoogleApiClient`
  + the pure `cfs-http-core` DTOs; reqwest stays confined to `cfs-driver-http`. The runtime-leaf /
...
`cfs-http-core::HttpMethod` (lines 72-82) defines only `Get/Post/Put/Delete` — there is no `Patch`
...
**Proposal (structural, low-cost):** add a `Patch` arm to `cfs-http-core::HttpMethod` (the single
...
Clean. `cfs-driver-gdrive` is a leaf runtime consumer: it appears in both the generic leaf-check
