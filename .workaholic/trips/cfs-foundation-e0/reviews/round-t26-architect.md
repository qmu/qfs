...
- Status: under-review
- Reviewed: commit `5f5ce18` — `crates/driver-git/` (cfs-driver-git), `docs/adr/0003-git-object-access.md`, `crates/cmd/tests/dep_direction.rs`
...
`cfs-driver-git` depends only on `cfs-driver`/`cfs-plan`/`cfs-types`/`cfs-codec`/`cfs-runtime` +
...
`dep_direction.rs` change is a single allowlist append (`"cfs-driver-git"`, line 334) guarded by
