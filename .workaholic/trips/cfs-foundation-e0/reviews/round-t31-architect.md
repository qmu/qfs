...
`normalize_body_column` (lower.rs L214) keeps a non-cfs `plan`/`query` string literally. I
judge this sound, with a bounded scope argument:
  - The CREATE path *cannot* produce a non-parseable body — `from_server_ddl` only ever
    ...
    via an explicit hand-written `INSERT INTO /server/…` carrying a non-cfs marker string.
  ...
  - The carve-out does **not** weaken the equivalence claim: equivalence is asserted between
    a CREATE body and an INSERT twin carrying *the same genuine cfs source*. A non-cfs marker
...
**data**, never a live `cfs_plan::Plan`. `PlanSpec` is a newtype over `StatementSpec`
...
non-cfs string. The runtime's fire-time `from_canonical` (E7) will `Err` on the opaque case.
...
**S1 — placement in `cfs-core::ddl::server`: CORRECT, confinement green.** The DDL is
closed-core (frozen keywords, shared across backends), so it belongs in core, not a driver —
ticket-intent-aligned (Key components L56-57; Considerations L120-121). `cfs-server`'s
...
**no matches**. Core's ddl imports only `cfs_parser` (owned AST, already a wired edge),
`cfs_plan`, `cfs_types`, `serde`/`serde_json`. No vendor/SDK/async leak into closed core.
...
making the AST fully round-trippable. `cfs_lang::Span` stays serde-free (the lexer crate
...
declarative "make this exist" → UPSERT so a `config.cfs` replay/boot converges (RFD §6,
