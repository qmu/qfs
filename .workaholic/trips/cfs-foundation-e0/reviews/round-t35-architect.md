...
`cfs-parser` POLICY grammar, `cfs-server::lower`, the three fire-path committers
(cfs-http / cfs-cron / cfs-watchtower), the `cfs` composition root (serve/cron/watchtower),
...
- **cfs-cron** (`crates/cron/src/commit.rs`): `build_plan` (161) → `gate_plan` (177) →
  `record_fired` (179, unconditional, one record) → `if !is_allow() return PolicyDenied`
  (185) → applier constructed (197) and `cfs_core::commit` (199). The single apply is
  gated and after the deny return.
- **cfs-watchtower** (`crates/watchtower/src/commit.rs`): `build_plan` (198) →
  `gate_plan` (207) → `if Deny return PolicyDenied{...}` (209) → applier (225) +
  `cfs_core::commit` (226). Single apply, gated, after deny return.
- **cfs-http** (`crates/http/src/{policy,route,handler}.rs`): a write-lowering endpoint is
...
`grep` confirms the only `cfs_core::commit(`/`RecordingApplier` call sites in the three
...
config-write channel (`cfs-server::runtime::boot` → `apply_source` → `lower_statement` →
`cfs_core::commit` over `ServerConfigApplier`, runtime.rs:166-192) and the t28 shell COMMIT
(`cfs_exec::run_oneshot`) are trusted operator actions and are correctly OUT of
...
6. **Fixtures/tests are genuine default-deny, NOT weakened.** `server_boot.cfs`:
   `leastpriv ALLOW INSERT,REMOVE` (explicit REMOVE for the JOB), `trigger_writer`-style
   scoping; the read ENDPOINT carries NO policy (read path is separate). `watchtower.cfs`:
...
9. **Placement + confinement — CLEAN.** enforcer/model/gate/audit in `cfs-server::policy`;
   grammar tokens in `cfs-parser`; desugar in `cfs-server::lower::lower_policy`. No spine
   inversion (parser is a leaf; cfs-server depends on cfs-core). Re-exports are coherent
   (lib.rs:55-59).
