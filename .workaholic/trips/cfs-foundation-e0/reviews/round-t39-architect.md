...
sibling of the t28/t32 launchers and keeps cfs-cmd off the driver crates. One genuine
...
I read SKILL.md, the seven `assets/examples/*.cfs`, and the golden corpus. Every example uses the
identical four steps: (1) DESCRIBE excerpt, (2) one cfs statement, (3) PREVIEW reading affected
...
1. **`cp` → `UPSERT` lowering (drive).** This is a *shell-builtin desugar*, identical in kind to
   the `SEND` alias and `CREATE TRIGGER` desugar. SKILL.md and `drive.cfs` are explicit: `cp` is
...
The facts: `cfs-skill` embeds `SKILL.md` via `include_str!` (`SKILL_MD` const, `EXAMPLES` manifest),
but **nothing links `cfs-skill` into the binary** — it is a `publish=false` dev/assets crate whose
only non-test consumer is… nothing. There is no `cfs skill [print]` subcommand. So the claim in
...
- **Intent**: the ticket's Overview and RFD §1 frame the whole point as *the agent learns the loop*;
  ...
  agent driving `cfs` discovers capabilities by running the binary (`cfs describe`, `cfs --help`),
  not by reading the repo's `docs/` tree. Under that intent, "discoverable from the running binary"
  is load-bearing, and a `cfs-skill` crate that no shipped artifact links is a const that exists only
...
- Add a `Skill { #[arg(...)] examples: bool }` subcommand to `cfs-cmd` that prints
  `cfs_skill::SKILL_MD` (and, with a flag, the `EXAMPLES` statements). This requires the binary to
  add a `cfs-skill` *normal* (non-dev) dependency — which is the edge that actually makes the embed
  claim true and which the dep guards already anticipate (`cfs-skill` is a terminal sink).
- Wiring is logic-free (mirror the `Describe`/`Serve` dispatch): cfs-cmd routes `Skill` to a thin
  print, the const lives in the leaf crate. No new C4/leaf risk (cfs-skill carries no runtime; its
...
**(b) Composition-root injection keeps cfs-cmd logic-free + off the driver crates.** Confirmed.
`crates/cfs/src/describe.rs::describe_registry()` builds the `MountRegistry` from the eight concrete
driver crates (cred-free `Mock*Client` / empty `ObjRegistry`) and `main.rs` injects it as the third
launcher arg. `cfs-cmd` only declares `DescribeProvider = dyn Fn() -> cfs_core::MountRegistry` and
`dispatch_describe` builds the registry via the provider then hands off to
`cfs_exec::run_describe` — no driver crate is named in cfs-cmd. The dep_direction guard's
...
the same terminal-sink rationale as t28/t32/t36. No C4 / leaf-confinement violation: cfs-cmd stays
on `cfs-core` + `cfs-server` only. This is a faithful structural sibling of `ShellLauncher`/
`ServeLauncher`.
...
1. **DescribeReport DTO** — Confirmed Serialize-only (no `Deserialize`, with a documented rationale:
   cfs never reads a report back). Reuses `cfs_types::Column` / `Capabilities` / `ProcSig` /
...
2. **`cfs describe -json`** — The `-json` single-dash token normalization (`normalize_json_alias`)
   ...
   **Observation**: `dispatch_describe` resolves format from `json || --format` else TTY/pipe
   ...
   the same deterministic-pipe behavior as `cfs run`. Consistent. No concern.

3. **Golden corpus reuses the t38 cfs-test harness** — Confirmed: imports `cfs_test::{assert_plan,
...
4. **cfs-skill confinement — exemption is narrow + sound. Confirmed.** The runtime-leaf guard
   (`runtime_is_confined_to_plan_and_types`) exempts `cfs-skill` (lines 424-430) with the exact right
   ...
   can never transit tokio into a *shipped* artifact, and nothing depends on `cfs-skill` (terminal
   ...
   dead-ends here" property as the `cfs` binary, scoped to the dev-only sink. This is exactly why the
   embed-into-binary fold-in (Headline 2) stays safe: a *normal* `cfs-skill` dep would carry only the
...
- **[Primary] CO-t39-1 binary-embed gap** → Fold in a thin `cfs skill [--examples]` subcommand
  printing `cfs_skill::SKILL_MD`, adding `cfs-skill` as a *normal* (zero-runtime-dep) binary
...
- **[Minor] slack `#`-vs-bare path drift** → SKILL.md uses `/slack/acme/#general/messages`; the
  ...
  read as a smuggled exception to a careless agent. Proposal: add one line to `slack.cfs` /
