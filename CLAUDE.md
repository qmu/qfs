...
and both `version` fields in `.claude-plugin/marketplace.json`) — independent of the binary's
`v0.0.x`. The gen-skills ratchet keeps skill *content* true, but only a plugin version bump
propagates regenerated skills to installed caches (a stale cache keeps teaching retired commands).
So: a shipped hard break — or any skill-affecting change — bumps all four plugin `version` fields in
the same PR (minor for a taught-surface break, patch otherwise).
