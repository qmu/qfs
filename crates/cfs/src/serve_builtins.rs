//! Built-in self-contained read sources the `cfs serve` daemon registers (t36).
//!
//! A bare `cfs serve` registers NO external read drivers — the deployment registers the real E4
...
//! a liveness ENDPOINT serves a real well-formed body over loopback (the t36 acceptance: `cfs
...
fn status_row() -> Row {
    Row::new(vec![Value::Int(1), Value::Text("cfs".into())])
}

/// A self-contained, credential-free built-in read source: the `/status` daemon-liveness table.
/// Implements both `cfs_core::Driver` (so the engine can `describe`/plan it) and
/// `cfs_exec::ReadDriver` (so the executor can scan it). A read-only source — it advertises only
...
pub fn register_builtins(engine: &mut Engine, reads: &mut ReadRegistry) {
    ...
    if let Err(e) = engine.mounts.register(Arc::new(StatusDriver::new())) {
        ...
        tracing::warn!(target: "cfs::serve", error = %e, "could not register /status built-in");
        return;
    }
    ...
    reads.register(
        cfs_core::DriverId::new(source_id),
