//! `cfs::version` — the version + build-metadata surface (ticket t40, RFD §9).
//!
...
//! `rustc-env`. [`long_version`] assembles the multi-line `cfs --version` long form — the
...
pub const GIT_SHA: &str = env!("CFS_GIT_SHA");

/// The target triple this binary was compiled for (e.g. `x86_64-unknown-linux-musl`).
/// Baked in by `build.rs`.
pub const TARGET: &str = env!("CFS_TARGET");

/// Whether this build's target is `wasm32-*` capable (RFD §9 Cloudflare Workers target).
/// The string `"true"` / `"false"`, baked in by `build.rs`.
pub const WASM_CAPABLE: &str = env!("CFS_WASM_CAPABLE");

/// The long `cfs --version` form: semver + git sha + target triple (+ the wasm-capable flag),
/// one field per line. This is the clap `long_version`, surfaced on `cfs --version`.
///
...
mod tests {
    ...
    fn long_version_carries_semver_sha_and_target() {
        let v = long_version();
        assert!(
            v.starts_with(&format!("cfs {VERSION}")),
            "leads with semver"
        );
