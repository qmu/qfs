...
//!
//! `cfs` runs cross-service effect-plans **unattended** — a large blast radius. Some effects
...
//! plan carrying an irreversible effect may proceed to `COMMIT`, given **how** cfs is being
//! driven ([`RunMode`]).
//!
//! ## Why it lives in `cfs-core` (the hub)
//! Both faces of the binary commit through `cfs-core`'s [`Plan`]: the CLI one-shot
//! (`cfs run … --commit`, via `cfs-exec`) and the serve fire paths (HTTP / cron / watchtower
//! committers, via `cfs-server`). Placing the guard on the hub's `Plan` lets *both* reuse one
...
/// How cfs is being driven — the context the [`IrreversibleGuard`] decides against. Whether a
...
pub enum RunMode {
    ...
    Cli,
    /// A non-interactive one-shot CLI invocation (`cfs run … --commit`, no TTY): there is no
...
mod tests {
    use super::*;
    use cfs_plan::{EffectKind, EffectNode, NodeId, Plan, Target, VfsPath};
