//! `cfs-driver-cf` — the **Cloudflare driver** (RFD-0001 §5, E4 t23): one driver crate mounted at
//! `/cf` exposing three Cloudflare primitives through the uniform cfs DSL, each on its correct
//! archetype:
//!
//! - **D1** (`/cf/d1/<db>/<table>`) — [`Archetype::RelationalTable`]. D1 is SQLite-over-HTTP, so
//!   this **reuses the t17 `cfs-driver-sql` sqlite machinery**: the [`Dialect::Sqlite`] emitter
...
//! - [`cf_apply_driver`] — wraps the applier in a [`cfs_runtime::PlanApplierBridge`] ready to
...
//! `reqwest` stays in `cfs-driver-http`; this crate rides a LOCAL [`HttpExchange`](backend::HttpExchange)
//! seam (the cfs-google-auth precedent) through the [`HttpApiBackend`] instead. The token is a [`cfs_secrets::Secret`] behind the backend.
//!
...
use cfs_sql_core::{compile, Dialect, QuerySpec};
use cfs_types::{Predicate, Row};
...
// `cfs-sql-core`) so a caller can render/inspect a D1 statement directly — D1 IS the sqlite
// dialect (t23).
pub use cfs_sql_core::{render_dml, render_select, Catalog, DmlOp, Param, SelectPlan};
...
impl CfDriver {
    ...
    pub fn execute_d1_query(
        ...
        // Reuse the t17 compiler: cfs query -> SelectPlan + truthful residual. The compiler is
...
impl Driver for CfDriver {
    ...
    fn describe(&self, path: &Path) -> Result<NodeDesc, cfs_driver::CfsError> {
        let node = CfNode::parse(path).map_err(|e| cfs_driver::CfsError::InvalidPath {
            path: path.as_str().to_string(),
            reason: match e {
        ...
        match node {
            CfNode::D1Table { db, table } => {
                ...
                let handle =
                    self.registry()
                        .d1(&db)
                        .map_err(|_| cfs_driver::CfsError::InvalidPath {
                            path: path.as_str().to_string(),
            ...
            CfNode::D1Db { .. } | CfNode::Root => Err(cfs_driver::CfsError::InvalidPath {
                path: path.as_str().to_string(),
