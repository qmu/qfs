//! [`ObjectBackend`] — the thin, **mockable** S3-compatible transport seam (RFD-0001 §9
//! no-heavy-SDK, boundary B3), plus the real [`HttpBackend`] (SigV4 over a local
//! [`HttpExchange`] seam on `cfs-http-core` — the `cfs-google-auth` / `cfs-driver-cf` precedent,
//! so this crate does NOT depend on `cfs-driver-http` and stays an independent runtime leaf), the
...
//! which the shared `cfs-http-core` redaction authority hides in every `Debug`/log. The secret is
...
/// `TransportError`); kept local so this crate does NOT depend on `cfs-driver-http`.
...
/// over (the `cfs-google-auth` / `cfs-driver-cf` `HttpExchange` precedent). A non-2xx status is
...
/// The S3-compatible transport seam every backend implements (RFD §9). The driver's `ls`/`get`/
...
/// them over the local [`HttpExchange`] seam. `reqwest` stays inside `cfs-driver-http`; no vendor
...
// --------------------------------------------------------------------------------------------
// Parked wasm R2 binding backend (RFD §8 deployment mapping: /r2 → native binding, not HTTP)
// --------------------------------------------------------------------------------------------

/// The native Cloudflare R2 Workers-binding backend (RFD §8): under `wasm32`, an `/r2` mount can
