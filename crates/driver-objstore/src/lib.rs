//! `cfs-driver-objstore` — the **object-storage driver(s)** (RFD-0001 §5, E4 t22): the
...
//!   [`cfs_runtime::SharedApplier`] the bridge drives under `COMMIT` (single PUT vs multipart with
...
//!   [`cfs_runtime::PlanApplierBridge`] ready to `register` into a `DriverRegistry`.
//!
//! ## Truthful pushdown with residual (the t20 lesson, RFD §6)
...
//! `reqwest` stays in `cfs-driver-http`; this crate rides a LOCAL [`HttpExchange`] seam over
//! `cfs-http-core` (the cf precedent), so it is an **independent runtime leaf**.
//!
//! ## Secret discipline (RFD §10)
//! Credentials are a [`cfs_secrets::Secret`] exposed only inside the signer; the `Authorization`
//! header is redacted by the shared `cfs-http-core` authority — never logged, never in an
//! [`ObjError`].
//!
...
/// The AWS/R2 least-privilege scope labels (RFD §10). Documented labels only — never a credential.
...
/// The shared object-storage driver (RFD §5) — one implementation behind both the [`S3Driver`] and
...
impl Driver for ObjDriver {
    ...
    fn describe(&self, path: &Path) -> Result<NodeDesc, cfs_driver::CfsError> {
        let node = ObjNode::parse(path).map_err(|_| cfs_driver::CfsError::InvalidPath {
            path: path.as_str().to_string(),
        ...
        match node {
            ...
            ObjNode::Root { .. } => Err(cfs_driver::CfsError::InvalidPath {
                path: path.as_str().to_string(),
    ...
    fn version_support(&self, path: &Path) -> VersionSupport {
        // A versioned bucket exposes full @versionId history (RFD §4); a non-versioned bucket only
...
fn key_prefix_of(pred: &Predicate) -> Option<(String, bool)> {
    const KEY: &str = "key";
    let is_key = |c: &cfs_types::ColRef| c.path.len() == 1 && c.path[0].as_str() == KEY;
    match pred {
