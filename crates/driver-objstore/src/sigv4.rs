...
//! machinery, and the [`cfs_secrets::Secret`] access never cross the crate's public API — only
//! the already-signed owned [`cfs_http_core::HttpRequest`] does.
//!
//! ## What SigV4 is
...
//! redacted by the shared `cfs-http-core` authority in every `Debug`/log.
...
/// `cfs-http-core` `Debug` redacts — the secret-safety invariant.
#[must_use]
pub fn sign(
    mut req: HttpRequest,
...
mod tests {
    ...
    /// redacted by cfs-http-core) — the token-safety invariant.
    #[test]
