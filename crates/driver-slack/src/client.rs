...
//! redaction authority** the t18 REST seam, t24 GitHub seam, and `cfs-google-auth` trade in — and
...
//! - **Bearer (bot-token) auth**: the token, resolved from a [`cfs_secrets::Secret`] at
...
/// `cfs-driver-http` (runtime-consumer confinement).
...
impl RestSlackClient {
    ...
    fn request(&self, method: HttpMethod, url: String) -> Result<HttpRequest, SlackError> {
        ...
        Ok(HttpRequest::new(method, url)
            ...
            .header("User-Agent", "cfs-driver-slack")
