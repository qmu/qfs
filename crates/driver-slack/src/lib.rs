//! `cfs-driver-slack` — the **Slack multi-archetype `Driver`** (RFD-0001 §5, E4 t25). It mounts
...
//! (users) — each a per-node [`Archetype`](cfs_driver::Archetype) with a typed
//! [`Schema`](cfs_types::Schema) powering
//! `DESCRIBE`. It implements the t13 [`cfs_driver::Driver`] contract and reuses the t18/t24
//! reusable-HTTP-seam *shape* — Bearer (bot-token) injection, cursor pagination, 429/`Retry-After`
//! bounded retry on idempotent GETs only — over the **shared `cfs_http_core` HTTP DTOs + the single
...
//! leak stays closed) and the driver does **not** depend on `cfs-driver-http` as a crate: a
//! `cfs-runtime` consumer must stay a leaf (the dep-direction confinement test).
//!
...
//! introspective `Driver` surface) carries **zero** `cfs-runtime`/tokio dependency (`cfs-runtime` is
...
//!
//! ## Token safety (RFD §10)
//! The bot token + the signing secret are [`cfs_secrets::Secret`]s read **only** at commit / verify
//! time — the bot token into an `Authorization: Bearer …` header the redacting
//! [`cfs_http_core::HttpRequest`] `Debug` hides, the signing secret only inside [`events`]. Neither
...
/// The per-workspace Slack configuration — an owned DTO deserialized from cfs config, **one block
/// per workspace `<ws>`** (RFD §5). The bot token + signing secret are [`CredentialKey`]
/// indirections into the [`cfs_secrets`] store; the raw values are resolved only at commit / verify
...
impl SlackDriver {
    ...
    /// Borrow the synchronous applier (e.g. to drive a `cfs_plan::commit` directly, or to build the
    /// runtime bridge).
    #[must_use]
...
impl Driver for SlackDriver {
    ...
    fn describe(&self, path: &Path) -> Result<NodeDesc, cfs_driver::CfsError> {
        ...
        let parsed = SlackPath::parse(path).map_err(|_| cfs_driver::CfsError::InvalidPath {
...
/// [`PlanApplierBridge`](cfs_runtime::PlanApplierBridge), yielding the async `ApplyDriver` ready to
...
/// builds for `wasm32-unknown-unknown` without pulling `cfs-runtime`/tokio.
#[cfg(feature = "runtime")]
#[must_use]
pub fn slack_apply_driver(driver: &SlackDriver) -> cfs_runtime::PlanApplierBridge<SlackApplier> {
    cfs_runtime::PlanApplierBridge::new(Arc::new(driver.slack_applier().clone()))
}
