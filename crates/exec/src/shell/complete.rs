...
impl<'a> Completer<'a> {
    ...
    ///
    /// - An empty / first-token line completes **builtin names** (and lets a raw cfs keyword
    ...
    pub fn complete(&self, line: &str, cwd: &VfsPath) -> Vec<String> {
        let mut out = match classify(line) {
            ...
            Line::Raw(raw) => {
                ...
                // cfs statement is left to the grammar (we do not complete keywords).
...
/// completer.
fn bounded_read(
    stmt: &cfs_parser::Statement,
    mounts: &cfs_core::MountRegistry,
    reads: &ReadRegistry,
    timeout: Duration,
