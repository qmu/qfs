...
//! SAME `cfs-exec` pipeline (parse → plan → PREVIEW/COMMIT, or parse → scan → rows). So the
//! shell logic belongs in `cfs-exec` (the integration crate that already owns that pipeline),
//! NOT in `cfs-cmd` (which the t01 C4 guard keeps logic-free) and NOT in the spine. The thin
//! `cfs` bin / `cfs-cmd` dispatches into this module when invoked with no subcommand; it owns
...
//! - [`desugar`] — line classification (builtin-at-head vs raw cfs) + lowering each builtin to
//!   closed-core cfs **source statements** (so a builtin produces the same `Plan` as the
