...
//! PREVIEW/COMMIT safety gate. This is the shell's brain — the REPL loop (in `cfs-cmd`) only
...
use cfs_core::{Engine, MountRegistry};
...
impl<'a> Session<'a> {
    ...
    /// Run a batch of cfs source statements (one for most builtins/raw lines; several for `mv`
    ...
    fn eval_statements(&self, statements: &[String], commit: bool) -> Result<Outcome, ExecError> {
        ...
        if matches!(inner, cfs_parser::Statement::Query(_)) {
...
fn unwrap_commit(stmt: &cfs_parser::Statement) -> (&cfs_parser::Statement, bool) {
