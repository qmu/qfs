...
//!
//! Black-box: this exercises ONLY the public `cfs_host` API + the checked-in golden fixture, from
...
//!  1. host-agnostic binding-set derivation from the fixture `config.cfs` via the public
...
fn scenario6_secret_values_cannot_flow_into_the_wrangler_or_a_ledger_line() {
    ...
    // (b) Even if an operator's cfs-secrets ACCOUNT-ID handle is present, the wrangler emits it
    ...
    const CANARY: &str = "cfs-CANARY-TOKEN-do-not-leak-1a2b3c4d";
    let wrangler = generate_wrangler_toml("cfs-deploy", &set);
