...
//! `cfs-http-core`. Before this crate, both `cfs-driver-http` and `cfs-google-auth` hand-copied
...
//! in the workspace — here. The sibling crates may only *re-export* it (`pub use cfs_http_core::…`)
...
fn redaction_authority_is_defined_exactly_once_in_http_core() {
    ...
    // Definition signatures (a *re-export* `pub use cfs_http_core::…` does not match these).
    ...
    assert_eq!(
        ...
        "single-source violation: `SENSITIVE_HEADERS` must be defined ONLY in \
         cfs-http-core/src/lib.rs (the lone redaction authority). A second definition reintroduces \
    ...
    assert_eq!(
        ...
        "single-source violation: `is_sensitive_header` must be defined ONLY in \
         cfs-http-core/src/lib.rs. Sibling crates must re-export (`pub use cfs_http_core::…`) or \
