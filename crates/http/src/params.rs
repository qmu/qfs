...
//! become owned, **typed** [`cfs_core::Value`]s collected in [`QueryArgs`]. These are the
//! values the injection-safe AST rewrite ([`crate::rewrite`]) substitutes into the pre-parsed
//! endpoint query.
//!
//! ## Injection safety starts here
//! A param value is converted to a typed [`cfs_core::Value`] by *literal inference* (an
...
impl QueryArgs {
    ...
    pub fn bind(
        ...
        //    undeclared one is rejected (closed contract — RFD §5/§6 honest typing).
...
/// Infer a typed [`cfs_core::Value`] from a raw param token. The inference is intentionally
