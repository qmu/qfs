//! The **policy gate** (t35, replacing the t32 read-only-default; RFD §3 purity / §10
//! least-privilege).
//!
//! An endpoint serves the *query face* of cfs (pure reads route through the read path and
//! produce an empty commit plan), but a write-lowering endpoint COMMITs a write plan. That
//! plan is now enforced against the endpoint's bound [`cfs_server::Policy`] via the REAL pure
//! enforcer [`cfs_server::evaluate`] — the same `evaluate` the cron/watchtower committers use.
//! Enforced at BOTH:
...
use cfs_server::{evaluate, policy_from_def, Policy, PolicyDecision, PolicyDef};
