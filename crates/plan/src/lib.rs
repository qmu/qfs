...
//!
//! ## Purity invariant (load-bearing, RFD §3)
...
//!
//! ## Least-privilege / no vendor leak (RFD §9/§10)
//! A [`Plan`] carries [`DriverId`] + [`VfsPath`] + owned [`cfs_types::RowBatch`] data
...
//!
//! ## Evaluator wiring (E1)
//! The evaluator that consumes the `cfs-parser` AST and emits a [`Plan`] lives in
//! `cfs-core` (the reserved `cfs-core → cfs-parser` edge, acceptance criterion C5),
...
//! AST-verb → [`EffectKind`] mapping without `cfs-plan` depending on the AST type.
//!
...
/// `cfs_parser::EffectVerb` into an [`EffectKind`] without this crate depending on the
/// parser (keeping the spine acyclic and `cfs-plan` parser-free). The evaluator maps
/// `cfs_parser::EffectVerb::{Insert,Upsert,Update,Remove}` onto these identically-named
/// variants.
...
/// evaluator's verb translation; `Remove` is inherently irreversible (RFD §10) and
