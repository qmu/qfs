...
//! [`RowBatch`](cfs_types::RowBatch) `args` (the established effect-data carrier).
//!
//! ## Why the DTO is not here (purity invariant, RFD §3)
//! `cfs-plan` is the I/O-free effect substrate; it must not depend on `cfs-server` (that
...
//! the `RwLock` write guard live in `cfs-server`. Booting a config file therefore replays
