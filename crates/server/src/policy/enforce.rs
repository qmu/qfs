...
pub fn classify_effect(kind: &EffectKind) -> EffectClass {
    match kind {
        ...
        EffectKind::ServerConfigWrite { op, .. } => EffectClass::Verb(match op {
            cfs_core::ServerWriteOp::Insert => Verb::Insert,
            cfs_core::ServerWriteOp::Upsert => Verb::Upsert,
            cfs_core::ServerWriteOp::Update => Verb::Update,
            cfs_core::ServerWriteOp::Remove => Verb::Remove,
        }),
...
mod tests {
    ...
    use cfs_core::{DriverId, EffectNode, NodeId, ProcId, Target, VfsPath};
