//! The server **runtime** (RFD-0001 §6/§8): `cfs serve <config.cfs>`.
//!
//! [`Runtime::boot`] reads a `.cfs` config file, parses it to statements, lowers each to a
...
//! [`ServerState`] changes is an [`EffectKind::ServerConfigWrite`](cfs_core::EffectKind::ServerConfigWrite)
...
use cfs_core::{commit, AppliedEffect, ApplyError, EffectKind, EffectNode, PlanApplier};
...
impl Runtime {
    ...
    /// **Boot** from a `.cfs` config file (RFD §8). Reads the file, splits it into
    ...
    pub fn boot(&mut self, cfg: &FsPath) -> Result<(), ServerError> {
        ...
        tracing::info!(target: "cfs::server", config = %cfg.display(), %summary, "boot complete");
        Ok(())
    }
