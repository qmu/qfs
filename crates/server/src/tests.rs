...
//!
//! Acceptance coverage:
//! - boot a fixture `.cfs` → deterministic [`ServerState`] snapshot; re-apply is a no-op;
...
use cfs_core::{commit, preview, Driver, EffectKind, Path, Plan, ServerNode, ServerWriteOp};
...
fn fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("fixtures/server_boot.cfs")
}
...
fn unsupported_verb_is_rejected_at_plan_time_with_structured_error() {
    ...
    use cfs_core::{check_capability, Verb};
...
fn describe_server_triggers_returns_the_trigger_schema_with_no_backend() {
    ...
    assert_eq!(desc.archetype, cfs_core::Archetype::RelationalTable);
...
fn run_loop_drains_audit_on_ctrl_c() {
    ...
    // audit ledger is flushed on exit. The full ctrl_c path is exercised by `cfs serve` E2E.
    let mut rt = Runtime::new();
    rt.boot(&fixture_path()).expect("boot");
