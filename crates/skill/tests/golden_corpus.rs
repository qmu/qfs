//! The cfs-skill **golden example corpus** (ticket t39, RFD §3/§5/§6/§8).
//!
...
//! This REUSES the t38 `cfs-test` harness (`assert_plan` / `golden` / `preview_handler`) — it does
...
//! `cfs describe` registry uses (mock clients / empty registries / fixture catalog+repo); only the
...
fn github_merge_call_resolves_declared_irreversible_proc() {
    ...
    let merge = cfs_core::resolve_proc(driver.as_ref(), "merge").unwrap();
...
mod sql_fixture {
    ...
    fn sql_registry() -> MountRegistry {
        let orders = TableCatalog::new(
            ...
            RelationKind::Table,
            vec![
                ColumnDef::new("id", cfs_core::ColumnType::Int, false, true, true),
                ColumnDef::new("total", cfs_core::ColumnType::Int, false, false, false),
            ],
        );
    ...
    fn sql_describe_reports_relational_pushdown() {
        ...
        let report =
            cfs_core::DescribeReport::from_driver(driver.as_ref(), &Path::new("/sql/pg/orders"))
                .expect("/sql/pg/orders describes");
        assert_eq!(report.archetype, cfs_core::Archetype::RelationalTable);
...
mod git_fixture {
    use super::*;
    use cfs_driver_git::{GitApplier, GitDriver, LooseObjectDb, Repo, RepoResolver};
