//! The structured SQL error (RFD-0001 §5: machine-readable for an AI, never prose, and **never**
//! secret-bearing). This is the **pure-leaf** home of [`SqlError`] (extracted into `cfs-sql-core`
...
//!
//! The runtime/secrets adapters (`From<SqlError> for cfs_runtime::EffectError`,
//! `From<cfs_secrets::SecretError> for SqlError`) live in the consuming **driver** crate
//! (`cfs-driver-sql`), NOT here — so this crate stays a pure leaf over `cfs-types` and carries no
...
pub enum SqlError {
    ...
    Credential {
        ...
        /// (The `cfs_secrets::SecretError` → `SqlError` adapter lives in the driver crate.)
        code: &'static str,
    },
}
