//! `cfs-sql-core` — the **pure-leaf SQL compile/emit core** (RFD-0001 §5/§6, extracted from t17
//! for the t23 Cloudflare D1 reuse). It owns the dialect-agnostic, **injection-safe**
//! cfs-query → parameterized-SQL machinery and the owned catalog DTOs, with **no** runtime,
//! secrets, or driver coupling — its only workspace dependency is `cfs-types`.
//!
...
//! inside `cfs-driver-sql`, but that crate is a **runtime leaf** (it depends on `cfs-runtime` for
...
//! same single-source pattern as `cfs-http-core` — lets BOTH driver crates reuse one emitter
