...
//! credential-shape scrub (t38, RFD §3/§5/§9/§10).
//!
...
//!
//! ## Determinism (the hard part, RFD §6)
...
//! When the env var `CFS_BLESS=1` is set, [`assert_golden`] **writes** the fixture instead of
//! comparing — the single, obvious update path (`CFS_BLESS=1 cargo test -p cfs-test`). With
...
/// timestamp / request-id never flaps a golden (RFD §6 determinism).
...
pub fn canonical_json<T: Serialize>(value: &T) -> String {
    let raw = serde_json::to_value(value).unwrap_or_else(|e| {
        panic!("cfs-test golden: value is not serializable to JSON: {e}");
    });
    let canon = canonicalize(raw);
    let mut out = serde_json::to_string_pretty(&canon).unwrap_or_else(|e| {
        panic!("cfs-test golden: canonical value did not re-serialize: {e}");
    });
    out.push('\n');
    out
}
...
/// under `CFS_BLESS=1`, (re)write the fixture — the cargo-native bless workflow.
///
...
///
/// # Panics
/// - Under `CFS_BLESS=1`: panics only if the fixture file cannot be written (an environment
...
pub fn assert_golden<T: Serialize>(name: &str, value: &T) {
    ...
    if std::env::var_os("CFS_BLESS").is_some() {
        std::fs::create_dir_all(&dir).unwrap_or_else(|e| {
            panic!("cfs-test bless: cannot create fixtures dir {dir:?}: {e}");
        });
        std::fs::write(&path, actual.as_bytes()).unwrap_or_else(|e| {
            panic!("cfs-test bless: cannot write fixture {path:?}: {e}");
        });
        return;
    }
    ...
    let expected = std::fs::read_to_string(&path).unwrap_or_else(|_| {
        panic!(
            "cfs-test golden `{name}`: fixture {path:?} is missing.\n\
             Seed it with:  CFS_BLESS=1 cargo test -p cfs-test"
        );
    });

    assert!(
        actual == expected,
        "cfs-test golden `{name}` drift (fixture {path:?}).\n\
         --- expected (checked-in) ---\n{expected}\n\
         --- actual (this run) ---\n{actual}\n\
         If the change is intended, re-bless:  CFS_BLESS=1 cargo test -p cfs-test"
    );
}

// ---------------------------------------------------------------------------
// Credential-shape scrub (RFD §10): a golden must contain no token-shaped string.
// ---------------------------------------------------------------------------

/// Assert a rendered golden contains **no credential-shaped string** (RFD §10): a "passing"
    ...
    for shape in SECRET_SHAPES {
        assert!(
            ...
            "credential-shape leak: golden contains `{shape}` — a token shape must never appear \
             in an owned-DTO snapshot (RFD §10). Rendered:\n{rendered}"
        );
    }
}
