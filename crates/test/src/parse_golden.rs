...
//!
//! The owned [`cfs_parser::Statement`] AST is `Serialize`, so a golden is just its canonical
...
use cfs_parser::{parse_statement, ParseError, Statement};
...
pub fn golden_parse(src: &str) -> AstSnapshot {
    let ast = parse_statement(src)
        .unwrap_or_else(|e| panic!("cfs-test golden_parse: `{src}` did not parse: {e}"));
    AstSnapshot {
...
pub fn error_snapshot(src: &str) -> ParseErrorSnapshot {
    match parse_statement(src) {
        Ok(_) => panic!("cfs-test error_snapshot: `{src}` parsed successfully (expected an error)"),
