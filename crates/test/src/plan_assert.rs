...
//! (by node id, the dense order the [`cfs_core::PlanBuilder`] allocates) and edges are sorted
...
use cfs_core::{EffectKind, EffectNode, EvalValue, Evaluator, MountRegistry, NodeId, Plan};
...
/// Evaluate `src` (a cfs statement) against `reg` to its effect [`Plan`] and return the
...
pub fn assert_plan(src: &str, reg: &MountRegistry) -> PlanAssert {
    let stmt = parse_statement(src)
        .unwrap_or_else(|e| panic!("cfs-test assert_plan: `{src}` did not parse: {e:?}"));
    let evaluator = Evaluator::new(reg);
    let value = evaluator
        .eval(&stmt)
        .unwrap_or_else(|e| panic!("cfs-test assert_plan: `{src}` did not evaluate: {e:?}"));
    match value {
        EvalValue::Plan(plan) => PlanAssert { plan },
        EvalValue::Relation(_) => {
            panic!("cfs-test assert_plan: `{src}` is a pure read, not an effect statement")
        }
    }
}
...
impl PlanAssert {
    ...
    pub fn no_io_performed(self) -> Self {
        self.plan
            .validate()
            .unwrap_or_else(|e| panic!("cfs-test no_io_performed: plan is not a valid DAG: {e}"));
        self
    }
    ...
    /// map keys and redacts non-deterministic fields. Compares against (or, under `CFS_BLESS=1`,
