//! [`WatchtowerBinding`] — the top-level [`cfs_server::Binding`] (kind `Ingest`) that owns the
...
//! the desired-watcher snapshot — the same pattern as the cfs-http listener reading the route
...
use cfs_server::{Binding, BindingKind, ServerError, ServerState, TriggerDef};
...
/// daemon reads. Constructed by the `cfs` binary's serve composition root.
...
impl WatchtowerBinding {
    ...
    /// routes `/hooks/...` to `ingest` — option b: cfs-watchtower serves no HTTP itself).
    #[must_use]
