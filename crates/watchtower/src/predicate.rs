...
//!   2. lowers it to a typed [`cfs_core::Predicate`] via [`cfs_pushdown::lower_predicate`]
...
use cfs_core::{CmpOp, ColRef, Literal, Pattern, Predicate, Row, Value};
use cfs_parser::{PipeOp, Statement};
...
pub fn guard_matches(guard: &Statement, columns: &[String], new: &Row) -> Result<bool, GuardError> {
    ...
    let predicate =
        cfs_pushdown::lower_predicate(expr).map_err(|e| GuardError::Lower(e.to_string()))?;
...
/// match) rather than panicking. Lives here (not reused from `cfs-engine`, where it is
...
fn resolve(col: &ColRef, columns: &[String], row: &Row) -> Option<Value> {
    ...
    let path: &[cfs_core::Name] = match col.path.split_first() {
...
mod tests {
    use super::*;
    use cfs_parser::{Pipeline, Source, Values};
    ...
    fn guard_stmt(where_src: &str) -> Statement {
        ...
        let stmt = cfs_parser::parse_statement(&src).expect("parse trigger");
        let cfs_parser::Statement::Ddl(d) = stmt else {
            panic!("expected ddl")
        };
