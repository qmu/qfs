...
export const qfsGrammar = {
  ...
  repository: {
    ...
    keyword: {
      match:
        '(?i)\\b(INSERT INTO|UPSERT INTO|MATERIALIZED VIEW|GROUP BY|ORDER BY|TRANSACTION|CREATE|ENDPOINT|TRIGGER|JOB|VIEW|WEBHOOK|POLICY|LET|DEF|FROM|WHERE|SELECT|EXTEND|SET|AGGREGATE|LIMIT|DISTINCT|JOIN|UNION|EXCEPT|INTERSECT|EXPAND|UPDATE|REMOVE|VALUES|RETURNING|CALL|DECODE|ENCODE|PREVIEW|COMMIT|ALLOW|DENY|EVERY|DO|ON|AS|AND|OR|NOT|IN|ANY|BETWEEN|LIKE|ASC|DESC|NEW)\\b',
      name: 'keyword.control.qfs',
    },
    pipe: { match: '\\|>', name: 'keyword.operator.pipe.qfs' },
    // Higher-order / stdlib functions used as values (the M6 functional core), e.g. map(xs, f).
    builtin: {
      match: '(?i)\\b(map|filter|reduce)\\b(?=\\s*\\()',
      name: 'support.function.builtin.qfs',
    },
    ...
    // `==` (equivalence) and `=>` must precede the single `=` (ordered alternation, not longest-match).
