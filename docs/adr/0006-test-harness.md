...
- **Ticket**: t38 — Test harness: per-driver fakes, plan assertions, golden tests. The cross-cutting
  `cfs-test` dev-only crate that lets every other epic be proven correct offline, in CI, and in `wasm32`.
...
patterns into one `cfs-test` dev-only crate. The ticket *suggested* `insta` (snapshots),
...
**Build dependency-light in-house equivalents, consistent with ADR-0001..0005.** `cfs-test`
depends only on the pure spine (`cfs-core`, `cfs-parser`, `cfs-http-core`) plus `serde`/
...
1. **Golden / snapshot = canonical JSON, not `insta`.** A golden serializes an owned DTO
   ...
   `tests/fixtures/*.json`. The update path is a cargo-native **`CFS_BLESS=1 cargo test`** env
...
3. **MockHttp = a scripted in-memory transport, not `httptest`/`wiremock`.** A `RefCell`-backed
   FIFO of canned responses + a recorded-request log, built on the **pure** `cfs-http-core` DTOs
   (`HttpMethod`/`HttpRequest`/`HttpResponse` + the redacting `Debug`). Wasm-clean (no socket, no
   threads, no reqwest). The real reqwest `HttpClient` stays in `cfs-driver-http`, never pulled
   into the harness. This is the same recording/scripted pattern `cfs-driver-http::MockHttpClient`
   already established.

4. **Fakes reuse the existing apply seam.** `FakeBackend` **is** a `cfs_core::PlanApplier` — the
...
- **Positive.** No new uncached/heavy dependency; the harness builds on the tight disk and on
  ...
  dep-graph test proves the `cfs` binary never links `cfs-test`).
- **Negative / trade-offs.** (a) No interactive snapshot review UI (`cargo insta review`); the
  diff is shown in the panic message and re-blessed via `CFS_BLESS=1` — acceptable, and the diff is
  ...
  `cfs-driver-http` for the runtime's blocking apply threads.
...
`cargo metadata`-based dep test asserts each wasm-gated leaf (`cfs-cron`/`cfs-watchtower`/`cfs-host`/
`cfs-driver-slack`) builds its pure core **without tokio** under `--no-default-features` — the same
