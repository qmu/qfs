...
`cfs --version`, SemVer). Two forces shaped the decision:
...
`docs/language.md`, `docs/drivers.md`, and `docs/server.md` are rendered by `cfs::docs` from the
binary's live data, never hand-authored:

- **language.md** ← `cfs_lang::RESERVED_KEYWORDS` (an *alias of* the one frozen `KEYWORDS` slice —
  no second transcription) + `cfs_lang::grammar_ebnf()`.
- **drivers.md** ← `cfs::catalog::driver_catalog()`, built by walking the **existing** t39 describe
...
A committed-docs-vs-generated golden test (`cfs::docs::tests::committed_docs_match_generated_output`,
in an **existing** test binary) and `cargo run -p xtask -- gen-docs --check` (CI) fail on any drift.
A frozen-keyword test (`cfs_lang::reference::tests`) locks `RESERVED_KEYWORDS` to the 38-entry §3
...
recompile, which the 147M-class disk cannot survive. Instead `cfs::catalog` folds each driver's
...
external dependency — the `cfs` path crate (whose new `[lib]` facet exposes the doc generator). It
adds zero uncached crates. `cfs` gained a thin `[lib]` so both `main.rs` and `xtask` reuse the same
...
**not run locally**. `xtask dist` refuses to execute unless `CFS_DIST_ALLOW=1` (never set locally),
...
installs a runnable `cfs`. No live credential ever appears in a doc, an example, a golden, or a
...
- **Positive.** Docs cannot drift (generated + golden-locked). No driver recompile was forced. The
  ...
  binary stays lean (xtask never ships); `cfs --version` is the field-debug anchor.
