...
skill_description: Use when a task needs to query or modify a SQL database through qfs — filter, aggregate, join, update, and set operations over /sql/<conn>/<table> relational tables, plus creating and dropping tables by writing to the /sql/<conn> catalog (SQLite, Postgres, MySQL).
---
