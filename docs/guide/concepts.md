...
Five ideas explain almost everything in qfs. None of them is complicated. For the complete current
operator model — state stores, mounts, OAuth app labels, `/sys`, DDL history, dump/restore, and
automation — see the [current design snapshot](/guide/design-snapshot).
...
— run `qfs app add google <app>`, `qfs account add google <email> --app <app>`, then `qfs connect <path> --driver gmail --account <email>`" }}
```
...
They only exist in the [interactive shell](/guide/shell). `cd` walks a **blob** namespace: `/local`
(always mounted) and connected blob services such as `/drive`. A relational mount is queried, not
walked (`cd /sql/…` does not apply), and a service you have not connected reports `unknown_mount`. To
join across services you use the query language (`/sql/…`, `/git/…`, `/mail/…`), not `cd`.
