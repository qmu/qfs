...
### Mounts available in the shell

The shell always mounts the `local` driver, rooted at the directory you launched `qfs` from — that
directory is the `/local` sandbox boundary the session cannot escape. Alongside it, the shell mounts
the same path surface `qfs run` sees: every service you have connected (a `/sql/<name>` or
`/git/<name>` connection, and cloud accounts such as `/mail`, `/drive`, `/github`, `/slack`, `/s3`,
`/r2`, `/cf`) plus the `/sys` administration surface. So `cp /local/report.pdf
/drive/my/report.pdf` resolves both paths and copies the file — qfs materializes the local bytes at
the commit boundary. A service you have **not** connected fails closed (`cd /drive` →
`error[unknown_mount]: no driver is mounted there`). Reads are pure; a write to any mount previews
first and applies only after you type `COMMIT` (an irreversible action needs the extra
confirmation), so the preview→commit gate governs cloud writes from the shell exactly as it does on
`qfs run`.
