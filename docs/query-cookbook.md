...
an actionable error (for Google, run `qfs app add google <app>`,
`qfs account add google <email> --app <app>`, then
...
**Compute invoice line totals before projecting.**

```qfs
# qfs-cookbook: grammar=core; milestone=M0; features=where,extend,arithmetic,select,order by
/sql/pg/invoice_lines
|> where status == 'open'
|> extend line_total = unit_price * qty
|> select invoice_id, sku, qty, unit_price, line_total
