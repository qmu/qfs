...
|---------|-------------------|--------------|
| **Identity** | The operator identity is registered by `qfs init` with **no password**: authentication is delegated to the OS login (one operator per OS user; the email is an accountability label), so there is no password hash to store or leak. Identity is authentication, **not** authorization — a registered operator holds no capability until a policy/ACL grants one. | — |
...
| **Credential lifecycle** | `qfs account rotate` (re-mint in place), `qfs account revoke` (mark unresolvable, fail-closed at bind), and `qfs vault rekey` (re-wrap the store data-key under a new passphrase) — the offboarding + key-hygiene answers. | — |
