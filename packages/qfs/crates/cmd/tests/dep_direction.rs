...
fn binary_is_the_thin_entrypoint_plus_the_t28_shell_composition_root() {
    ...
    let allowed = [
        ...
        "qfs-driver-sys",
        // §15 transform definitions (decision W): the `/transform` driver is a qfs-runtime consumer
        // that must stay a LEAF — only the terminal binary depends on it — so the production
        // rusqlite-backed TransformBackend lives IN the binary (src/transform.rs), like SysBackend.
        "qfs-driver-transform",
        // §5.4/§5.5 declared types: the `/type` CATALOG driver — the read face over the
        // `kind='type'` declarations that makes `ls /type` = SHOW TYPES. Unlike its `/transform`
        // sibling it is NOT a qfs-runtime consumer (it is read-only: no applier, so no
        // PlanApplierBridge — hence its absence from the runtime-leaf allowlist below), but the same
        // decision-F rule puts its rusqlite-backed read source IN the binary (src/type_catalog.rs).
        "qfs-driver-type",
        // blueprint §16 provisioning: the binary wires `qfs plan`/`qfs apply` (src/provision.rs) over
        // the pure fetch->diff->reconcile core. It owns the current-config fetch (System/Project DB
        // reads + the daemon statement-face transport) and COMPOSES qfs-provision's generic
        // ReconcileApplier with the concrete `SysApplier` here (the terminal leaf where the runtime
        // dead-ends). qfs-provision stays off qfs-driver-sys, so that runtime consumer stays a leaf;
        // the `/server` ServerState DTOs come re-exported THROUGH qfs-provision, so the binary keeps
        // NO direct qfs-server edge (the lower-spine ban above still holds).
        "qfs-provision",
        ...
        // binary (src/claude.rs) and its std::fs reads/append dead-end here. Decision W: the
        // `/claude` driver is a path façade over session metadata + an append-log, never an
        // LLM/inference dependency (qfs's model-calling surface is `|> transform`, §15, whose
        // injected provider this same guard keeps OUT of the pure engine).
        "qfs-driver-claude",
        ...
        "qfs-store",
        // ticket 20260704170000 (time-boxed session unlock): the binary derives the machine/session-
        // bound KEK that wraps the cached DEK from the blessed pure SHA-256 leaf (t34). qfs-crypto-core
        // is a TRUE pure leaf (zero deps, no tokio/rusqlite/driver), consumed off the hot path, so the
...
fn types_is_a_leaf_and_codec_depends_on_it() {
    ...
    // The canonical row model and §5 type-surface vocabulary flow up: codec, core, and parser
    // depend on qfs-types. The parser edge is still downward into the leaf, not the semantic
    // `qfs-core -> qfs-parser` edge guarded below.
    ...
    assert!(
        parser_deps.iter().any(|d| d == "qfs-types"),
        "qfs-parser must depend on qfs-types for the shared §5 type-surface vocabulary"
    );
...
fn runtime_is_confined_to_plan_and_types() {
    ...
    // stage (blueprint §3/§7 COMMIT); tokio/futures live there and MUST NOT leak into the spine.
    ...
    let runtime_consumers_allowed = [
        ...
        // terminal binary depends on it — so tokio still dead-ends in the binary.
        "qfs-driver-sys",
        // §15: the `/transform` definition driver bridges its TransformApplier into the runtime via
        // qfs-runtime's PlanApplierBridge (like every other driver leaf). It is a leaf — only the
        // terminal binary depends on it — so tokio still dead-ends in the binary.
        "qfs-driver-transform",
...
fn secrets_is_confined_to_types_and_core_consumes_it() {
    ...
    // (blueprint §8). It is consumer-side, owned-DTO only, and reuses the canonical
