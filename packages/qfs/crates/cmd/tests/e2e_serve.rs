...
//! the REAL `qfs serve` binary as a subprocess and assert its observable behaviour (exit
//! status + the secret-free tracing it emits). The remaining scenarios drive the
//! `qfs-server` PUBLIC API as a black box — the embedder/agent surface — observing the
//! COMMITTED EFFECT (the resulting `ServerState`), never private internals. No live creds,
//! no network: `/server` writes are in-memory.
//!
//! ## Why these live in `qfs-cmd`, not in the `qfs` binary crate
//! The `qfs` binary crate is guarded (by `tests/dep_direction.rs`) to depend on `qfs-cmd`
//! ONLY among workspace crates. So the E2E lives one layer down in `qfs-cmd` (which already
//! owns the `serve` dispatch and depends on `qfs-core` + `qfs-server`) and locates the built
//! `qfs` binary next to the test runner.
//!
//! ## Why no `qfs-parser` here (deliberate constraint)
//! `tests/dep_direction.rs` forbids `qfs-cmd` from depending on `qfs-parser` — and the guard
...
//!  1. Boot the real binary: `qfs serve <fixture>` boots without network/creds, reaches the
...
use qfs_core::{check_capability, Archetype, Driver, Path, Verb};
use qfs_server::driver::ServerDriver;
use qfs_server::{
...
fn serve_shuts_down_cleanly_on_sigterm_and_drains_audit() {
    ...
    // as the SIGINT path does. This pins the systemd `qfs.service` graceful-drain claim.
    let mut child = Command::new(qfs_bin())
        .args(["serve", fixture_path().to_str().unwrap()])
        .env("RUST_LOG", "qfs::server=info,qfs::server::audit=info")
        .env("NO_COLOR", "1")
...
/// the whole boot unit without ever touching `qfs-parser` directly.
...
fn secret_hygiene_logged_projections_never_render_a_canary_verbatim() {
    ...
    // construction); the audit summary + ServerState::summary are counts/names only (blueprint §8).
