...
//! single row (blueprint §4: "a markdown file with YAML frontmatter becomes a queryable table,
...
//! are serialized back as a key-stable YAML frontmatter block (deterministic, blueprint §7).
//!
//! Documented non-preservation (blueprint §4): comments and exact whitespace in the
...
impl Codec for MarkdownFrontmatterCodec {
    ...
    fn decode(&self, bytes: &[u8]) -> Result<RowBatch, CfsError> {
        ...
        if let Some(fm) = frontmatter {
            let parsed: serde_json::Value =
                serde_yaml_ng::from_str(fm).map_err(|e| CfsError::Decode {
                    fmt: "md",
    ...
    fn encode(&self, batch: &RowBatch) -> Result<Vec<u8>, CfsError> {
        ...
        if !frontmatter.is_empty() {
            let yaml =
                serde_yaml_ng::to_string(&serde_json::Value::Object(frontmatter)).map_err(|e| {
                    CfsError::Encode {
                        fmt: "md",
                        detail: e.to_string(),
                    }
...
fn split_frontmatter(text: &str) -> (Option<&str>, &str) {
    ...
    // Unterminated frontmatter: treat the whole thing as body (lenient, blueprint §4).
    (None, text)
}
