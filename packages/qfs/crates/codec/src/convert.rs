//! The shared JSON-tree ↔ `qfs-types` bridge (t15). Every structured codec
//! (`json`, `jsonl`, `yaml`, `toml`, `md+frontmatter`) parses its bytes into a
//! `serde_json::Value` tree (`serde_yaml_ng`/`toml` deserialize *into* `serde_json::Value`
...
//! schema-inference logic live in exactly one place (blueprint §4).
//!
//! The vendor type (`serde_json::Value`) is confined to this module's signatures and
//! the codec impls — it never crosses the `qfs-codec` public boundary (blueprint §11).

use qfs_types::{Column, Fields, Row, RowBatch, Schema, Value};

/// Map a single parsed JSON node to a qfs [`Value`] (blueprint §4):
...
///   [`Value::Json`] (the irregular fallback — decode never fails on shape, blueprint §4).
///
...
/// schema inferred by **unifying** every row's structural schema (blueprint §4 schema
...
/// own schema contributes [`Value::Null`] (blueprint §4 missing-column rule).
...
/// schema columns). Used by every structured encoder. Determinism (blueprint §7): column
...
/// Map a qfs [`Value`] back to a JSON node. Inverse of [`json_to_value`]; a
...
pub fn value_to_json(value: &Value) -> serde_json::Value {
    match value {
        ...
        // than failing, keeping encode total (blueprint §4 irregular-data tolerance).
