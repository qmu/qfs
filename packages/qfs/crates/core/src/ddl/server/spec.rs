//! [`StatementSpec`] / [`PlanSpec`] — the deferred-body serializable specs (t31, blueprint §10).
//!
...
fn normalize_pipe_op(op: &mut PipeOp) {
    match op {
        ...
        // TRANSFORM carries a span in its ref; zero it for a byte-stable canonical form.
        ...
        // SWITCH (blueprint §18): zero the stage/arm/write spans and recurse into each arm's
        // continuation ops for the same byte-stable canonical form.
        PipeOp::Switch(s) => {
            s.span = ZERO;
