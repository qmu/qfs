...
#[test]
fn a_transform_stage_inside_a_server_body_is_refused_at_store_time() {
    // §15 containment (decision W): a `|> transform` stage baked into a stored server-binding body
    // would spend model tokens unattended on every fire — refused with a structured error.
    for src in [
        "CREATE VIEW risky AS /mail/inbox |> transform classify",
        "CREATE ENDPOINT e ON 'GET /x' AS /mail/inbox |> transform classify",
        // Nested: a transform hiding in a subquery / JOIN source is caught by the whole-tree walk.
        "CREATE VIEW risky AS /sql/db/t |> JOIN (/mail/inbox |> transform c) ON id == id",
    ] {
        let err = parse_server_binding_ddl(src).expect_err("must refuse a transform in the body");
        assert_eq!(
            err.code(),
    ...
    // A body WITHOUT a transform is still accepted (the guard is precise, not blanket).
    assert!(
        parse_server_binding_ddl("CREATE VIEW ok AS /mail/inbox |> WHERE unread == true").is_ok()
    );
}
