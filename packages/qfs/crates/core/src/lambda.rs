//! The **pure lambda evaluator** (blueprint §3 purity invariant, M6 ticket t61): the value
...
//!
//! ## Purity (the safety property, blueprint §3)
...
/// effect can never be named here (a lambda is pure, blueprint §3); an unsupported body form is a
...
pub fn eval_expr(
    ...
    match expr {
        ...
        // The predicate forms (`LIKE` / `IN` / `BETWEEN` / `ANY`): row-local boolean tests a
        // refinement predicate (blueprint §5.4) evaluates over the bound row. All pure — value in,
        // boolean out, no I/O.
        Expr::Like { expr, pattern } => {
            let subject = eval_expr(expr, env, stdlib, ctx)?.into_data()?;
            let pat = eval_expr(pattern, env, stdlib, ctx)?.into_data()?;
            let result = match (&subject, &pat) {
                (Value::Text(s), Value::Text(p)) => like_match(s, p),
                _ => false,
            };
            Ok(LambdaValue::Data(Value::Bool(result)))
        }
        Expr::In { expr, set } => {
            let subject = eval_expr(expr, env, stdlib, ctx)?.into_data()?;
            let mut found = false;
            for member in set {
                let m = eval_expr(member, env, stdlib, ctx)?.into_data()?;
                if values_equal(&subject, &m) {
                    found = true;
                    break;
                }
            }
            Ok(LambdaValue::Data(Value::Bool(found)))
        }
        Expr::Between { expr, low, high } => {
            let subject = eval_expr(expr, env, stdlib, ctx)?.into_data()?;
            let lo = eval_expr(low, env, stdlib, ctx)?.into_data()?;
            let hi = eval_expr(high, env, stdlib, ctx)?.into_data()?;
            let within = compare(&subject, &lo)? != std::cmp::Ordering::Less
                && compare(&subject, &hi)? != std::cmp::Ordering::Greater;
            Ok(LambdaValue::Data(Value::Bool(within)))
        }
        Expr::AnyOp { op, expr, set } => {
            let subject = eval_expr(expr, env, stdlib, ctx)?.into_data()?;
            let mut satisfied = false;
            for member in set {
                let m = eval_expr(member, env, stdlib, ctx)?.into_data()?;
                let ok = match op {
                    Op::Eq => values_equal(&subject, &m),
                    Op::Ne => !values_equal(&subject, &m),
                    Op::Lt => compare(&subject, &m)? == std::cmp::Ordering::Less,
                    Op::Le => compare(&subject, &m)? != std::cmp::Ordering::Greater,
                    Op::Gt => compare(&subject, &m)? == std::cmp::Ordering::Greater,
                    Op::Ge => compare(&subject, &m)? != std::cmp::Ordering::Less,
                    Op::Add
                    | Op::Sub
                    | Op::Mul
                    | Op::Div
            ...
            Ok(LambdaValue::Data(Value::Bool(satisfied)))
        }
        // Any remaining future form is outside the small pure expression surface a lambda
        // body uses — a structured error, never a panic.
...
/// A minimal, anchored `LIKE` matcher: `%` matches any run (including empty), `_` matches any
/// single character; every other character is literal. Mirrors the engine's residual matcher so a
/// refinement's `LIKE` evaluates the same on the write/`OF` boundary as it would in a `WHERE`.
fn like_match(s: &str, pattern: &str) -> bool {
    like_inner(s.as_bytes(), pattern.as_bytes())
}

fn like_inner(s: &[u8], p: &[u8]) -> bool {
    match p.split_first() {
        None => s.is_empty(),
        Some((b'%', rest)) => (0..=s.len()).any(|i| like_inner(&s[i..], rest)),
...
/// Evaluate a boolean / comparison / arithmetic binary operator over two values. Arithmetic is
/// monomorphic by design: no implicit Int/Float promotion, and Int division is rejected until it
/// has its own explicit language ruling.
fn eval_binary(
    op: Op,
    lhs: &Expr,
    rhs: &Expr,
    ...
    let b = match op {
        ...
        Op::Add | Op::Sub | Op::Mul | Op::Div => return eval_arithmetic(op, &l, &r),
...
fn eval_arithmetic(op: Op, lhs: &Value, rhs: &Value) -> Result<LambdaValue, EvalError> {
    let out = match (op, lhs, rhs) {
        (Op::Add, Value::Int(a), Value::Int(b)) => {
            Value::Int(a.checked_add(*b).ok_or_else(arithmetic_overflow)?)
        }
        (Op::Sub, Value::Int(a), Value::Int(b)) => {
            Value::Int(a.checked_sub(*b).ok_or_else(arithmetic_overflow)?)
        }
        (Op::Mul, Value::Int(a), Value::Int(b)) => {
            Value::Int(a.checked_mul(*b).ok_or_else(arithmetic_overflow)?)
        }
        (Op::Add, Value::Float(a), Value::Float(b)) => Value::Float(a + b),
        (Op::Sub, Value::Float(a), Value::Float(b)) => Value::Float(a - b),
        (Op::Mul, Value::Float(a), Value::Float(b)) => Value::Float(a * b),
        (Op::Div, Value::Float(_), Value::Float(b)) if *b == 0.0 => {
            return Err(EvalError::Fn(FnError::Domain {
                name: "arithmetic".to_string(),
                reason: "divide_by_zero",
            }))
        }
        (Op::Div, Value::Float(a), Value::Float(b)) => Value::Float(a / b),
        (Op::Div, Value::Int(_), Value::Int(_)) => {
            return Err(EvalError::Fn(FnError::Domain {
                name: "arithmetic".to_string(),
        ...
        _ => {
            return Err(EvalError::Fn(FnError::Type {
                ...
                found: type_label(lhs),
            }))
        }
    };
    Ok(LambdaValue::Data(out))
}

fn arithmetic_overflow() -> EvalError {
