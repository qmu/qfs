//! Name resolution (blueprint §3, ticket t06) — the semantic pass that sits between
...
//!
//! ## Purity invariant (blueprint §3, the safety property)
...
/// to (blueprint §3). `driver` is the plan [`DriverId`] (e.g. `mail`); `proc` is the
...
/// Everything the evaluator needs to lower a **terminal** `|> CALL driver.proc(args)` pipeline
/// (see [`Resolver::resolve_call_lowering`]). Distinguishes the two shapes a terminal CALL can
/// take: an **effect** procedure (`returns_rows == false` — `mail.send`, `drive.copy`, …) lowers
/// to a [`Call`](qfs_plan::EffectKind::Call) effect node; a **result-returning** procedure
/// (`returns_rows == true` — a declared `search`) stays a read relation whose rows the agent
/// consumes.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CallLowering {
    /// The resolved proc identity + per-procedure irreversibility.
    pub resolved: ResolvedCall,
    /// The declared parameter names, in order — used to name a positional argument for the
    /// applier's column-keyed decode.
    pub params: Vec<String>,
    /// Whether the procedure declares a result schema (returns rows) rather than being an effect.
    pub returns_rows: bool,
}

/// The structured, machine-readable outcome of name resolution (blueprint §6: errors are
...
/// string-parsing. Credentials/secrets never appear here (blueprint §8).
#[derive(Debug, Clone, PartialEq, Eq)]
#[non_exhaustive]
pub enum ResolveError {
    ...
    /// (capability gate, blueprint §3). Carries the driver's available procedures so the AI
    /// can pick a real one.
    UnknownProcedure {
    ...
    /// it in its prelude (receiver-typed resolution, blueprint §3).
    AliasNotProvided {
    ...
    /// the user to the qualified `CALL` (blueprint §3 ambiguity policy).
    AmbiguousAlias {
        /// The alias surface name.
        name: String,
    ...
    /// than guess (blueprint §3 receiver typing).
    UnknownReceiver {
    ...
    /// resolve-time capability gate (blueprint §6). Wraps the structured driver-side error.
    UnsupportedVerb {
        /// The target path.
        path: String,
...
/// The name resolver (blueprint §3, t06): a pure pass over a parsed [`Statement`] that binds
...
impl<'r> Resolver<'r> {
    ...
    /// Resolve a read pipeline. The **receiver driver** (blueprint §3 receiver typing) is the
    ...
    fn resolve_pipe_op(
        ...
        match op {
            ...
            // SWITCH (blueprint §18): each arm's continuation runs over the routed partition of
            // the SAME receiver — resolve every arm's ops (a terminal effect `CALL` inside an
            // arm is capability-gated here, exactly like a top-level terminal CALL), and gate
            // every arm WRITE's verb against its target's capabilities (the same
            // `resolve_effect` gate a top-level write passes) — a denied arm verb fails before
            // any plan exists, exactly the §6 posture.
            PipeOp::Switch(s) => {
                for arm in &s.arms {
                    for op in &arm.ops {
                        self.resolve_pipe_op(op, scope, receiver, out)?;
                    }
                    if let Some(w) = &arm.write {
                        let gate = EffectStmt {
                            verb: w.verb,
                            target: w.target.clone(),
                            body: qfs_parser::EffectBody::SetWhere {
                                set: Vec::new(),
                                filter: None,
                            },
                            returning: w.returning.clone(),
                        };
                        self.resolve_effect(&gate, scope, out)?;
                        if let Some(projs) = &w.returning {
                            for p in projs {
                                if let Projection::Expr { expr, .. } = p {
                                    self.resolve_alias_in_expr(expr, receiver, out)?;
                                }
            ...
            // Pure structural / codec ops carry no callables to resolve here. TRANSFORM
            // names a declared definition resolved later against the transform registry (at
            // execution planning), not a driver call — nothing to resolve at this stage.
            // FOLLOW is likewise a declared-body stage naming a delivered field, not a
            // callable — the declared evaluator (not this resolver) gives it meaning.
            // OF is a plan-time type assertion (blueprint §5.6): it carries no callable and no
            // argument expression — its structural/refinement check happens against the computed
            // schema in the evaluator's `fold_op`, not here.
            PipeOp::Limit(_)
            | PipeOp::Distinct
            | PipeOp::As(_)
            | PipeOp::Expand(_)
    ...
    fn resolve_expr_fns(
        ...
        match expr {
            ...
            // is a pure value, never an effect — blueprint §3 purity).
    ...
    fn resolve_call(&self, call: &CallRef) -> Result<ResolvedCall, ResolveError> {
        Ok(self.resolve_call_lowering(call)?.resolved)
    }

    /// Resolve a single `CALL driver.action(args)` to everything the evaluator needs to lower a
    /// **terminal** `|> CALL` pipeline (see [`CallLowering`]). The validation (unknown driver/proc,
    /// arity, named-arg param names) is identical to [`resolve_call`](Self::resolve_call) — that
    /// method delegates here and keeps only the resolved identity.
    pub fn resolve_call_lowering(&self, call: &CallRef) -> Result<CallLowering, ResolveError> {
        ...
        Ok(CallLowering {
            resolved: ResolvedCall::new(driver.id(), &call.action, sig.irreversible),
            params: sig.params.iter().map(|p| p.name.clone()).collect(),
            returns_rows: sig.returns.is_some(),
        })
    }
    ...
    /// Determine the receiver driver of a pipeline source (blueprint §3 receiver typing). Only
    ...
    /// Gate an effect statement's verb against the target node's capabilities (blueprint §6).
    fn resolve_effect(
        &self,
        effect: &EffectStmt,
        scope: &Scope,
...
/// The receiver driver context threaded down a pipeline's `|>` walk (blueprint §3 receiver
