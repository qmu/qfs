//! **Aggregate** built-ins over groups (blueprint §3, ticket t08): `COUNT`/`SUM`/`AVG`/
...
//!
//! ## Aggregate-vs-scalar dispatch (blueprint §3, the typed-error rule)
...
/// A factory for a fresh [`AggregateState`] — one per group (blueprint §3 init/accumulate/
...
/// The running state of one aggregate over one group (blueprint §3). `accumulate` folds each
...
impl AggregateState {
    ...
    /// Produce the group's final value (blueprint §3). `COUNT` is always an `Int`; `SUM`/`AVG`
