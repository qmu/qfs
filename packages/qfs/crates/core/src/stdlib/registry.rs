...
//! merges driver **preludes** (blueprint §3/§6, ticket t08).
//!
...
//!
//! ## Purity invariant at registration (blueprint §3)
...
/// One alias a driver declares in its prelude (blueprint §6). The `body` is **qfs source** (a
...
/// What a driver ships alongside its procedures (blueprint §6 Prelude). A set of receiver-typed
...
/// A parsed, purity-checked prelude alias (blueprint §3) — the `(name, desugars_to)` binding
...
/// The structured error a prelude registration can raise (blueprint §6). Carries the driver +
...
/// preludes (blueprint §3, ticket t08). The surface t06 resolves `fn(...)` against.
#[derive(Default)]
...
impl StdlibRegistry {
    ...
    /// stdlib edit, by design — the core vocabulary is small and stable, blueprint §3).
    #[must_use]
    pub fn with_core() -> Self {
        let mut reg = Self::new();
        for f in core_builtins() {
            let key = canonical_fn_name(&f.name);
            assert!(
                reg.core.insert(key.clone(), f).is_none(),
                "duplicate stdlib builtin after case-folding: {key}"
            );
        }
        reg
    }
    ...
    pub fn builtin(&self, name: &str) -> Option<&BuiltinFn> {
        self.core.get(&canonical_fn_name(name))
    }

    /// Whether `name` is a registered core built-in.
    #[must_use]
    pub fn is_builtin(&self, name: &str) -> bool {
        self.core.contains_key(&canonical_fn_name(name))
    }
    ...
    pub fn is_aggregate(&self, name: &str) -> bool {
        self.core
            .get(&canonical_fn_name(name))
            .is_some_and(BuiltinFn::is_aggregate)
    }
    ...
    /// [`DriverId`] (blueprint §3 receiver scoping).
    ///
    /// # Errors
