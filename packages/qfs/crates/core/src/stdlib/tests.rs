...
fn registry_with_core_registers_the_stdlib_and_resolves_by_name() {
    ...
    assert_eq!(
        reg.builtin("UPPER").map(|b| b.name.as_str()),
        Some("upper"),
        "stdlib canonical names are lowercase"
    );
...
#[test]
fn replace_split_concat_and_like_is_not_a_function() {
    let reg = StdlibRegistry::with_core();
    assert_eq!(
        scalar(
            &reg,
    ...
    assert!(!reg.is_builtin("LIKE"));
    assert!(matches!(
        classify_fn(&reg, "LIKE"),
        Err(FnError::UnknownFunction { .. })
    ));
}
