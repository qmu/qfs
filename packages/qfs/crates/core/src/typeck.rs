//! The **static primitive type checker**, run at **plan time** (blueprint §3 purity
...
//! — text compared to an `int` column, a lambda annotated `(x: text)` applied to a
...
/// `qfs-types` (`bool`/`int`/`float`/`text`/…, plus the late-bound `Unknown`/`Json`), a
...
pub enum Ty {
    /// A primitive / column type (`bool`, `int`, `float`, `text`, `date`, …, or the
...
impl Ty {
    ...
    /// undescribable source, a `Null` literal, or an unannotated lambda parameter.
    #[must_use]
    pub fn unknown() -> Self {
...
pub fn check_expr(
    ...
    match expr {
        ...
        Expr::Lambda { params, body } => {
            ...
            // type (`(x: int) => x ~ 'p'`) fails right here.
...
/// comparability via the [`require_comparable`] matrix and yields a boolean. Arithmetic is
/// deliberately monomorphic: same-type numeric operands only, with explicit casts required
/// before mixed numeric work.
fn check_binary(
    op: Op,
    lhs: &Expr,
    rhs: &Expr,
    ...
    match op {
        ...
        Op::Add | Op::Sub | Op::Mul | Op::Div => arithmetic_type(op, &lt, &rt),
    }
}

fn arithmetic_type(op: Op, lhs: &Ty, rhs: &Ty) -> Result<Ty, EvalError> {
    let (Some(l), Some(r)) = (lhs.as_prim(), rhs.as_prim()) else {
    ...
    if matches!(l, ColumnType::Unknown | ColumnType::Json)
        || matches!(r, ColumnType::Unknown | ColumnType::Json)
    {
        return Ok(Ty::unknown());
    }
    match (op, l, r) {
        (Op::Add | Op::Sub | Op::Mul, ColumnType::Int, ColumnType::Int) => {
            Ok(Ty::Prim(ColumnType::Int))
        }
        (Op::Add | Op::Sub | Op::Mul | Op::Div, ColumnType::Float, ColumnType::Float) => {
            Ok(Ty::Prim(ColumnType::Float))
        }
...
fn check_fn(
    ...
    // An aggregate in scalar/predicate position is a typed misuse (blueprint §3 dispatch).
...
/// (`Unknown`) — a bare null carries no type (blueprint §4); a `Size` is an integer magnitude; a
...
const ACCEPTED_TYPE_ANNOTATIONS: &[&str] = &[
    "bool",
    "int",
    "float",
    "decimal",
...
/// The type of a lambda parameter type annotation (t61's retained [`TypeAnn`]).
/// An annotation is the canonical §5 `ColumnType` grammar (`text`/`int`/`array<text>`/…)
/// plus the CamelCase non-column value `Resource`. `unknown` is intentionally rejected in
/// annotation position: omit the annotation to say "late-bound". Retired spellings such as
/// `string`, `i64`, lowercase `resource`, and `Row` are structured plan-time errors.
fn param_type(annotation: Option<&str>) -> Result<Ty, EvalError> {
    let Some(name) = annotation else {
        return Ok(Ty::unknown());
    };
    if name == "Resource" {
        return Ok(Ty::Resource);
    }
    match ColumnType::parse(name) {
        Some(ColumnType::Unknown) | None => Err(EvalError::UnknownTypeAnnotation {
            name: name.to_string(),
            accepted: ACCEPTED_TYPE_ANNOTATIONS.to_vec(),
        }),
...
fn cmp_op(op: Op) -> CmpOp {
    match op {
        ...
        Op::Add | Op::Sub | Op::Mul | Op::Div => CmpOp::Eq,
...
/// (decision T's headline rejection: `where total == 'paid'` against an `int` column). A
...
/// (decision T), or raise [`FnError::Type`] (`UPPER(<int column>)`). A function / `Resource`
