//! `qfs-driver-cf` — the **Cloudflare driver** (blueprint §6, E4 t23): one driver crate mounted at
//! `/cf` exposing Cloudflare primitives through the uniform qfs DSL, each on its correct
//! archetype:
//!
...
/// The Cloudflare API token scopes this driver needs (blueprint §8 least-privilege). Documented
...
/// The Queue least-privilege scope label.
pub const CF_QUEUE_SCOPE: &str = "queues:send queues:consume";
/// The Artifacts least-privilege scope label.
pub const CF_ARTIFACTS_SCOPE: &str = "artifacts:read artifacts:write";

/// The Cloudflare driver (blueprint §6). Owns the synchronous [`CfApplier`] the contract returns from
...
impl CfDriver {
    ...
    pub fn execute_d1_query(
        ...
        let api_db = handle.api_database_id(&db).to_string();
        ...
        let sql = alias_d1_projection(&sql, &result.plan, table_cat);
        let rows = backend.d1_query(&api_db, &sql, &params)?;
    ...
    pub fn kv_list_keys(
        ...
        handle
            .backend()
            .clone()
            .kv_list(handle.api_namespace_id(ns), prefix, limit)
    }
    ...
    ///
    /// # Errors
    /// [`CfError`] when Artifacts is not registered or the backend fails.
    pub fn artifact_repos(&self) -> Result<Vec<ArtifactRepo>, CfError> {
        let handle = self.registry().artifacts()?;
        let backend = handle.backend().clone();
        let mut rows = Vec::new();
        for namespace in backend.list_artifact_namespaces()? {
            rows.extend(backend.list_artifact_repos(&namespace)?);
        }
        Ok(rows)
    }

    /// Read one Artifacts repo by `(namespace, name)`.
    ///
    /// # Errors
    /// [`CfError`] when Artifacts is not registered or the backend fails.
    pub fn artifact_repo(
        &self,
        namespace: &str,
        name: &str,
    ) -> Result<Option<ArtifactRepo>, CfError> {
        let handle = self.registry().artifacts()?;
        handle.backend().clone().get_artifact_repo(namespace, name)
    }
    ...
    fn caps_for(&self, path: &Path) -> Capabilities {
        match CfNode::parse(path) {
            ...
            Ok(CfNode::ArtifactRepo { .. }) if self.registry().has_artifacts() => {
                Capabilities::from_verbs(&[Verb::Select, Verb::Remove])
            }
            _ => Capabilities::none(),
        }
    }
...
fn alias_d1_projection(sql: &str, plan: &SelectPlan, table: &qfs_sql_core::TableCatalog) -> String {
    let columns: Vec<&str> = if plan.projection.is_empty() {
        table.columns.iter().map(|col| col.name.as_str()).collect()
    } else {
        plan.projection.iter().map(String::as_str).collect()
    };
    if columns.is_empty() {
        return sql.to_string();
    }
    let Some(from_idx) = sql.find(" FROM ") else {
        return sql.to_string();
    };
    let projection = columns
        .iter()
        .enumerate()
        .map(|(idx, col)| format!("{} AS c{idx}", Dialect::Sqlite.quote_ident(col)))
        .collect::<Vec<_>>()
        .join(", ");
    format!("SELECT {projection}{}", &sql[from_idx..])
}
