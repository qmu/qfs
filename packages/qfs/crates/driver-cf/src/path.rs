...
//! it names (blueprint §4/§6: "the path is the type"). One `/cf` mount fans out to three
...
//! - **Artifacts** (`/cf/artifacts`, `/cf/artifacts/<namespace>/<repo>`) — account-scoped Git
//!   repositories. `SELECT` lists repos, `UPSERT` creates one and seals the returned token, and
//!   `REMOVE` deletes one repo.
//!
...
/// The `/cf/artifacts` service segment.
pub const ARTIFACTS_SEGMENT: &str = "artifacts";

/// A parsed Cloudflare address — what a `/cf/...` path resolves to (blueprint §4 "the path is the
...
pub enum CfNode {
    ...
    /// `/cf/artifacts` — the account-scoped Artifacts repository list.
    Artifacts,
    /// `/cf/artifacts/<namespace>/<name>` — one Artifacts repository.
    ArtifactRepo {
...
impl CfNode {
    ...
    pub fn parse_str(raw: &str) -> Result<Self, CfError> {
        ...
        match segments.as_slice() {
                    ...
                    || *service == QUEUE_SEGMENT =>
            {
                ...
                Err(CfError::InvalidPath {
                    ...
                    reason: "a /cf service needs a target (e.g. /cf/d1/<db>/<table>, \
                             /cf/kv/<ns>, /cf/queue/<name>, /cf/artifacts)",
                })
            }
            [service] if *service == ARTIFACTS_SEGMENT => Ok(CfNode::Artifacts),
            ...
            [svc, namespace, name] if *svc == ARTIFACTS_SEGMENT => Ok(CfNode::ArtifactRepo {
            ...
            [svc, ..] if *svc == ARTIFACTS_SEGMENT => Err(CfError::InvalidPath {
                path: raw.to_string(),
                reason: "a /cf/artifacts path is /cf/artifacts[/<namespace>/<repo>]",
            }),
            _ => Err(CfError::InvalidPath {
                path: raw.to_string(),
                reason: "unknown /cf service (expected d1, kv, queue, or artifacts)",
            }),
        }
    }
    ...
    pub fn account_selector(&self) -> Option<&str> {
        match self {
            ...
            CfNode::Root | CfNode::Artifacts | CfNode::ArtifactRepo { .. } => None,
        }
    }
}
