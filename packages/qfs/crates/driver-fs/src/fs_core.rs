...
//! and remove. All `std::fs` types stay **internal** to this module (blueprint §11) — only [`FsRow`],
...
//!
//! ## Escape guard (blueprint §8 least privilege) — validated at BOTH scan and apply time
...
//!
//! ## Streaming (blueprint §7, the hard part)
...
/// The operator-configured **root allowlist** (blueprint §8): a map from a root NAME (the first path
...
/// the source (the copy→verify shape, blueprint §7). Returns the byte count copied. Does **not** delete
...
/// is **irreversible** (blueprint §8) — the engine flags the `Remove` effect so `PREVIEW` warns and
