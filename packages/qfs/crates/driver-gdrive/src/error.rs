//! The structured Drive driver error (blueprint §6: machine-readable for an AI, never prose,
...
//! [`AuthError`](qfs_google_auth::AuthError) is mapped in at the boundary so no auth-internal
...
pub enum DriveError {
    ...
    /// A create-only `INSERT` named a path that already holds a Drive node. Structured: the path
    /// and the existing node's id, so an agent can address or replace it deliberately. Fail-closed:
    /// an `INSERT` never overwrites — this is the guard against silently replacing an operator's
    /// file on an inferred copy (use `UPSERT` for an explicit content replace).
    #[error(
        "cannot INSERT at {path:?}: a Drive file already exists there (id {id:?}); INSERT never \
         replaces — use UPSERT to replace its content deliberately"
    )]
    TargetExists {
        /// The path the create was attempted at.
        path: String,
        /// The existing node's Drive id (a low-sensitivity handle, never a token).
        id: String,
    },

    /// A path name resolved to more than one Drive node (Drive names are not unique). Structured:
    /// the path. Fail-closed: the driver refuses rather than guess which same-named file was meant.
    #[error(
        "path {path:?} is ambiguous: more than one Drive file shares this name — address the one \
         you mean by its id (/drive/id:<file-id>)"
    )]
    AmbiguousTarget {
        /// The ambiguous path.
        path: String,
    ...
    /// A multi-row write failed part-way: `applied` of `total` per-row operations had already
    /// committed when row `row` failed. Surfaced instead of a full-success count so the operator
    /// sees exactly how far the batch got (honest counts; ticket 20260712005000). Not retryable
    /// as a whole — the applied prefix would double-apply.
    #[error(
        "multi-row write stopped at row {row} of {total}: {applied} file(s) were already written \
         before it failed: {reason}"
    )]
    PartialApply {
        /// Per-row operations that HAD applied before the failure.
        applied: u64,
        /// Total rows in the batch.
        total: usize,
        /// The zero-based row whose operation failed.
        row: usize,
        /// The failing row's secret-free error text.
        reason: String,
    },
}
...
impl From<DriveError> for EffectError {
    ...
    /// interpreter's retry/ledger logic can branch on its class (blueprint §6/§7). A capability
    ...
    fn from(err: DriveError) -> Self {
        match err {
            DriveError::CapabilityDenied { path, verb } => EffectError::CapabilityDenied {
                driver: qfs_types::DriverId::new("drive"),
                verb: verb.to_string(),
                path,
            },
