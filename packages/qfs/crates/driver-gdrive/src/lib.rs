//! `qfs-driver-gdrive` — the **Google Drive blob/namespace `Driver`** (blueprint §6, E4 t21) and,
...
pub use effect::{
    ...
    PARENT_ID_COL, PARENT_PATH_COL, REMOVE_PARENTS_COL,
};
...
/// opens. NOT a full-account or admin scope (blueprint §8 blast radius). Declared on the `drive.copy`
...
/// The Google Drive driver (blueprint §6). Owns the synchronous [`DriveApplier`] the contract returns
...
impl GDriveDriver {
    ...
    pub fn new(client: Arc<dyn GDriveClient>) -> Self {
        Self {
            ...
            // projection / joins stay local (blueprint §7). Residual predicates combine locally — see
            // `query::build_query`.
            ...
            procs: vec![
                ...
                ProcSig::new("copy")
                    .with_params(vec![
                        ...
                        Param::new("parent_path", ColumnType::Text),
    ...
    fn caps_for(&self, path: &Path) -> Capabilities {
        match DrivePath::parse(path) {
            // The My Drive corpus root is a WRITABLE folder (Drive's reserved `root` alias):
            // gdrive-ftp puts, mkdirs, and rms at the top level, so INSERT (create a child),
            // UPSERT (create/replace a child blob), and REMOVE (trash-by-name — the bare
            // no-filter form fails closed in the effect decode, so the root itself can never
            // be trashed) work here too. The root cannot be renamed (no Update/Cp/Mv).
            Ok(DrivePath::MyRoot) => Capabilities::from_verbs(&[
                Verb::Ls,
                Verb::Select,
                Verb::Insert,
                Verb::Upsert,
...
impl Driver for GDriveDriver {
    ...
    fn describe(&self, _path: &Path) -> Result<NodeDesc, qfs_driver::CfsError> {
        // Every /drive node is the Blob/namespace archetype; its relation is the UNIFIED content
        // schema — the canonical FileMeta listing columns PLUS a nullable `content` (Bytes) that a
    ...
    fn version_support(&self, _path: &Path) -> VersionSupport {
        // Drive files carry revisions addressable by `@<rev>` (blueprint §4).
