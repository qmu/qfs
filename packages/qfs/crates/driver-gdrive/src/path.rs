...
//! concrete Drive node it names (blueprint §6). Drive maps onto the **Blob/namespace
...
impl DrivePath {
    ...
    pub fn parse_str(raw: &str) -> Result<Self, DriveError> {
        // `id:` addressing — a file/folder by id, independent of any corpus. Accept both the
        // mount-relative `id:<id>` form and the operator-facing `/drive/id:<id>` form the gdrive
        // cookbook documents (round-5 defect: the mounted spelling returned `invalid_path`).
        if let Some(rest) = raw.strip_prefix("id:") {
            return id_selector(rest, raw);
        }
        ...
        // The mounted id-selector: `/drive/id:<file-id>` (the cookbook's escape hatch for a name
        // that is ambiguous or unaddressable — e.g. a space-bearing name a model chose).
        if let Some(rest) = after.strip_prefix("id:") {
            return id_selector(rest, raw);
        }
    ...
    /// The CHILD of this address named `name` — the write path's "this folder, that entry"
    /// composition (`/drive/my` + `q3.pdf` → `/drive/my/q3.pdf`). `None` for addresses that
    /// cannot contain named children structurally (`id:` — the id is opaque; the roots that
    /// are not folders).
    #[must_use]
    pub fn child(&self, name: &str) -> Option<DrivePath> {
        match self {
            DrivePath::MyRoot => Some(DrivePath::My {
            ...
            DrivePath::Shared {
                ...
                let mut segments = segments.clone();
                segments.push(name.to_string());
                Some(DrivePath::Shared {
                    drive: drive.clone(),
                    segments,
                    revision: None,
                })
            }
            DrivePath::Root | DrivePath::SharedRoot | DrivePath::ById { .. } => None,
        }
    }

    /// The CONTAINING folder of this address — the path with the terminal segment peeled off
    /// (`/drive/my/a/b.txt` → `/drive/my/a`; a single-segment path → its corpus root). `None`
    /// for the roots and an `id:` address, which name no parent structurally. Used by the
    /// write path to resolve an upload destination from a blob-shaped path.
    #[must_use]
    pub fn parent(&self) -> Option<DrivePath> {
        match self {
            DrivePath::My { segments, .. } => Some(if segments.len() <= 1 {
                DrivePath::MyRoot
            } else {
                DrivePath::My {
...
/// Build a [`DrivePath::ById`] from the text after an `id:` selector (shared by the bare
/// `id:<id>` and mounted `/drive/id:<id>` spellings). A `@rev` tail pins a revision. An empty id is
/// a structured error carrying the ORIGINAL `raw` for context.
fn id_selector(rest: &str, raw: &str) -> Result<DrivePath, DriveError> {
...
/// The `@` is the t21/blueprint §4 revision-pin sigil; a name legitimately containing `@` (rare for
