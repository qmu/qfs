//! WHERE → Drive `q` search pushdown (blueprint §7 "push down what the backend runs natively;
...
/// (blueprint §7). `query` is empty when nothing pushed down; `residual` is `None` when the whole
...
/// as residual so the engine re-applies exact filtering locally (blueprint §7 over-fetch-then-filter).
#[must_use]
...
fn lower(p: &Predicate, terms: &mut Vec<String>) -> Option<Predicate> {
    match p {
        ...
        Predicate::Cmp(col, op, lit) => match lower_cmp(col, *op, lit) {
            ...
            // so the engine re-checks it — over-fetch then filter, never wrong rows (blueprint §7).
