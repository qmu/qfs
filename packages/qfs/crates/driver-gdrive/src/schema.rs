//! Owned file/folder/revision DTOs and their canonical typed [`Schema`] (blueprint §6/§11).
//!
...
//! invariant, blueprint §11). The file [`Schema`] is the canonical `qfs_types::Schema` so
...
/// One Drive file/folder projected into the owned, vendor-free DTO (blueprint §11). `modified_time` is
...
impl FileMeta {
    ...
    /// The **content** schema: the canonical [`FileMeta::schema`] listing columns plus a nullable
    /// `content` ([`ColumnType::Bytes`]) column. This is the UNIFIED shape `describe()` advertises
    /// and BOTH read paths conform to, so plan-time and runtime agree: a single-file `/drive/<file>`
    /// read materialises the downloaded/exported bytes under `content` (the rest of the row is the
    /// file's own listing metadata), while a folder listing carries the same schema with a null
    /// `content`. Advertising `content` here is what lets `/drive/<file> |> select content |>
    /// transform …` type-check at plan time — mirroring the `/local` v0.0.60 fix, but the gdrive
    /// file-vs-folder read schemas are unified onto this superset first (they previously differed).
    #[must_use]
