//! [`GitEffect`] — the effects-as-data (blueprint §7) the git driver realises a write plan as.
...
//!   concurrency via `@version`, blueprint §7): a stale `old` is rejected, never clobbered. A `force`
...
impl GitEffect {
    ...
    /// the git driver does is inherently irreversible (the deliberate safety win, blueprint §7).
    #[must_use]
