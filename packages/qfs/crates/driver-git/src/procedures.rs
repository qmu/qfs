//! The `CALL git.*` procedure declarations (blueprint §3): `merge`, `rebase`, `checkout`, `tag` — the
...
//! reflog-recoverable (the deliberate git safety win, blueprint §7). `requires_scopes` carries the local
