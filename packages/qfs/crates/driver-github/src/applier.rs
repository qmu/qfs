//! [`GitHubApplier`] — the GitHub driver's synchronous apply leg (blueprint §7). It is the lone
...
impl SharedApplier for GitHubApplier {
    fn apply_shared(&self, node: &EffectNode) -> Result<EffectOutput, EffectError> {
        ...
        // never auto-retried (blueprint §7 — do not re-attempt an irreversible merge).
