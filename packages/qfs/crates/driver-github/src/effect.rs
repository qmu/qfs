//! [`GitHubEffect`] — the owned effect the driver realises a plan leaf as (blueprint §7), and the
...
//! No vendor type appears here. `merge`/`dispatch`/`Delete*` carry irreversibility per blueprint §8/§7.
...
pub enum GitHubEffect {
    ...
    /// at-least-once and never silently retries (blueprint §7).
    PostComment {
        /// `owner/repo` slug.
        slug: String,
...
impl GitHubEffect {
    ...
    /// Whether this effect is irreversible (blueprint §8/§7): the deletes, plus `merge`/`dispatch`.
    #[must_use]
    ...
    /// (at-least-once, blueprint §7): a comment post, an open-issue/PR, a create, a review, a merge, a
