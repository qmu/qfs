//! `qfs-driver-github` — the **GitHub object-graph + workflow `Driver`** (blueprint §6, E4 t24).
...
//!
//! ## Universal CRUD + the irreducible `CALL` procedures (blueprint §3)
...
/// The GitHub driver (blueprint §6). Owns the synchronous [`GitHubApplier`] the contract returns from
...
impl GitHubDriver {
    ...
    pub fn new(client: Arc<dyn GitHubClient>) -> Self {
        Self {
            ...
            // residual keeps exact correctness (blueprint §7).
    ...
    /// The node-keyed capability set (blueprint §6), gating verbs at parse time. The effective namespace
