//! WHERE → GitHub REST query-param pushdown (blueprint §7 "push down what the backend runs
...
/// locally (blueprint §7). `params` is empty when nothing pushed down; `residual` is `None` when the
...
/// (blueprint §7 over-fetch-then-filter).
#[must_use]
