//! [`RestApplier`] — the REST driver's synchronous apply leg (blueprint §7). It is the lone
...
impl RestApplier {
    ...
    /// Execute a READ effect (a GET, following pagination) and return the decoded rows. Unlike
    /// `apply_shared`/`apply` — which report only the affected COUNT to the runtime — this surfaces
    /// the `RowBatch` a read facet needs (the §13 declared-driver read path). Host confinement and
    /// auth injection apply identically (every request funnels through `send_one`).
    ///
    /// # Errors
    /// The structured [`HttpError`] on a transport / auth / decode / confinement failure.
    pub fn read(&self, node: &EffectNode) -> Result<RowBatch, HttpError> {
        let effect =
            HttpEffect::from_node(node).map_err(|e| HttpError::Invalid { reason: e.reason })?;
        let (rows, _n) = self.apply_effect(&effect)?;
        Ok(rows)
    }
    ...
    /// The §13 declared-view `FOLLOW` GET (ticket 20260711121526): one request to the delivered
    /// absolute `url`, returning the RAW response bytes (no codec decode — the follow target is
    /// a payload, e.g. a file download, not rows). Two deliberate differences from every other
    /// request through this applier:
    ///   * **No auth injection.** The follow URL is self-authorizing (the service minted it);
    ///     sending the driver's credential to the URL's (foreign) host would leak it.
    ///   * **The URL's own host is data-admitted** — the operator-installed declaration carried
    ///     the `FOLLOW` stage, so the one host the delivered field names joins the allowed set
    ///     for exactly this request (the `confine` chokepoint still refuses everything else,
    ///     including any redirect the transport's confined policy would chase).
    ///
    /// Default headers (e.g. the versioned User-Agent) still apply; pagination does not.
    ///
    /// # Errors
    /// The structured [`HttpError`] on a transport failure or a non-success status.
    pub fn follow_bytes(&self, url: &str) -> Result<Vec<u8>, HttpError> {
        let mut req = HttpRequest::new(HttpMethod::Get, url.to_string());
        for (name, value) in &self.config.default_headers {
            req = req.header(name.clone(), value.clone());
        }
        let resp = self.client.send(&req)?;
        tracing::debug!(method = "GET", url = %url, status = resp.status, "rest follow");
        if resp.is_success() {
            Ok(resp.body)
        } else {
            Err(HttpError::from_status(resp.status, "GET", url))
        }
    }
    ...
    /// §13 host confinement: reject a request URL whose host is outside the driver's declared
    /// `allowed_hosts` set. An EMPTY set is unconfined (compiled drivers) and passes everything. A
    /// URL whose host cannot be parsed is treated as foreign (fail closed).
    fn confine(&self, url: &str) -> Result<(), HttpError> {
        ...
        if self.config.allowed_hosts.iter().any(|h| h == &host) {
            ...
            Err(HttpError::Confinement { host })
        }
    }
    ...
    fn inject_auth(&self, req: HttpRequest) -> Result<HttpRequest, HttpError> {
        let (header_name, value_prefix, secret_ref) = match &self.config.auth {
            ...
            // An account-referenced bearer is wire-identical to `Bearer`; the difference is only the
            // coordinate (`secret_ref` names the shared provider account). The injected `Secrets`
            // handle resolves the live token — running an OAuth refresh where the provider needs one.
            AuthStrategy::Account { secret_ref, .. } => ("Authorization", "Bearer ", secret_ref),
        };
        let key = secret_ref
            .credential_key()
    ...
    fn send_one(&self, req: &HttpRequest) -> Result<HttpResponse, HttpError> {
        // §13 host confinement: the single chokepoint EVERY request funnels through — the first
        // request, every cursor/link-header follow-up, and every write — so a confined (declared)
        // driver is structurally unable to reach a host outside its `allowed_hosts` set, including
        // after a pagination follow or an `override_url`. Unconfined (empty set) is a no-op.
        self.confine(&req.url)?;
        let resp = self.client.send(req)?;
        // Structured observability (blueprint §7): URL + status, redacted headers. `req` Debug is
    ...
    /// `max_pages` cap (blueprint §7 — bound runaway fetches). The follow loop lives here, at the
...
/// The host (authority without userinfo/port) of an absolute request URL — for the §13
/// confinement check. Best-effort string parse (no url dep on this sync path): strips the scheme,
/// takes up to the first `/`/`?`/`#`, then drops any `user@` and `:port`. `None` for an empty host.
...
/// Read a string cursor out of a JSON response body at `field` — a **dotted path** walked
/// segment by segment (`response_metadata.next_cursor` descends nested objects; a plain name
/// is the 1-segment case). Returns `None` if the body is not JSON, any path segment is
/// absent, or the value is null/non-stringy (blueprint §13 tier 2: real services nest their
/// cursors, so the descriptor speaks a path, not only a top-level field).
