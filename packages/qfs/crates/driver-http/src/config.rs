...
impl RestApiConfig {
    ...
    /// `/slack/{ws}/users` — is the declared-node table a DECLARED mount carries
    /// (`qfs_driver_http::DeclaredNodeDesc`), which its capability and irreversibility answers both
    /// read instead of ever reaching here.
    #[must_use]
