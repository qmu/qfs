//! [`HttpEffect`] — the owned effect DTO the REST driver realises a plan leaf as (blueprint
...
//! no HTTP-verb keywords in the DSL (blueprint §3 "the path is the type"). [`HttpEffect::from_node`]
...
/// The well-known column carrying a **pre-encoded request body** — set by the `http.get` TVF and by
/// the §13 declared-map write path (which encodes its evaluated `VALUES (<expr>)` wire body here so
/// the applier POSTs it verbatim rather than re-encoding the row args).
pub const BODY_COL: &str = "__http_body";

/// One fully-decoded REST effect — what the apply leg executes against the World. Owned DTOs;
/// no `reqwest`/`url` type appears here. `Remove` is inherently irreversible (blueprint §8).
...
impl HttpEffect {
    ...
    pub const fn verb_label(&self) -> &'static str {
        match self.method {
            ...
            // `qfs_http_core::HttpMethod` is a foreign `#[non_exhaustive]` enum: a wildcard is
