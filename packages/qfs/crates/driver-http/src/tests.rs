...
fn describe_reports_relational_table_with_open_json_schema() {
    ...
    // JSON is dynamic — an open `value` json column, not invented typed columns (blueprint §4).
...
#[test]
fn account_auth_injects_the_provider_bearer_at_wire_time() {
    // `AUTH ACCOUNT '<provider>'` is wire-identical to Bearer: the account provider's live token is
    // injected as `Authorization: Bearer <token>`. The coordinate names the shared provider account
    // (here `(github, work)`), resolved through the injected Secrets handle — an account-backed
    // adapter in the binary; a planted secret here. The declaration itself carries no token.
    let config = things_config().with_auth(AuthStrategy::Account {
    ...
    d.rest_applier().apply_shared(&node).unwrap();
    assert_eq!(
        mock.recorded()[0].header_value("Authorization"),
        Some(format!("Bearer {PLANTED_TOKEN}").as_str()),
        "the account provider's live bearer is injected at wire time"
    );
}
...
fn server_5xx_on_a_post_is_terminal_never_retried() {
    // POST is not idempotent — a 5xx (or timeout) must NOT be auto-retried (blueprint §7).
    let mock =
        Arc::new(MockHttpClient::new().with_response(HttpResponse::new(503, b"oops".to_vec())));
    let d = driver_from_mock(things_config(), Arc::clone(&mock), empty_secrets());
    let node = EffectNode::new(
...
#[test]
fn cursor_pagination_walks_a_dotted_nested_path() {
    // Blueprint §13 tier 2: real services nest their cursors (Slack's is
    // `response_metadata.next_cursor`), so `next` is a dotted path, not only a top-level field.
    let config = things_config().with_pagination(Pagination::Cursor {
        next_field: "response_metadata.next_cursor".to_string(),
        param: "cursor".to_string(),
        max_pages: 5,
    });
    let mock = Arc::new(MockHttpClient::new());
    mock.push_response(HttpResponse::new(
        200,
        br#"{"p":1,"response_metadata":{"next_cursor":"c2"}}"#.to_vec(),
    ));
    mock.push_response(HttpResponse::new(
        200,
        br#"{"p":2,"response_metadata":{"next_cursor":""}}"#.to_vec(),
    ));
    ...
    let out = d.rest_applier().apply_shared(&node).unwrap();
    assert_eq!(out.affected, 2, "both pages concatenated");

    let reqs = mock.recorded();
    assert_eq!(reqs.len(), 2, "stopped when the nested cursor was empty");
    assert!(!reqs[0].url.contains("cursor="));
    assert!(
        reqs[1].url.contains("cursor=c2"),
        "the nested cursor drove page 2: {}",
        reqs[1].url
    );
}

#[test]
fn redirect_confinement_follows_own_host_and_refuses_foreign() {
    // The pure policy decision behind `ReqwestClient::with_confined_hosts` (blueprint §13
    // tier 2): a 30x hop may only stay on a confined host; a foreign or hostless target is
    // refused — fail closed.
    let allowed = vec!["api.example.com".to_string()];
    assert!(crate::redirect_allowed(&allowed, Some("api.example.com")));
    assert!(
        crate::redirect_allowed(&allowed, Some("API.EXAMPLE.COM")),
        "host comparison is case-insensitive"
    );
    assert!(!crate::redirect_allowed(&allowed, Some("evil.example.net")));
    assert!(
        !crate::redirect_allowed(&allowed, None),
        "hostless target fails closed"
    );
    assert!(!crate::redirect_allowed(&[], Some("api.example.com")));
}
...
#[test]
fn confined_driver_rejects_a_foreign_link_header_follow() {
    // A confined driver's first page is its own host (allowed); a `Link: rel="next"` pointing at a
    // FOREIGN host is rejected BEFORE dispatch — the anti-exfiltration guarantee holds post-pagination.
    let config = things_config()
        .with_pagination(Pagination::LinkHeader { max_pages: 5 })
        .with_allowed_host("api.example.com");
    let mock = Arc::new(MockHttpClient::new());
    mock.push_response(
        HttpResponse::new(200, br#"[{"p":1}]"#.to_vec())
            .header("Link", "<https://evil.example.org/steal>; rel=\"next\""),
    );
    // A second response is queued but must NEVER be reached (the follow-up is confined away).
    mock.push_response(HttpResponse::new(200, br#"[{"p":2}]"#.to_vec()));
    let d = driver_from_mock(config, Arc::clone(&mock), empty_secrets());
    let node = EffectNode::new(
        NodeId(0),
        EffectKind::Read,
    ...
    assert!(
        ...
        "confinement error: {err}"
    );
    // The foreign follow-up was rejected before the network: the mock only ever saw page 1 (its
    // own host).
    let reqs = mock.recorded();
    assert_eq!(reqs.len(), 1);
    assert!(reqs[0].url.starts_with("https://api.example.com/"));
}

#[test]
fn confined_driver_rejects_a_foreign_override_url() {
    // The `override_url` (http.get) escape is confined too: a confined driver cannot be pointed at a
    // foreign host, and the request never leaves.
    let config = things_config().with_allowed_host("api.example.com");
    let mock = Arc::new(MockHttpClient::new().with_response(json_two_things()));
    let d = driver_from_mock(config, Arc::clone(&mock), empty_secrets());
    let node = http_get_node(NodeId(0), "https://other.example.org/probe", &[]);
    let err = d.rest_applier().apply_shared(&node).unwrap_err();
    assert!(
        err.to_string().contains("confinement"),
        "confinement error: {err}"
    ...
    assert!(
        ...
        "the confined request never dispatched"
    );
}

#[test]
fn an_unconfined_driver_still_reaches_any_host() {
    // Backward compatibility: an EMPTY allowed_hosts set (the default — compiled `/rest` and the
    // http.get TVF) is unconfined, so a foreign override URL still works exactly as before.
    let mock = Arc::new(MockHttpClient::new().with_response(json_two_things()));
    let d = driver_from_mock(things_config(), Arc::clone(&mock), empty_secrets());
    let node = http_get_node(NodeId(0), "https://other.example.org/probe", &[]);
    let out = d.rest_applier().apply_shared(&node).unwrap();
    assert_eq!(out.affected, 2);
    assert_eq!(mock.recorded()[0].url, "https://other.example.org/probe");
}
...
#[test]
fn write_irreversible_reflects_the_resource_config() {
    // A `posts` resource whose INSERT is declared irreversible; a `notes` resource whose INSERT is not.
    let config = RestApiConfig::new(
        "https://api.example.com/v1",
        vec![
            ResourceMap::new("posts", vec![RestVerb::Insert])
                .with_irreversible_verbs(vec![RestVerb::Insert]),
            ResourceMap::new("notes", vec![RestVerb::Insert]),
        ],
    );
    let d = driver_with(config, MockHttpClient::new(), empty_secrets());
    // The irreversible-marked resource+verb gates; the same verb on a reversible resource does not.
    assert!(d.write_irreversible(&Path::new("/rest/api/posts"), Verb::Insert));
    assert!(!d.write_irreversible(&Path::new("/rest/api/notes"), Verb::Insert));
    // A non-write verb and an unknown resource are never irreversible here.
    assert!(!d.write_irreversible(&Path::new("/rest/api/posts"), Verb::Select));
    assert!(!d.write_irreversible(&Path::new("/rest/api/unknown"), Verb::Insert));
}
