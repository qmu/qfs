...
#[test]
fn send_inside_an_ambient_tokio_runtime_does_not_panic() {
    // The commit boundary drives the read engine on a tokio runtime, and a live-provider
    // transform (the §18 switch live round, or any live `|> transform` commit) issues its HTTP
    // call SYNCHRONOUSLY from within that walk. `Runtime::block_on` panics in that ambient
    // context ("cannot start a runtime from within a runtime") — the client must detect the
    // ambient runtime and drive its owned runtime on a worker thread instead. This is the
    // regression lock for the live-round panic at client.rs `rt.block_on`.
    use qfs_driver_http::{HttpMethod, HttpRequest};
    let rt = tokio::runtime::Runtime::new().unwrap();
    rt.block_on(async {
        let (base, _server) = spawn_loopback().await;
        let client = ReqwestClient::new(5);
        let req = HttpRequest::new(HttpMethod::Get, format!("{base}/things"));
        // Inside `block_on`: before the fix this PANICKED; now it completes on a worker thread.
        let resp = client.send(&req).expect("the loopback GET completes");
