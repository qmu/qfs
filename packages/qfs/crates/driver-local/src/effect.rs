...
//! mapping from a runtime [`EffectNode`] onto them (blueprint §7). The driver's apply leg
...
//! driving the native copy→verify→[delete] recovery shape (blueprint §7).
//!
...
/// `std::fs` type appears here. `Move`/`Remove` are inherently irreversible (blueprint §8).
