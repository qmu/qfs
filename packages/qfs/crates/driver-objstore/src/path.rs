//! [`ObjNode`] — the parse of a qfs [`Path`](qfs_driver::Path) into the object-storage node it
//! names (blueprint §4/§6: "the path is the type"). Two mounts, one parser:
//!
...
//! - The optional `@<versionId>` temporal coordinate (blueprint §4) addresses a specific object version
...
/// A parsed object-storage address — what an `/s3/...` or `/r2/...` path resolves to (blueprint §4).
...
pub enum ObjNode {
    ...
    Object {
        ...
        key: String,
        /// The optional `@versionId` temporal coordinate (blueprint §4).
...
/// `@`. The `@` is the temporal-coordinate delimiter (blueprint §4); only the **last** `@` separates a
