//! Owned Slack DTOs and their canonical typed [`Schema`]s (blueprint §6/§11).
//!
...
pub struct FileDto {
    ...
    /// The size in bytes.
    pub size: i64,
    /// The Slack-created timestamp as epoch milliseconds.
    pub created: i64,
...
impl FileDto {
    ...
    pub fn schema() -> Schema {
        Schema::new(vec![
            ...
            Column::new("created", ColumnType::Timestamp, true),
            Column::new("user", ColumnType::Text, true),
        ])
    }

    /// The single-file download schema. `content` mirrors other blob drivers and carries raw
    /// bytes, so a Slack file can feed a Drive/S3/local blob write without text transcoding.
    #[must_use]
    ...
    /// Build the one-row downloaded-file batch.
    #[must_use]
    pub fn content_batch(&self, content: Vec<u8>) -> RowBatch {
        RowBatch::new(
            Self::content_schema(),
            vec![Row::new(vec![
                ...
                if self.mimetype.is_empty() {
                    ...
                    Value::Text(self.mimetype.clone())
                },
