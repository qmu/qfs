//! The **inbound event normalizer** (blueprint §10) — the genuinely-hard, genuinely-**wasm-required**
...
//! not touch `qfs-runtime`/tokio — so it (and the introspective `Driver` surface) compiles for
//! `wasm32-unknown-unknown` (the Workers `WEBHOOK` ingress, blueprint §10). It is unit-testable from
//! fixtures (no live Slack).
//!
//! ## Signature verification (blueprint §8 replay defense)
...
//!
//! ## Idempotency / dedupe (blueprint §7)
...
/// The normalized kind of an inbound event — the closed set this driver recognizes (blueprint §11 enums).
...
/// One normalized inbound Slack event — the owned DTO the server's trigger bus consumes (blueprint §10).
...
pub struct SlackEvent {
    ...
    /// Slack's `event_id` (the at-least-once dedupe key, blueprint §7).
...
/// a normalized event delivery (blueprint §10).
#[derive(Debug, Clone, PartialEq)]
#[non_exhaustive]
...
pub enum EventError {
    ...
    /// (validly signed) delivery is rejected here (blueprint §8).
...
/// Verify and normalize an inbound Slack Events-API delivery (blueprint §10) — **pure, no I/O**.
///
