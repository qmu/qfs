...
//! names (blueprint §6). Slack is a **multi-archetype** mount: an **Append/log** of messages,
...
//!   the applier, never during planning — blueprint §3 purity).
...
//! - `/slack/<ws>/files/<file_id>` — one downloadable Slack file blob.
...
/// The closed set of node *kinds* a `/slack/<ws>` mount exposes (blueprint §6). Each carries a distinct
...
pub enum SlackNode {
    ...
    /// `dms/<user>/files` — files SHARED in the DM with `<user>`, backed by the real IM-channel
    /// file-share (`files.list?channel=<im>`), so "latest file in my DM" is provable rather than
    /// inferred from uploader + created time.
    DmFiles {
        /// The DM peer whose IM channel scopes the file listing.
        user: UserRef,
    },
    /// `files`.
    Files,
    /// `<#channel>/files` — files SHARED in `<channel>` (`files.list?channel=<id>`).
    ChannelFiles {
        /// The channel the file listing is scoped to.
        channel: ChannelRef,
    },
    /// `files/<id>`.
    File {
        /// The Slack file id (`Fxxxx`).
        id: String,
    },
    /// `users`.
    Users,
...
impl SlackNode {
    ...
    pub const fn kind(&self) -> NodeKind {
        match self {
            ...
            SlackNode::DmFiles { .. } => NodeKind::Files,
            SlackNode::Files => NodeKind::Files,
            SlackNode::ChannelFiles { .. } => NodeKind::Files,
            SlackNode::File { .. } => NodeKind::Files,
...
impl SlackPath {
    ...
    pub fn parse_str(raw: &str) -> Result<Self, SlackError> {
        ...
        let node = match rest {
            ...
            // /slack/<ws>/dms/<user>/files
            ["dms", user, "files"] => SlackNode::DmFiles {
                user: UserRef::new(*user),
            },
            ...
            // /slack/<ws>/<#channel>/files
            [channel, "files"] => SlackNode::ChannelFiles {
                channel: ChannelRef::new(*channel),
            },
