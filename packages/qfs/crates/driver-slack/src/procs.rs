//! The `CALL slack.*` procedure declarations (blueprint §3 — the irreducible state transitions
...
//! always surfaces them and `COMMIT` requires explicit confirmation (blueprint §7/§8).
//!
...
//!
//! ## The pure `POST` prelude alias (blueprint §3)
...
use qfs_driver::{AliasFn, Param, ProcSig};
use qfs_types::ColumnType;

/// The least-privilege scope a posting/reaction proc advertises (blueprint §8 blast-radius reasoning).
...
/// Build the prelude alias set (blueprint §3): `POST` desugars to a message INSERT (`slack.post`).
#[must_use]
