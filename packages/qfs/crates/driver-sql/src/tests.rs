...
/// A one-column `WHERE <col> == <value>` selector (blueprint §7) — the channel a filter travels on.
fn selector(col: &str, ty: ColumnType, value: Value) -> RowBatch {
    RowBatch::new(
        Schema::new(vec![Column::new(col, ty, false)]),
        vec![Row::new(vec![value])],
    )
}

/// A filtered write: `args` is the SET payload, the selector is the `WHERE`.
fn write_node_where(
    id: u32,
    kind: EffectKind,
    schema: Schema,
    ...
    write_node(id, kind, schema, values).with_selector(sel)
}
...
fn insert_update_delete_effects_change_the_right_rows() {
    ...
    // UPDATE: `args` is the SET payload; the WHERE rides the selector (§7). Note `id` is NOT in the
    // payload — the old lowering inferred the match from the payload's key columns, which is exactly
    // the PK-inference this increment retires.
    let update = write_node_where(
        2,
        EffectKind::Update,
        Schema::new(vec![
            Column::new("name", ColumnType::Text, false),
        ...
        selector("id", ColumnType::Int, Value::Int(4)),
    );
    ...
    // DELETE (REMOVE) the row with id=4. A REMOVE writes nothing — `args` is EMPTY — so the match
    // comes wholly from the selector.
    let remove = EffectNode::new(
        NodeId(3),
        EffectKind::Remove,
        Target::new(PlanDriverId::new("sql"), VfsPath::new("/sql/db/users")),
    )
    .with_selector(selector("id", ColumnType::Int, Value::Int(4)));
    applier.apply_shared(&remove).unwrap();
    assert_eq!(backend.count("users"), 3);
}

#[test]
fn update_honours_a_non_key_where_instead_of_inferring_the_pk() {
    // The semantic change increment 2 buys: the operator's REAL `WHERE` is honoured. The old
    // lowering scanned the payload for PRIMARY-KEY columns and matched on those, so a non-key
    // filter like `WHERE name == 'carol'` was silently IGNORED — it matched by whatever key
    // happened to be in the payload, or refused outright.
    use qfs_runtime::SharedApplier;
    let (driver, backend) = driver_over(USERS_DDL);
    let applier = SqlApplier::new(driver.registry().clone());
    ...
    let update = write_node_where(
        1,
        EffectKind::Update,
        Schema::new(vec![Column::new("age", ColumnType::Int, true)]),
        vec![Value::Int(99)],
        selector("name", ColumnType::Text, Value::Text("alice".to_string())),
    );
    applier.apply_shared(&update).unwrap();

    // alice's age moved; nobody else's did.
    let (rows, _, _s) = driver
        .execute_query(
            &Path::new("/sql/db/users"),
            &QuerySpec::new(vec!["age".to_string()]).with_predicate(Predicate::Cmp(
                ColRef::col("name"),
                CmpOp::Eq,
    ...
    assert_eq!(backend.count("users"), 3, "no row was added or removed");
}

#[test]
fn a_same_column_set_and_where_round_trips() {
    // `UPDATE … SET id = 5 WHERE id = 1` — the shape a single flat `args` batch structurally could
    // NOT express (the WHERE key collided with the SET column and was de-duped away, so the applier
    // could not tell the selector from the new value). The selector channel carries both.
    use qfs_runtime::SharedApplier;
    let (driver, backend) = driver_over(USERS_DDL);
    let applier = SqlApplier::new(driver.registry().clone());
    ...
    let update = write_node_where(
        ...
        vec![Value::Int(5)],
        selector("id", ColumnType::Int, Value::Int(1)),
    );
    applier.apply_shared(&update).unwrap();

    // id 1 became id 5 — the row moved, it was not duplicated or left alone.
    let (rows, _, _s) = driver
        .execute_query(
            &Path::new("/sql/db/users"),
    ...
    assert_eq!(
        rows.len(),
        1,
        "the same-column SET/WHERE must rename in place"
    );
    assert_eq!(backend.count("users"), 3);
}
...
// ----------------------------------------------------------------------------------------------
// ADR 0009 — schema DDL over the /sql/<conn> catalog node (create/drop a table as data).
// ----------------------------------------------------------------------------------------------

/// A column-definition struct value for a CREATE TABLE `columns` array (ADR 0009 §1).
fn col_def(name: &str, ty: &str, pk: bool) -> Value {
    let mut entries = vec![
        ("name".to_string(), Value::Text(name.to_string())),
        ("type".to_string(), Value::Text(ty.to_string())),
    ];
    if pk {
        entries.push(("primary_key".to_string(), Value::Bool(true)));
    }
    Value::Struct(qfs_types::Fields::new(entries))
}

/// Build a catalog-node write effect targeting `/sql/db` (the connection's catalog node).
fn catalog_write(id: u32, kind: EffectKind, schema: Schema, values: Vec<Value>) -> EffectNode {
    let target = Target::new(PlanDriverId::new("sql"), VfsPath::new("/sql/db"));
    let batch = RowBatch::new(schema, vec![Row::new(values)]);
    EffectNode::new(NodeId(id), kind, target).with_args(batch)
}

#[test]
fn create_table_via_insert_into_catalog_node() {
    use qfs_runtime::SharedApplier;
    let (driver, backend) = driver_over(USERS_DDL);
    let applier = SqlApplier::new(driver.registry().clone());
    ...
    let create = catalog_write(
        ...
        vec![
            ...
            Value::Array(vec![
                col_def("id", "int", true),
                col_def("total", "int", false),
            ]),
        ],
    );
    let out = applier.apply_shared(&create).unwrap();
    assert_eq!(out.affected, 1);

    // The new table is now in the backend catalog with the right columns.
    let catalog = backend.introspect().unwrap();
    let orders = catalog
        .tables
        .iter()
        .find(|t| t.name == "orders")
        .expect("orders table created through the language");
    assert!(orders.columns.iter().any(|c| c.name == "id" && c.pk));
    assert!(orders.columns.iter().any(|c| c.name == "total"));

    // In-process catalog refresh (ADR 0009 §4): the applier refreshed the shared catalog after the
    // DDL commit, so the driver — which shares the registry's handle cell — now DESCRIBEs the new
    // table without rebuilding the registry. (Before the create, /sql/db/orders was undescribable.)
    assert!(
        driver.describe(&Path::new("/sql/db/orders")).is_ok(),
        "the driver's cached catalog was refreshed after the create"
    );
}

#[test]
fn drop_table_via_remove_from_catalog_node_and_is_irreversible() {
    use qfs_runtime::SharedApplier;
    // A standalone table (no dependent view) so the post-drop introspect is clean.
    let (driver, backend) = driver_over("CREATE TABLE gone (id INTEGER PRIMARY KEY, note TEXT);");
    let applier = SqlApplier::new(driver.registry().clone());

    // REMOVE FROM /sql/db WHERE name = 'gone' → the `name` key rides the WHERE-SELECTOR (§7); a
    // REMOVE writes nothing, so its `args` is empty.
    let drop = EffectNode::new(
        NodeId(1),
        EffectKind::Remove,
        Target::new(PlanDriverId::new("sql"), VfsPath::new("/sql/db")),
    )
    ...
    // REMOVE is inherently irreversible, so the commit gate requires an explicit acknowledgement.
    assert!(drop.irreversible);

    applier.apply_shared(&drop).unwrap();
    let catalog = backend.introspect().unwrap();
    assert!(
        catalog.tables.iter().all(|t| t.name != "gone"),
        "gone table dropped through the language"
    );
}

#[test]
fn catalog_node_advertises_insert_and_remove_capabilities() {
    let (driver, _be) = driver_over(USERS_DDL);
    // INSERT (create table) and REMOVE (drop table) pass the parse-time gate on /sql/db.
    assert!(check_capability(&driver, &Path::new("/sql/db"), Verb::Insert).is_ok());
    assert!(check_capability(&driver, &Path::new("/sql/db"), Verb::Remove).is_ok());
    // A verb with no catalog meaning (UPDATE) is rejected on the catalog node.
    assert!(check_capability(&driver, &Path::new("/sql/db"), Verb::Update).is_err());
}

#[test]
fn describe_catalog_node_teaches_the_create_table_shape() {
    let (driver, _be) = driver_over(USERS_DDL);
    let desc = driver
        .describe(&Path::new("/sql/db"))
        .expect("the /sql/<conn> catalog node is describable");
    ...
    assert!(
        cols.contains(&"name"),
        "describe teaches the table `name` field"
    );
    assert!(
        cols.contains(&"columns"),
        "describe teaches the `columns` field"
    );
}

#[test]
fn the_catalog_node_is_navigable_but_a_table_is_a_leaf() {
    // §9 enumerable-children conformance (slice 2): `cd /sql/<conn>` enters the catalog — its
    // children are the connection's tables, which are locations. A table itself is a row-bearing
    // leaf. Both describe as `RelationalTable`, so the archetype provably cannot separate them —
    // which is why the `cd` gate reads this per-node fact instead.
    let (driver, _be) = driver_over(USERS_DDL);
    let catalog = driver.describe(&Path::new("/sql/db")).unwrap();
    let table = driver.describe(&Path::new("/sql/db/users")).unwrap();
    assert_eq!(catalog.archetype, table.archetype);
    assert!(catalog.navigable, "cd /sql/<conn> must enter the catalog");
    assert!(
        !table.navigable,
        "a table's children are rows, not locations"
    );
}

#[test]
fn insert_into_catalog_node_without_columns_is_rejected() {
    use qfs_runtime::SharedApplier;
    let (driver, _be) = driver_over(USERS_DDL);
    let applier = SqlApplier::new(driver.registry().clone());
    let schema = Schema::new(vec![Column::new("name", ColumnType::Text, false)]);
    let bad = catalog_write(
        1,
        EffectKind::Insert,
        schema,
    ...
    assert!(
        applier.apply_shared(&bad).is_err(),
        "a CREATE TABLE with no columns is a structured error, not a silent no-op"
    );
}

#[test]
fn reading_the_connection_lists_its_tables_show_tables() {
    // Reading /sql/db (the connection node) returns one row per relation — the SHOW TABLES surface.
    let (driver, _be) = driver_over(USERS_DDL); // USERS_DDL defines table `users` + view `adults`.
    let (rows, residual, schema) = driver
        .execute_query(&Path::new("/sql/db"), &QuerySpec::new(vec![]))
        .expect("listing a connection's tables");
    ...
    assert!(
        ...
        "the users table is listed"
    );
    assert!(
        names.contains(&"adults".to_string()),
        "the adults view is listed"
    );

    // The view row is marked as a view, the base table as a table.
    let kind_of = |name: &str| -> String {
        let row = rows
            .iter()
            .find(|r| matches!(&r.values[0], Value::Text(t) if t == name))
            .unwrap();
        match &row.values[1] {
            Value::Text(k) => k.clone(),
            other => panic!("expected a text kind, got {other:?}"),
        }
    };
    assert_eq!(kind_of("users"), "table");
    assert_eq!(kind_of("adults"), "view");

    // No pushdown: an empty spec leaves no residual.
    assert!(residual.is_none());
}

#[test]
fn connection_node_advertises_select_for_show_tables() {
    let (driver, _be) = driver_over(USERS_DDL);
