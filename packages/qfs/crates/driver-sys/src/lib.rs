//! `qfs-driver-sys` — the **administration driver** (blueprint §6; roadmap §3.4 / M3, t53).
//!
...
pub struct SysDriver {
    ...
    // declaration, blueprint §7). A bounded live tail / small tables — filtering is the engine's work.
...
/// The per-node capability set (blueprint §6). Every admin relation is readable; only `/sys/policies`
...
pub fn sys_node_capabilities(node: SysNode) -> Capabilities {
    match node {
        ...
        // The service-account consent registry (20260703040000, the CREATE ACCOUNT model): SELECT to
        // review, INSERT to declare (`CREATE ACCOUNT` — records consent, gated on a signed-in
        // operator), REMOVE to delete an account (token + consent). Like `/sys/paths`, a user-owned
        // registry the user may withdraw from — the second `/sys` node that accepts REMOVE.
        SysNode::Accounts => Capabilities::from_verbs(&[Verb::Select, Verb::Insert, Verb::Remove]),
