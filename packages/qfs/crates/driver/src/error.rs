...
//! workspace-wide (blueprint §6: errors must be parseable by an AI, not prose).
//!
...
pub enum CfsError {
    ...
    /// A codec failed to decode bytes for its format (blueprint §4/§6). Structured so an AI
    ...
    /// for its format (blueprint §4). Structured for AI recovery: names the `fmt` and reason.
    ...
    /// Carries the offending text and a stable machine-facing reason (blueprint §6).
    ...
    /// (decision P / blueprint §1.3) — e.g. `/members`, `/projects`, `/hosts`, `/directories`,
    ...
    /// A delivered row failed a refined type's `WHERE` membership predicate at the write/`OF`
    /// boundary (blueprint §5.4). Structured for AI recovery: names the path/type and a
    /// secret-free reason (the predicate + constrained columns), never the offending row value.
    #[error("type membership violation at {path:?}: {detail}")]
    TypeMembership {
        /// The declared-view / type path whose contract the row violated.
        path: String,
...
impl CfsError {
    ...
    /// this rather than on the human message (blueprint §6).
    #[must_use]
