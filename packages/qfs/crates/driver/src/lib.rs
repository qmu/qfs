//! `qfs-driver` — the driver contract (blueprint §6).
//!
...
//!
//! ## Schema is the canonical typed model (blueprint §6)
...
/// How a node maps onto qfs's uniform model (blueprint §6, "Four archetypes").
///
...
/// A universal verb (blueprint §3/§6). A **closed set** mirroring the frozen core verbs;
...
/// The set of universal verbs a node supports (blueprint §6). Unsupported verbs are
...
/// Which of blueprint §5.5's **two categories** a node's rows belong to: is this a location holding
/// DATA, or a catalog holding DEFINITIONS? "Paths are data, names are definitions" — the two
/// categories never pool, so a data-row write into a definition catalog is a category error, not a
/// schema mismatch.
///
/// Like [`NodeDesc::navigable`] this is a fact the DRIVER states, never a shell-side list of catalog
/// mount names: a new definition catalog should not require editing the shell.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[serde(rename_all = "snake_case")]
#[non_exhaustive]
pub enum NodeCategory {
    /// A data location — the overwhelming default (`/mail/inbox`, `/sql/db/users`, `/local/x`).
    Data,
    /// A definition catalog (`/type`, `/transform`): its rows ARE definitions, referenced elsewhere
    /// by NAME, never by this path.
    Definition,
}
...
pub struct NodeDesc {
    ...
    pub schema: Schema,
    /// Whether this node's **children are locations** rather than values — i.e. whether it is an
    /// interior you can enter (`cd`), not a row-bearing leaf (blueprint §9's enumerable-children
    /// rule). Set by [`NodeDesc::navigable`]; defaults from the archetype (see
    /// [`navigable_by_default`]).
    ///
    /// **Why this is not derivable from the archetype.** A navigable catalog interior and a leaf
    /// table report the SAME archetype: `describe /transform` (enterable) and `describe /sys/drivers`
    /// (a row leaf) are both `RelationalTable`. The archetype answers "what shape are this node's
    /// rows"; it cannot answer "are this node's children locations". Only the driver knows, per path
    /// — so this is a describe-contract fact the driver states, never a shell-side heuristic.
    pub navigable: bool,
...
/// from, which a driver overrides per path with [`NodeDesc::navigable`].
///
/// A blob namespace's children are paths, and an object graph's are entities addressed as paths, so
/// both default navigable. A relational table's and an append log's children are ROWS — values, not
/// locations — so they default non-navigable, and the drivers whose `/transform`-shaped *interiors*
/// happen to carry one of those archetypes opt back in per path. This keeps the default equal to the
/// gate's historical behaviour: exactly `BlobNamespace | ObjectGraphWorkflow` were enterable.
#[must_use]
pub fn navigable_by_default(archetype: Archetype) -> bool {
    match archetype {
        Archetype::BlobNamespace | Archetype::ObjectGraphWorkflow => true,
        Archetype::RelationalTable | Archetype::AppendLog => false,
    }
}
...
impl NodeDesc {
    ...
    ///
    /// `navigable` starts at the archetype's default ([`navigable_by_default`]); a driver whose node
    /// is an interior the default gets wrong states it with [`NodeDesc::navigable`].
    #[must_use]
    ...
    /// (`/type`, `/transform`) set [`NodeCategory::Definition`]; everything else is data.
    #[must_use]
    ...
    /// State whether this node's children are locations (`cd`-able), overriding the archetype
    /// default. The two directions this exists for:
    /// - `true` on a **catalog interior** that carries a row-shaped archetype (`/transform`, `/type`,
    ///   `/sql/<conn>`, `/sys`, `/server`, a mail label tree) — enterable, though its archetype says
    ///   `RelationalTable`/`AppendLog`.
    /// - `false` on a **leaf** under a navigable-by-default archetype (a blob FILE, not a directory).
    #[must_use]
...
/// (blueprint §3/§6 — the irreducible state transitions).
///
/// `CALL` only resolves procedures a driver declares (capability). `irreversible` lets
/// `PREVIEW` warn and `POLICY` block (blueprint §7/§8); `requires_scopes` lets the server
/// `POLICY` reason about blast radius (blueprint §8) — a hint only, never a credential. Owned
/// data only.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[non_exhaustive]
pub struct ProcSig {
    ...
    /// `Call` effect node so `PREVIEW` can warn (blueprint §7/§8).
    pub irreversible: bool,
    /// The procedure's result schema, if it returns rows (e.g. a search proc).
    pub returns: Option<Schema>,
    /// Least-privilege scope hints the server `POLICY` reasons over (blueprint §8). Owned
...
/// Whether a node carries temporal coordinates (`@version`, blueprint §4) — git refs, S3
...
/// A pure alias function shipped in a driver's prelude (blueprint §3, e.g.
...
/// The consumer-side narrow driver trait (blueprint §6, §11).
///
...
pub trait Driver: Send + Sync {
    ...
    /// [`Target`](qfs_plan::Target) routed here (blueprint §11). Registry identity ([`mount`])
    ...
    /// What this driver can run natively (blueprint §7) — the planner's pushdown input. Pure.
    ...
    /// `@version` support for a node (blueprint §4). Pure; per-node (a driver may version some
    ...
    ///
    /// **Purity (blueprint §3):** like every contract method this is pure — the driver reads only its
    /// already-loaded describe/repo *snapshot* (e.g. the current branch tip) and constructs a
    /// `Plan`; it performs no network/disk I/O (COMMIT remains the sole impure seam).
    ///
    /// The two channels are separate here for the same reason they are on an `EffectNode`
    /// (blueprint §7): `args` is the write's evaluated row **payload** (the `SET`/`VALUES` rows,
    /// positional per the node's write contract), and `selector` is the `WHERE` — *which* existing
    /// rows the write addresses — as a single row of `col == const` bindings, `None` when the
    /// statement carried no `WHERE`. A driver that lowers a filtered `UPDATE`/`REMOVE` reads its
    /// match from `selector`, never from `args`.
    fn plan_write(
        &self,
        _path: &Path,
        _verb: Verb,
    ...
    /// **Optional driver-specific plan-time CALL validation.** Consulted by the evaluator when a
    /// pipeline terminates in `|> CALL <proc>` on this driver's path, *before* the effect node is
    /// committed. Returning `Some(Err(..))` rejects the statement at **plan time** so `PREVIEW` and
    /// `COMMIT` agree (a `PREVIEW` never decodes the effect, so an apply-time-only refusal would let
    /// preview claim an effect the commit then rejects). `Some(Ok(()))` and the default `None` both
    /// let the CALL lower unchanged. `args` is the call's literal-argument row (each column named by
    /// its `name =>` label / declared parameter), and `path` is the CALL's source target — so a
    /// driver can require that a proc resolve a concrete entity (e.g. `mail.send` needs an addressed
    /// draft or `to` recipients, never a byteless create-then-send). Pure: reads only the describe
    /// snapshot, performs no I/O (COMMIT stays the sole impure seam).
    fn plan_call(
        &self,
        _path: &Path,
        _proc: &str,
    ...
    /// Whether a WRITE with `verb` at `path` is **irreversible** (blueprint §7/§8) — consulted by
    /// the planner so it can set [`EffectNode::irreversible`](qfs_plan::EffectNode::irreversible)
    /// on the generic write node, exactly as an inherently-irreversible `REMOVE` (set by the node
    /// kind) or a declared-irreversible `CALL` (`mail.send`) already is. That single bit is what
    /// `PREVIEW` surfaces and `COMMIT` gates behind `--commit-irreversible`. The default is `false`
    /// (reversible): a driver whose universal writes are all reversible needs no override, and
    /// `REMOVE`'s irreversibility is inherent regardless of this hook. A driver that declares a
    /// specific `(path, verb)` write irreversible — e.g. a **declared MAP** marked `IRREVERSIBLE`
    /// onto an external POST (a Slack send) — overrides this. Pure: reads only the describe snapshot.
    fn write_irreversible(&self, _path: &Path, _verb: Verb) -> bool {
        false
    }
...
/// gate for `CALL`. An undeclared `CALL` is rejected structurally (blueprint §3: `CALL`
...
/// The **parse/resolve-time capability gate** (blueprint §6): check that `verb` is supported
...
/// blueprint §3 purity invariant). It is **deliberately not an introspective `Driver`
