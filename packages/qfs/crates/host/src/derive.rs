...
/// (every-minute) — the safe over-fire (at-least-once is idempotent, blueprint §7) rather than a panic.
///
...
pub struct DerivationInput {
    ...
    /// — so its source IS scanned for native-store bindings (blueprint §10: the native binding set is the
