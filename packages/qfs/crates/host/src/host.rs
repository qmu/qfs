//! The **`RuntimeHost` seam** (t36, blueprint §10): "the runtime is just what causes a plan to run"
...
/// or a credential-bearing value (blueprint §8) — it is safe to log and to surface to an operator.
...
/// — the watcher cursors + `LAST_RUN` high-water marks (blueprint §10). `cas` is the at-least-once /
/// idempotency primitive (blueprint §7): a redelivered job/event advances the cursor only if it still
...
pub trait DurableStore {
    ...
    /// redelivery is a no-op). The idempotency primitive (blueprint §7).
    fn cas<'a>(
        &'a self,
        key: &'a StateKey,
...
/// What causes a plan to run, abstracted over EC2 vs Workers (blueprint §10). A host attaches the owned
...
/// runs unchanged on top — the host never bypasses it (the purity invariant, blueprint §3/§7).
pub trait RuntimeHost {
    /// The host clock (blueprint §7: wasm has no `SystemClock`; the daemon reads the system clock, the
...
/// An opaque handle to a bound native store (blueprint §10). Owned + vendor-free: on CF it names the
/// `env` binding (`env.d1(name)`), on the daemon it names the driver's HTTP client. The handle
/// carries the binding NAME only — never a token (blueprint §8). The concrete backend is resolved by
