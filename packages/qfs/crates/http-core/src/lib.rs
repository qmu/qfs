...
//! the **single** header-redaction authority (blueprint §6/§8, t19 refinement).
//!
...
/// presence is surfaced (the name), their value never is (blueprint §8).
///
...
pub const SENSITIVE_HEADERS: &[&str] = &[
    ...
    "api-key",
    "x-auth-token",
    // The Google Gemini API key rides this header (the live model provider's auth); redact its
    // value like every other credential so a `{req:?}` dump can never carry it.
    "x-goog-api-key",
];
...
/// The HTTP method a universal verb maps onto **internally** (blueprint §3 "the path is the type": the
...
pub enum HttpMethod {
    ...
    /// as **not** retry-safe (a timed-out PATCH may have applied; blueprint §7 — do not auto-retry).
    Patch,
    /// `DELETE` — a removal (`REMOVE`). Irreversible (blueprint §8) but idempotent on the wire.
    Delete,
}
...
impl HttpMethod {
    ...
    /// idempotent — blueprint §7, never auto-retry either).
    #[must_use]
...
/// dump cannot leak auth material (blueprint §8).
...
mod tests {
    ...
    fn methods_render_uppercase_tokens_and_post_is_not_retry_safe() {
        ...
        // PATCH is not guaranteed idempotent → not retry-safe (blueprint §7).
    ...
    fn sensitive_headers_are_matched_case_insensitively() {
        ...
        // The Gemini live-provider key header is redacted case-insensitively too.
