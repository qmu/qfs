...
//! [`tokio::net::TcpListener`], NO axum (see blueprint §11 — axum is not in
...
/// The default loopback bind address for `qfs serve`. Loopback-only by default (blueprint §8: a
...
/// Like [`serve_config_full`], but over a **caller-built** [`Runtime`] (blueprint §16 "The face,
/// named"): the daemon composition creates the shared `ServerState` lock + the reconfigure
/// channel FIRST — so the `/server` read facet and the statement-bridge commit path share the
/// live state — and passes `Runtime::with_shared(state, rx)` here. The HTTP binding is still
/// constructed and registered internally (the listener reads its atomically-swapped router).
///
/// # Errors
/// A boxed error if boot fails, the listener cannot bind, or the runtime wiring fails.
...
pub async fn serve_config_shared(
    ...
    let mut runtime = runtime.with_binding(Box::new(binding));
...
/// connection cannot exhaust memory (blueprint §7 resource discipline); a larger request is rejected.
