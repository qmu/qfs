...
//!
//! This crate is the single home of the **frozen reserved-keyword set** (blueprint
...
//! language core (blueprint §2.2). The lexer lives here, not in `qfs-parser`, so the
...
//! no dependencies. It must stay that way so the `wasm32` target (blueprint §11) remains
//! cheap.
...
pub use lex::lex;
pub use reference::{grammar_ebnf, language_model_reference, RESERVED_KEYWORDS};
