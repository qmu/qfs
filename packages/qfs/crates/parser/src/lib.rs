//! `qfs-parser` — the parser front door (blueprint §2.2, §3, §11).
//!
//! t04 promotes the E0/t02 skeleton to the **full blueprint §3 pipe-SQL grammar**: the
//! parser library is **winnow** (locked in blueprint §11), now
...
//!
//! ## Closed core + three open registries (blueprint §3 governance)
...
//! The intended semantic dependency direction is `qfs-core → qfs-parser`; `qfs-parser` must never
//! depend on `qfs-core` (no cycle). The parser may depend on lower leaf vocabulary crates such as
//! `qfs-lang` and `qfs-types` so grammar sugar and semantic validators share the same surface
//! tokens without introducing an upward edge. See `ARCHITECTURE.md`.
//!
...
pub use ast::{
    ...
    Ident, JoinOp, Literal, NamedArg, OfColumn, OfRef, OfTarget, Op, OrderKey, Param, PathExpr,
    PathRef, PathSegment, PipeOp, Pipeline, PlanWrap, PolicyRuleAst, PolicySubjectAst, Projection,
    ServerDdl, Source, Statement, SwitchArm, SwitchStage, TypeAnn, Values,
};
pub use error::{ParseError, ParseErrorCode};

/// The frozen closed-core keyword set (blueprint §3), re-exported from `qfs-lang` so the
...
///
/// Lexes the source via `qfs_lang::lex` (t03), then runs the full blueprint §3 grammar
...
/// code) on any lexing or parse failure — the AI structured-error path of blueprint §6.
