//! `COMMIT` semantics (blueprint §7): the [`PlanApplier`] seam — the **only** impure
...
/// secrets. The audit ledger (blueprint §8, deferred) reconstructs from these.
...
/// The **only** impure seam of the effect substrate (blueprint §3 purity invariant): a sink
...
/// A test double (blueprint §7 / acceptance): records every applied node in call order and
...
/// any failure. Enough for a future recovery pass to reconstruct progress (blueprint §7):
...
///
/// `on_applied` is the single ledger/observability funnel (blueprint §7/§8): it is invoked
