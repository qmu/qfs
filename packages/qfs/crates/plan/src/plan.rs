...
//! invariant check ([`Plan::validate`]). blueprint §7 (runtime / effects-as-data).
//!
...
/// A typed DAG of effects: the value a write statement evaluates to (blueprint §7).
///
...
/// carries the optional `RETURNING` projection schema (blueprint §3 effect statements).
///
/// Built immutably: every combinator returns a new `Plan`, so "constructing a plan
/// does I/O" is unrepresentable (blueprint §3 purity invariant).
...
impl Plan {
    ...
    /// pure read produces this. Reversible, no nodes, no edges (blueprint §3: pure functions
    ...
    /// Whether this plan contains any irreversible effect (blueprint §7/§8 seam).
    #[must_use]
    ...
    /// parallelise them, blueprint §7). Node ids must already be disjoint (the evaluator
