//! Pure plan-level coordinates for a `/server/...` self-config write (blueprint §7/§10).
//!
...
//!
//! ## Why the DTO is not here (purity invariant, blueprint §3)
...
/// Which `/server/...` config collection a self-config write targets (blueprint §10). A
...
/// (blueprint §7). `Upsert` is modelled distinctly so a boot replay / retry converges to the
/// same `ServerState` (idempotency, blueprint §7). Owned data; no I/O.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord, Serialize)]
#[serde(rename_all = "snake_case")]
