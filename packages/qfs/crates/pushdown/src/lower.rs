...
use qfs_types::{
    CmpOp, ColRef, Literal, Name, Pattern, Predicate, ResolvedTransform, Schema, Value,
};
...
pub enum LowerError {
    ...
    /// not wired). Carries the referenced name so an AI can create/rename it.
    TransformNotExecutable {
        /// The referenced (unresolved) transform definition name.
        name: String,
    },
    /// A `TRANSFORM <name>` stage whose declared INPUT names a column the incoming relation
    /// does not carry (surplus incoming columns are fine; a MISSING declared input is a
    /// plan-time error). Mirrors the eval-side `transform_input_missing` so both planning
    /// surfaces diagnose the same pipeline the same way.
    ...
    /// A `SWITCH` stage reached the read-plan lowering (blueprint §18). A switch routes rows to
    /// effect arms; it is terminal-only and lowers at the effect layer (the evaluator builds the
    /// per-arm effect union), never into a read plan — so any switch here is mid-pipe or in a
    /// read-only context (subquery, JOIN source, set-op branch, `LET` binding).
    SwitchNotTerminal,
    /// A `FOLLOW <field>` stage reached the general read-plan lowering (blueprint §13). The
    /// follow (a second wire GET off a delivered URL field) is evaluated by the DECLARED-view
...
impl LowerError {
    ...
    pub const fn code(&self) -> &'static str {
        match self {
            ...
            LowerError::TransformNotExecutable { .. } => "transform_not_executable",
...
impl std::fmt::Display for LowerError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            LowerError::TransformNotExecutable { name } => {
                write!(
                    f,
                    "transform '{name}' is not installed: no such CREATE TRANSFORM definition \
                     (blueprint §15)"
                )
            }
            LowerError::TransformInputMissing { name, column } => {
                write!(
                    f,
                    "transform '{name}' declares INPUT column '{column}' but the incoming \
                     relation does not carry it"
                )
            }
            LowerError::SwitchNotTerminal => f.write_str(
                "a switch stage must be the LAST op of a top-level query pipeline (it routes \
                 rows to effect arms; blueprint §18) — it cannot appear mid-pipe or in a \
                 read-only context (subquery, JOIN source, set-op branch, LET binding)",
            ),
            LowerError::FollowOutsideDeclaredBody => f.write_str(
...
pub fn lower_predicate(expr: &Expr) -> Result<Predicate, LowerError> {
    match expr {
        Expr::Binary { op, lhs, rhs } => match op {
            ...
            Op::Not | Op::Like | Op::Add | Op::Sub | Op::Mul | Op::Div => {
                Err(LowerError::UnsupportedPredicate {
...
pub fn lower_query(
    ...
    transform_of: &impl Fn(&str) -> Option<ResolvedTransform>,
) -> Result<LogicalPlan, LowerError> {
    let mut plan = lower_source(&pipeline.source, source_of, schema_of, transform_of)?;
    for op in &pipeline.ops {
        plan = lower_op(plan, op, source_of, schema_of, transform_of)?;
    }
    Ok(plan)
}
...
fn lower_source(
    ...
    match source {
        ...
        Source::Subquery(inner) => lower_query(inner, source_of, schema_of, transform_of),
        Source::Values(_) => {
...
fn lower_op(
    ...
    match op {
        ...
        PipeOp::Select(projs) => {
            // A projection of only plain **pass-through** columns (`*`/`col`/`a.b`) lowers to the
            // pushable name-only `Project`. Anything that changes a column's identity or value — a
            // rename (`col AS a`) or a computed term (a struct/array constructor, t92) — lowers to
            // the local `ProjectExpr` carrying the per-row exprs, since the name-only `Project`
            // selects by source name and cannot rename (a `col AS a` there would select a
            // non-existent `a` and silently drop the column).
        ...
        PipeOp::Join(join) => {
            let rhs = lower_source(&join.source, source_of, schema_of, transform_of)?;
        ...
        PipeOp::Union(p) => set_op(input, p, SetKind::Union, source_of, schema_of, transform_of),
        PipeOp::Except(p) => set_op(
            input,
            p,
            SetKind::Except,
            source_of,
        ...
        // OF (blueprint §5.6) is a plan-time type ASSERTION with no runtime effect and a schema-
        // identity result: never pushed, never rewrites the plan. Its structural/refinement check
        // runs in the EVALUATOR's schema fold (`Evaluator::check_of_assertion`), which describes the
        // *addressed* path and so has the true column schema — the lowering `schema_of` sees only the
        // driver ROOT. A read carrying an `of` stage is therefore routed through the evaluator
        // (`build_plan`) before it reaches this pure-read lowering (exec `contains_of`), so lowering
        // treats `of` as the identity it is.
        PipeOp::As(_) | PipeOp::Decode(_) | PipeOp::Encode(_) | PipeOp::Call(_) | PipeOp::Of(_) => {
            Ok(input)
        }
        // TRANSFORM (blueprint §15, decision W): resolve the definition and emit the
        // schema-transforming node. The relation becomes the definition's OUTPUT schema (with
        // Provenance tagging the converted columns to the transform), and the node is forced local
        // (`single_source` → None) — the model call is never pushed to a source. An unresolved name
        // is a structured lowering error, never a silent passthrough.
        PipeOp::Transform(t) => {
            let def = transform_of(&t.name).ok_or_else(|| LowerError::TransformNotExecutable {
                name: t.name.clone(),
            })?;
            // The same by-name INPUT-column presence check the eval schema fold applies
            // (`transform_input_missing`): every declared INPUT column must be carried by the
            // incoming relation (surplus incoming columns are ignored). Skipped only when the
            // incoming names are indeterminate at lowering (e.g. below an EXPAND) — the eval
            // fold still catches those.
            if let Some(incoming) = available_columns(&input) {
                if let Some(missing) = def
                    .input
                    .columns
                    .iter()
                    .find(|c| !incoming.contains(&c.name))
                {
                    return Err(LowerError::TransformInputMissing {
                        name: t.name.clone(),
                        column: missing.name.clone(),
                    });
                }
            }
            let output_schema = provenance_tagged(&def.output, &t.name);
            Ok(LogicalPlan::Transform {
                input: Box::new(input),
                name: t.name.clone(),
                output_schema,
                mode: def.mode,
        ...
        // SWITCH (blueprint §18): terminal-only, lowered at the EFFECT layer (the evaluator
        // builds the per-arm effect union; the exec commit boundary routes rows). A switch
        // reaching the read-plan lowering is mid-pipe or in a read-only context — a structured
...
/// The column NAMES a lowered subtree makes available to the next stage, or `None` when they are
/// indeterminate at lowering (below an `EXPAND`, whose flattening rewrites names type-dependently).
/// Name-availability is all the transform INPUT presence check needs — no type fold here; the
/// eval-side schema fold remains the typing authority.
fn available_columns(plan: &LogicalPlan) -> Option<Vec<Name>> {
    match plan {
        ...
        | LogicalPlan::Distinct { input } => available_columns(input),
        LogicalPlan::Project { columns, .. } => Some(columns.clone()),
        LogicalPlan::ProjectExpr { projections, .. } => {
            Some(projections.iter().map(|(n, _)| n.clone()).collect())
        }
        LogicalPlan::Extend { input, assignments } => {
            let mut names = available_columns(input)?;
        ...
        // EXPAND flattens a struct/array-of-struct column into its (type-dependent) field
        // columns; lowering does not carry the nested types, so the names are indeterminate.
        LogicalPlan::Expand { .. } => None,
        LogicalPlan::Join { lhs, rhs, .. } => {
            let mut names = available_columns(lhs)?;
            names.extend(available_columns(rhs)?);
            Some(names)
        }
        // A set op's relation is the unified (lhs-shaped) column set.
        LogicalPlan::SetOp { lhs, .. } => available_columns(lhs),
        LogicalPlan::Transform { output_schema, .. } => Some(
            output_schema
                .columns
                .iter()
                .map(|c| c.name.clone())
...
/// Tag every column of a transform's OUTPUT schema with the transform's provenance (the definition
/// name as the producing "driver"), so an audit / `unify` can see the columns were produced by the
/// model stage rather than read from a source.
fn provenance_tagged(output: &Schema, name: &str) -> Schema {
    use qfs_types::{DriverId, Provenance};
    let prov = Provenance {
        driver: Some(DriverId::new(format!("transform:{name}"))),
        source_col: None,
    };
    let cols = output
        .columns
        .iter()
        .map(|c| c.clone().with_provenance(prov.clone()))
        .collect();
    Schema::new(cols)
}
...
fn set_op(
    ...
    let rhs = lower_query(rhs_pipe, source_of, schema_of, transform_of)?;
...
/// Whether a projection term is a plain, pushable **pass-through** column (`*`, `col`, or a dotted
/// path) — it selects an existing column by its own name, unchanged. A rename (`col AS a`) is NOT
/// plain: the name-only `Project` selects by source name and cannot rename, so an aliased column
/// must route through the local `ProjectExpr` (which maps `source → alias` per row). Without this
/// an aliased plain projection selected a non-existent column named `a` and silently dropped it.
fn is_plain_projection(p: &Projection) -> bool {
    match p {
        Projection::Star => true,
        Projection::Expr { expr, alias } => {
            alias.is_none() && matches!(expr, Expr::Col(_) | Expr::Path(_))
        }
    }
}
...
///
/// Public because the §13 declared-map write path (`qfs_exec::declared::eval_map_body`) lowers a
/// stored `VALUES (<expr>)` wire-body expression the same way a computed `SELECT` term is lowered,
/// then evaluates it per incoming row via [`qfs_engine::eval_value`] — one lowering, both paths.
///
/// # Errors
/// [`LowerError::UnsupportedProjection`] if `expr` is not a per-row scalar (a column/path, a
/// literal, or a `[ ]`/`{ }` constructor).
pub fn lower_scalar(expr: &Expr) -> Result<ScalarExpr, LowerError> {
    match expr {
...
fn cmp_op(op: Op) -> Result<CmpOp, LowerError> {
    match op {
        ...
        Op::And | Op::Or | Op::Not | Op::Like | Op::Add | Op::Sub | Op::Mul | Op::Div => {
            Err(LowerError::UnsupportedPredicate {
