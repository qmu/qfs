//! [`partition_by_source`] — the source-splitting pass (blueprint §7, ticket t14 step 2/3).
//!
...
//!   side's pushed-down result (blueprint §7 federation). Native join pushdown into a single
...
/// (the verb-capability gate, blueprint §6). A source that cannot `SELECT` is rejected at plan
...
impl SourceRegistry {
    ...
    /// it fails at plan time with [`PlanError::CapabilityDenied`] (blueprint §6/§8).
...
/// [`CombineOp`] residual (blueprint §7). Pushes maximal work to each source; federates the
/// rest locally.
///
/// # Errors
...
fn single_source_chain(plan: &LogicalPlan) -> Option<SourceId> {
    match plan {
        ...
        // A transform is a unary stage: its input chain may still push to one source (the read
        // BELOW the transform), while the transform itself is force-localised in `walk_chain`.
        | LogicalPlan::Transform { input, .. } => single_source_chain(input),
...
fn push_chain(
    ...
    // Capability gate (blueprint §6/§8): a source whose node denies SELECT is rejected at
...
fn scan_path(plan: &LogicalPlan) -> String {
    match plan {
        ...
        | LogicalPlan::Aggregate { input, .. }
        | LogicalPlan::Expand { input, .. }
        | LogicalPlan::Transform { input, .. } => scan_path(input),
...
fn walk_chain(plan: &LogicalPlan, acc: &mut Acc) -> Schema {
    match plan {
        ...
        // TRANSFORM (blueprint §15, decision W): force local — push the upstream read, then keep
        // the model call (and, via `local_pinned`, everything after it) as a local residual. The
        // relation's schema becomes the definition's OUTPUT.
...
fn federate(plan: &LogicalPlan, reg: &SourceRegistry) -> Result<PhysicalPlan, PlanError> {
    match plan {
        ...
        // TRANSFORM over a cross-source input: federate the input (each side maximally pushed) and
        // keep the model call as a local combine above it (blueprint §15 — never pushed).
        LogicalPlan::Transform {
            input,
            name,
            output_schema,
