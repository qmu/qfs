...
//! COMPILED describe registry ([`crate::describe::compiled_describe_registry`]) — the connection-
//! independent driver TYPE set, never the operator's live mounts — and folds each driver's existing
...
//! [`DriverDoc`] DTO. No vendor SDK type leaks (blueprint §11); no credential is resolved and no I/O
//! happens (DESCRIBE is pure, blueprint §3) because only the introspective methods are called.
//!
//! ## Codecs are catalogued from the builtin codec set
//! The `DECODE`/`ENCODE` formats are a single workspace-wide open registry (blueprint §4), not a
...
use crate::describe::compiled_describe_registry;

/// One driver's catalog entry — an **owned DTO** rendered into `docs/drivers.md`. Every field
/// reuses an existing owned workspace type (blueprint §11: no vendor SDK type leaks); it is produced by
/// folding the t39 describe surface, never hand-authored.
#[derive(Debug, Clone, PartialEq)]
pub struct DriverDoc {
    /// The driver mount, e.g. `/mail` (the paths-registry key, blueprint §3).
    pub mount: String,
    /// A representative node path under the mount that the catalog describes, e.g. `/mail/drafts`.
    pub example_path: String,
    /// How a representative node maps onto qfs's uniform model (the archetype, blueprint §6).
    ...
    /// the unsupported ones (blueprint §6: never document a capability by omission).
    ...
    /// What a representative node pushes down natively (the planner input, blueprint §7).
...
pub struct Catalog {
    ...
    /// The builtin `DECODE`/`ENCODE` format names (the shared codec registry, blueprint §4).
...
fn representative_path(mount: &str) -> String {
    match mount {
        ...
        // into the generated driver catalog like any other introspective facet. Decision W: the
...
pub fn driver_catalog() -> Catalog {
    // The COMPILED registry only — gen-docs must be a pure function of the binary, never of the
    // operator's live CONNECT-ed/declared mounts (which would pollute + de-idempotent the catalog).
    let reg = compiled_describe_registry();
