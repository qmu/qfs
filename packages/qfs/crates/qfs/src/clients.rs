//! The shared **live-client builders** for the credentialed networked drivers (github) —
...
//! module the commit path built its `RestGitHubClient` inline; the read facet
...
//! `/github` statement fails honestly ("no driver / no source") rather than acting — or
