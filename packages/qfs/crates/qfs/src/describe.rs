...
pub(crate) fn register_defined_paths_where(
    ...
    // The declared `OF` contracts every declared mount's describe surface resolves against — read
    // once here, not once per binding (ticket 20260728085253).
    let declared_types = if declared.is_empty() {
        qfs_core::DeclaredTypeDefs::new()
    } else {
        crate::declared_driver::load_declared_type_defs()
    };
        ...
        .filter(|b| include(b))
    {
        ...
        if let Some(driver) = cred_free_driver(id) {
            ...
            if let Some(wrapped) =
                crate::declared_driver::declared_describe_mount(&b.path, d, &declared_types)
            {
                let _ = reg.register(Arc::new(wrapped));
            }
        }
...
mod tests {
    ...
    fn two_source_resolution_lets_compiled_win_and_reports_the_shadow() {
        ...
        let mount = crate::declared_driver::declared_describe_mount(
            "/slack",
            &declared[1],
            &qfs_core::DeclaredTypeDefs::new(),
        )
