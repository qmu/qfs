...
mod tests {
    ...
    fn open_system_db_creates_and_remigrates_idempotently() {
        ...
        // sessions t46 + v5 oauth keys t48 + v6 oauth flow clients/codes t49).
        let sys = open_system_db().unwrap().expect("config home resolves");
        assert_eq!(qfs_store::applied_migrations(sys.db()).unwrap().len(), 6);
        drop(sys);
        ...
        assert_eq!(qfs_store::applied_migrations(sys2.db()).unwrap().len(), 6);
        match prev_xdg {
            Some(v) => std::env::set_var("XDG_CONFIG_HOME", v),
            None => std::env::remove_var("XDG_CONFIG_HOME"),
        }
