//! `qfs::version` — the version + build-metadata surface (ticket t40, blueprint §11).
//!
...
//!
//! No secret is embedded here (blueprint §8): a semver, a commit hash, a target triple, and a
...
/// Whether this build's target is `wasm32-*` capable (blueprint §11 Cloudflare Workers target).
...
mod tests {
    ...
    fn long_version_carries_semver_sha_and_target() {
        ...
        // No credential shape ever leaks into the version banner (blueprint §8).
