//! The runtime-side **batch driver** seam (blueprint §6/§11) and the [`DriverRegistry`]
...
//! and `qfs-driver` stay I/O-free (the purity invariant, blueprint §3). A real E4 driver bridges
...
/// driver's own construction, blueprint §8). Reserved for E4 to thread a request id / deadline;
...
/// node (blueprint §11: owned DTOs, no vendor types). The driver fans its results back aligned to
...
/// the same `(DriverId, EffectKind)` grouping key) in one call (blueprint §7 auto-batching). The
