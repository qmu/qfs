...
//! (blueprint §7 observability, §6 structured-error path). Errors are owned DTOs with a
...
/// surfaces. The variants are the **recovery-relevant classes** (blueprint §7): the scheduler
...
pub enum EffectError {
    ...
    /// The effect's driver/verb is not in the active [`CapabilitySet`](crate::CapabilitySet), or
    /// is granted only under a [`PathScope`](crate::PathScope) the target path does not match.
    /// Rejected by the runtime **before** dispatch (defense in depth; the parse-time gate
    /// is t13). Never retried. The `path` is carried so a path-scoped denial is diagnosable
    /// (secret-free — a VFS path, never a payload or credential).
    #[error("capability denied: driver `{}` cannot {verb} at `{path}`", driver.as_str())]
    CapabilityDenied {
        /// The driver the denied effect targeted.
        driver: DriverId,
        /// The verb label that was denied (e.g. `REMOVE`, `CALL`).
        verb: String,
        /// The target VFS path that was denied (names which node the scoped grant refused).
        path: String,
    },
    ...
    /// is never retried even on timeout (blueprint §7 idempotency).
    ...
    /// re-apply, so this is terminal for the leg (blueprint §7 no-lost-update).
    ...
    /// the mount; blueprint §8). This is a distinct, security-relevant discriminant from a plain
