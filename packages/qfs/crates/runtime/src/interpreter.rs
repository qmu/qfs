//! The effect-plan **interpreter** (blueprint §3 `COMMIT : Plan -> World`, §7 runtime) — the
...
/// The effect interpreter (blueprint §3/§7). Holds the apply-time [`DriverRegistry`], the
...
impl Interpreter {
    ...
    /// **COMMIT** (blueprint §3 the only impure op): apply the plan against the World. Walks the
