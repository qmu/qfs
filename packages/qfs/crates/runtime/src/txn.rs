//! Interpreter integration of the transactional envelope (t11, blueprint §7/§8).
//!
...
impl Interpreter {
    ...
    pub async fn commit_txn(
        ...
        // Observability root span (blueprint §7): one trace id per execution, threaded through every
        // per-leg child span so every applied-effect log line carries `trace_id` + `plan_id`.
        // Metadata only — never a payload or credential (blueprint §8).
        ...
        for id in &order {
            ...
            // secret-free (blueprint §8). Every line carries the inherited `trace_id`/`plan_id` from
    ...
    async fn apply_txn_leg(
        ...
        // Per-leg child span (blueprint §7): inherits the root `trace_id`/`plan_id` and adds this
        // leg's identity, so every log line emitted while the leg applies carries all three
        // ids. Metadata only — driver + path + kind, never a payload or credential (blueprint §8).
        ...
        // must NOT be blindly replayed (blueprint §7/§8 apply-once): surface it as `Indeterminate`
...
fn map_effect_error(e: EffectError, precondition: &Precondition) -> LegOutcome {
    match e {
        ...
        EffectError::CapabilityDenied { driver, verb, path } => {
            LegOutcome::Failed(TxnError::terminal(format!(
                "capability denied: driver `{}` cannot {verb} at `{path}`",
                driver.as_str()
            )))
        }
