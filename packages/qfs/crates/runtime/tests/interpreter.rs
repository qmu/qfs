...
use qfs_plan::{depends_on, EffectKind, EffectNode, NodeId, Plan, ProcId, Target, VfsPath};
use qfs_runtime::{
...
fn registry(driver: Arc<MockDriver>) -> qfs_runtime::DriverRegistry {
    qfs_runtime::DriverRegistry::new().with(driver_id(), driver)
}
...
async fn irreversible_leg_is_never_retried() {
    // An irreversible CALL that fails retryably: it must NOT be retried (blueprint §7).
...
async fn ungranted_effect_is_capability_denied_before_dispatch() {
    ...
    match &entry.status {
        LegStatus::Failed {
            error: EffectError::CapabilityDenied { driver, verb, path },
            ..
        } => {
            assert_eq!(driver.as_str(), "mock");
            assert_eq!(verb, "REMOVE");
            assert!(!path.is_empty(), "denial names the target path");
        }
...
async fn unregistered_driver_fails_every_leg_terminally() {
    ...
    let interp = Interpreter::with_defaults(qfs_runtime::DriverRegistry::new());
    let outcome = interp.commit(plan, &allow_all()).await.unwrap();
    assert!(matches!(
        &outcome.ledger[0].status,
        LegStatus::Failed {
...
async fn wide_frontier_admits_at_most_global_group_futures() {
    ...
    let mut reg = qfs_runtime::DriverRegistry::new();
    let mut plan = Plan::pure();
    for i in 0..WIDTH {
...
async fn preview_skip_accounting_matches_commit_for_same_shaped_plan() {
    ...
    let disposition = |o: &qfs_runtime::Outcome| -> Vec<(NodeId, &'static str)> {
        o.ledger
            .iter()
            .map(|e| {
    ...
    let cause = |o: &qfs_runtime::Outcome, id: u32| {
        o.ledger
            .iter()
            .find(|e| e.id == NodeId(id))
