//! [`LocalStore`] — the native, encrypted-at-rest credential backend (blueprint §8).
//!
...
impl LocalStore {
    ...
    ///
    /// # Errors
    /// [`SecretError::Backend`] if an existing group/world-readable file cannot be tightened to
    /// owner-only `0600` (a foreign owner, or a mode-ignoring filesystem) or its directory cannot be
    /// prepared. A loose file we own is self-healed, not rejected (ticket 20260705015500).
    ...
    ///
    /// # Errors
    /// [`SecretError::Backend`] if key derivation fails or an existing loose file cannot be
    /// tightened to owner-only `0600`.
...
/// Ensure the credential blob is owner-only, **self-healing** a loose-but-owned file to `0600` on
/// open rather than refusing (ticket 20260705015500) — the same upgrade-friction fix the store DBs
/// got. A file we cannot chmod (foreign owner → `EPERM`, or a mode-ignoring filesystem) is still
/// refused. The guard only ever tightens, never loosens.
#[cfg(unix)]
fn verify_owner_only(path: &Path) -> Result<(), SecretError> {
    ...
    // Loose bits: tighten to 0600 (safe self-heal for our own blob). `set_permissions` fails with
    // EPERM for a foreign-owned file — the correct refusal without an explicit uid syscall.
    std::fs::set_permissions(path, std::fs::Permissions::from_mode(0o600)).map_err(|e| {
        SecretError::Backend(format!(
            "credential blob is group/other-accessible and could not be tightened to owner-only \
             0600 ({e}); run `chmod 600` on it — or investigate if it is not yours"
        ))
    })?;
    // Re-verify the tighten landed (a mode-ignoring filesystem would leave it loose).
    let mode = std::fs::metadata(path)
        .map_err(|e| SecretError::Backend(format!("re-stat credential blob: {e}")))?
        .permissions()
        .mode();
    if mode & 0o077 != 0 {
        return Err(SecretError::Backend(format!(
            "credential blob stayed group/other-accessible (mode {:o}) after chmod; refusing to \
             use it",
            mode & 0o777
        )));
    }
...
mod tests {
    ...
    /// A group/world-readable blob that we own is SELF-HEALED on open — tightened to 0600 and
    /// opened, not rejected (ticket 20260705015500).
    #[cfg(unix)]
    #[test]
    fn group_readable_blob_is_self_healed() {
        use std::os::unix::fs::PermissionsExt;
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("credentials");
        let store = LocalStore::open_with_key(&path, key_bytes()).unwrap();
        ...
        // Loosen perms behind the store's back, then a fresh open must tighten + succeed.
        ...
        match LocalStore::open_with_key(&path, key_bytes()) {
            Ok(_) => {
                ...
                assert_eq!(
                    mode, 0o600,
                    "the loose blob was tightened to owner-only, got {mode:o}"
                );
            }
            Err(err) => panic!("expected a loose owned blob to be tightened, not rejected: {err}"),
        }
    }
