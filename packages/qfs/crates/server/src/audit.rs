//! The server audit ledger (blueprint §7/§8): an append-only record of every `/server`
//! mutation (who/op/node/before-after) and every fired plan. Owned, **secret-free** data —
//! it records *names* and *ops*, never a row's credential-bearing contents (blueprint §8).
//!
...
pub enum AuditEntry {
    ...
    /// E7 bindings; the shape is reserved here so the ledger is the one funnel (blueprint §7).
    PlanFired {
    ...
    /// unattended-execution funnel (blueprint §7/§8).
...
impl AuditSink {
    ...
    /// breaks the operation, blueprint §7) rather than a panic — the lib stays panic-free.
    ...
    /// funnel every E7 committer calls so no plan fires unaudited (blueprint §7/§8).
    ...
    pub fn drain(&self) -> usize {
        ...
        for entry in &entries {
            tracing::info!(target: "qfs::server::audit", "{}", entry.summary());
        }
        entries.len()
    }
}
