//! Lower a server-config statement to a **pure** `/server` write plan (blueprint §7/§10).
//!
...
fn lower_ddl(ddl: &ServerDdl) -> Result<Plan, String> {
    ...
    // blueprint §7 idempotency), coherent with t30.
...
fn is_body_column(node: ServerNode, col: &str) -> bool {
    matches!(
        ...
        (ServerNode::Endpoints | ServerNode::Views, "query")
            | (ServerNode::Triggers | ServerNode::Jobs | ServerNode::Agents, "plan")
