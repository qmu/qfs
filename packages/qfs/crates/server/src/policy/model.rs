//! Owned, vendor-free policy DTOs (blueprint §10/§11/§8): [`Verb`], [`Effectivity`],
//! [`VerbSet`], [`DriverGlob`], [`Rule`], and [`Policy`].
//!
//! A [`Policy`] is the **may** layer (blueprint §8 least privilege): per handler, the set of
...
/// A universal write/read verb (blueprint §3/§6). The closed-core verb taxonomy the policy
...
pub enum Verb {
    ...
    /// `UPDATE`.
    Update,
    /// `REMOVE` — destructive / irreversible (blueprint §8).
    Remove,
...
impl Verb {
    ...
    /// Whether this verb is **irreversible** for the strictness rule (blueprint §7/§8): `REMOVE`
...
/// the `Policy::default` effectivity — `Deny` is the fail-closed default (blueprint §8).
...
///
/// **Strictness note (blueprint §7/§8):** `ALL` *does* include `REMOVE` and `CALL`, but the
...
/// A driver-scope glob (blueprint §10): matches the **leading `/driver/...` segment(s)** of an
...
impl DriverGlob {
    ...
    /// segments**, not driver internals (blueprint §10): the policy reads the plan node's
...
/// One policy rule (blueprint §10): grant/deny a [`VerbSet`] on a [`DriverGlob`]. Ordered within a
...
pub struct Rule {
    ...
    /// irreversible-strictness rule (blueprint §7/§8) uses this: a `ALLOW ALL` does **not** grant
...
impl Rule {
    ...
    pub fn matches(&self, verb: Verb, driver: &str, path: &str) -> bool {
        ...
        // Irreversible strictness (blueprint §7/§8): a broad `ALLOW ALL` does NOT grant the
...
/// A least-privilege policy (blueprint §10/§8): a named, ordered list of [`Rule`]s plus the
