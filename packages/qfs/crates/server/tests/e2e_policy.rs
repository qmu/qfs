//! E2E / external-interface validation of the t35 POLICY engine (blueprint §10/§8), driven
...
fn scenario1_grammar_golden_and_roundtrip() {
    // The blueprint §10 golden example parses to the expected Policy DTO.
    ...
    assert_eq!(
        parsed, expected,
        "CREATE POLICY parses to the blueprint §10 Policy DTO"
    );
...
//        after a kill mid-plan (blueprint §7 idempotency / recovery).
// ===========================================================================

/// A `cp` across sources lowers to copy → verify → delete (blueprint §7): three legs, the delete
...
//        PREVIEW applies ZERO effects (PREVIEW-as-CI, no live creds; blueprint §3/§10).
// ===========================================================================
