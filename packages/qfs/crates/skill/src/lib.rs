...
/// The shipped **declared-driver** script for Slack — blueprint §13.3 playbook entry #1, and now the
/// sole `/slack` driver (it replaced the retired compiled `driver-slack`). A multi-statement
/// `CREATE DRIVER`/`TYPE`/`VIEW`/`MAP` install program
...
/// read-over-POST stage (the DM `conversations.open`). Every read it declares was proven
/// ROW-EQUIVALENT to the retired compiled driver on shared hermetic fixtures before that crate was
/// deleted. Credential-free by
