//! The qfs-skill **golden example corpus** (ticket t39, blueprint §3/§6/§7/§10).
//!
...
//!   an effect procedure — it declares no result schema — so its terminal CALL is an effect the
//!   irreversible gate holds, not a pure read). A CALL to a result-returning procedure would
//!   instead stay a read relation.
...
//!   `/server` config write, the blueprint §10 PREVIEW-as-CI-test pattern).
//! - **NEGATIVE** — an unsupported verb for a node fails at resolve time with the structured
//!   `unsupported_verb` error (blueprint §6 — the agent-legible failure path), before any plan or I/O.
...
fn mail_insert_draft_previews_reversible_plan() {
    ...
    assert_plan(
        // Named columns: a draft's read shape (message rows) differs from its compose fields, so a
        // bare positional VALUES row is rejected at plan time — name `to`/`subject`/`body`.
        "INSERT INTO /mail/drafts VALUES (to, subject, body) ('alice@example.com', 'Hi', 'Body text')",
        &service_registry(),
    )
...
fn drive_upsert_blob_previews_plan() {
    // Step 2/3: a retry-safe blob write to Drive (UPSERT = the idempotent default, blueprint §7). This is
...
#[test]
fn github_merge_call_lowers_to_an_irreversible_call_effect() {
    // A terminal `…/pulls/42 |> CALL github.merge(method => 'squash')` lowers to a `Call` EFFECT
    // plan: github.merge declares no result schema, so its terminal CALL is an effect (not a pure
    // read), and its declared irreversibility rides on the node for the agent's COMMIT gate.
    let value = eval_value(
        "/github/acme/web/pulls/42 |> CALL github.merge(method => 'squash')",
        &service_registry(),
    );
    let EvalValue::Plan(plan) = value else {
        panic!("a terminal effect CALL lowers to a Plan, got: {value:?}");
    };
    assert_eq!(plan.nodes().len(), 1, "a lone Call node");
    assert_eq!(
        plan.nodes()[0].kind,
        EffectKind::Call(qfs_core::ProcId::new("github.merge"))
    );
    assert!(
        plan.nodes()[0].irreversible && plan.is_irreversible(),
        "github.merge is declared irreversible — the node and plan are flagged"
    );
    // The declared `merge` procedure is irreversible on its contract too (the resolve-layer view).
...
fn server_trigger_previews_single_config_write() {
    // CREATE TRIGGER desugars to exactly one /server config write (blueprint §10 PREVIEW-as-CI-test). No
