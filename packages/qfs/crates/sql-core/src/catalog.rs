//! `catalog` — owned, vendor-free catalog DTOs and the SQL→qfs schema reconciliation (blueprint
...
impl TableCatalog {
    ...
    /// Project this catalog into the canonical typed [`Schema`] (blueprint §6) — what `DESCRIBE`
    /// returns. Each column carries provenance back to the `sql` driver and its source column
    /// name, so a federated `JOIN` can disambiguate (blueprint §6/§8). No secrets.
    #[must_use]
