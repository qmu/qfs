//! `compile` — the pure **qfs query → parameterized SQL plan** compiler (blueprint §6/§7, ticket
...
/// engine still filters locally (blueprint §7). `residual` is `None` when the whole `WHERE` pushed down
/// with exact mappings.
