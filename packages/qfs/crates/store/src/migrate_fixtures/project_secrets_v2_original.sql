...
-- One row per stored credential, keyed by (driver, account) — exactly the `CredentialKey` model.
-- `ciphertext` is the secret value sealed under the project's data-key (DEK) with a fresh per-value
-- `nonce`; the plaintext NEVER lands here. `created_at` is plaintext metadata for `account list`.
...
-- The persistent `{driver -> active account}` selection (moved off the old `credentials.active`
-- sidecar). One row per driver; `qfs account use` UPSERTs (last-writer-wins), the commit resolver
