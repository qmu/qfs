//! Handler PREVIEW fixtures (t38, blueprint §10): `preview_handler(ddl_src) -> Plan` — drive a
//! `CREATE ENDPOINT/TRIGGER/JOB` binding to the `Plan` a fired binding would COMMIT, **no
//! socket, no live backend**.
//!
//! ## PREVIEW-as-test (blueprint §10)
//! A server binding (`CREATE ENDPOINT`/`TRIGGER`/`JOB`) **desugars** to exactly one
//! `/server/<kind>` config-write effect-plan (the canonical closed-core rule, blueprint §7/§10). That
...
mod tests {
    ...
    fn create_endpoint_previews_a_single_server_config_write() {
        ...
        // A config write is reversible (blueprint §7) — no irreversible node, nothing to warn on.
