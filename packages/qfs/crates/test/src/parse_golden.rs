//! Parser/grammar goldens (t38, blueprint §3/§6): `golden_parse(src) -> AstSnapshot` over the
...
//! (blueprint §6: machine code + non-empty expected-set) is locked too.
...
/// facing structured-error contract (blueprint §6): the machine code, the non-empty expected-set,
...
pub struct ParseErrorSnapshot {
    ...
    pub at: usize,
    /// The non-empty expected-token set (blueprint §6 guarantees non-empty).
    pub expected: Vec<String>,
    /// The kind of token actually found (never a literal value — blueprint §8 secret hygiene).
    pub found: String,
...
mod tests {
    ...
    fn error_snapshot_captures_a_stable_structured_message() {
        ...
        assert!(
            ...
            "expected-set is non-empty (blueprint §6)"
        );
