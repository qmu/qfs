//! Plan-shape assertions (t38, blueprint §3/§7): `assert_plan(src, reg) -> PlanAssert` and the
//! fluent `.nodes(...)` / `.irreversible(n)` / `.no_io_performed()` / `.snapshot(name)`.
//!
//! **Plan assertion over mocking is the thesis** (blueprint §3/§7): because a write statement
...
impl PlanAssert {
    ...
    /// Assert the number of **irreversible** nodes (blueprint §7 safety surface) equals `count`.
    ///
    /// # Panics
    ...
    /// Assert that **no I/O was performed** building this plan (blueprint §3 purity invariant from
...
/// Return a `Plan` with a **stable canonical node + edge order** (blueprint §7 determinism): nodes
