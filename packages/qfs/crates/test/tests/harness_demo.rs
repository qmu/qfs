...
fn remove_plan_is_irreversible() {
    // REMOVE is inherently irreversible (blueprint §7/§8) — the harness asserts the safety surface.
...
fn upsert_apply_twice_converges_through_fake_backend() {
    ...
    // Apply-twice CONVERGES — retry-safety via the FakeWorld state assertion (blueprint §7).
