...
//! (blueprint §7 recovery, §8 audit — credentials and payloads never logged).
...
/// **never** the row payload or any credential (blueprint §8 — redact at this boundary). The
...
impl EffectDescriptor {
    ...
    ///
    /// - `UPSERT` is create-or-update — re-applying it converges (blueprint §7 the driver-side
...
/// A structured, machine-readable per-leg failure (blueprint §7) — the saga/strategy counterpart
...
/// The outcome of applying one effect leg (blueprint §7) — the closed set the saga/strategy
...
pub enum LegOutcome {
    ...
    /// world coordinate (an `If-Version`/ETag token), never a credential (blueprint §8).
    Conflict {
    ...
    /// could double-apply (blueprint §7/§8 apply-once). Surfaced for `UPSERT`-style re-apply or
