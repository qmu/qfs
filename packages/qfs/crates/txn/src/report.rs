//! The per-commit **recovery report** (blueprint §7 observability, §8 audit-of-record).
//!
...
/// The whole-commit recovery report (blueprint §7/§8). Deterministic leg order (plan order), so
...
impl RecoveryReport {
    ...
    /// reconcile pass refused to silently replay it (blueprint §7/§8 apply-once). Each needs an
