//! The **saga executor** and the single-source ACID path (blueprint §7).
//!
...
/// Tuning for the saga's optimistic-concurrency auto-retry (blueprint §7). `conflict_retries` is
...
/// Drives an ordered `Vec<EffectLeg>` to completion or recovery (blueprint §7). Pure orchestration
...
impl<'a> SagaExecutor<'a> {
    ...
    fn apply_leg(&self, applier: &mut dyn LegApplier, leg: &EffectLeg) -> LegOutcome {
        ...
        loop {
            ...
            match outcome {
                ...
                // unchanged — never re-applied (blueprint §7/§8 apply-once); the saga treats it as a
                // hard stop below.
    ...
    /// Run the ordered legs as a saga (blueprint §7): apply in order; on the first **hard** failure
    ...
    /// The single-source **ACID** path (blueprint §7): the same per-leg idempotency rules, but a
