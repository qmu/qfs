//! Commit-strategy selection (blueprint §7) and the recoverable `cp`/`mv` triple.
//!
...
/// How a commit will be executed (blueprint §7), chosen by inspecting the plan's write targets.
...
/// Whether a driver declares real transaction support (blueprint §7) — the saga-vs-ACID input the
...
/// The three steps a cross-mount `cp`/`mv` compiles to (blueprint §7, the canonical recoverable
