//! In-crate unit tests for the transactional envelope (blueprint §7/§8). Every test uses an
...
fn recovery_report_with_every_outcome_variant_serializes() {
    ...
    // world coordinate — never a credential (blueprint §8).
    assert!(
...
/// `Insert`/`Remove`/`Call` is NOT (a blind replay could double-apply — apply-once, blueprint §7/§8).
#[test]
...
/// A cross-source reversible transaction has no distributed 2PC (blueprint §7): it is a best-effort
