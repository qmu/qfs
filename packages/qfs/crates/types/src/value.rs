...
//! orthogonal to type** (blueprint §4): a column is `nullable` in the schema, and a
//! `Value::Null` may appear wherever the column allows it.
//!
//! These are the DTOs codecs target (`DECODE`/`ENCODE` bridge `bytes ↔ rows`); the
//! canonical home so `qfs-codec` re-exports them rather than redefining placeholders.

use serde::{Deserialize, Serialize};

use crate::schema::{ColumnType, Name, Schema};

/// The **named** fields of a [`Value::Struct`] (blueprint §4). A nested record value is
/// self-describing: it carries each field's name alongside its value, in insertion
/// order (key order is stable, blueprint §7). This is what makes `a.b.c` path access and
...
/// A single runtime value (blueprint §4). Mirrors [`ColumnType`]; `Null` is orthogonal to
...
impl Value {
    ...
    /// because a bare null carries no type (its type comes from the column, blueprint §4).
    ...
    /// Whether this value conforms to `ty` under nullability `nullable` (blueprint §4).
    ...
    pub fn conforms_to(&self, ty: &ColumnType, nullable: bool) -> bool {
        match (self, ty) {
            ...
            // Unknown/Json accept any non-null value (late-bound columns, blueprint §4).
...
/// A single row: positional values aligned to a [`Schema`]'s columns (blueprint §4). Owned
...
impl Row {
    ...
    /// its column's type/nullability (blueprint §4). A debug/test aid, not the hot path.
    #[must_use]
...
impl Fields {
    ...
    /// conforms to the schema column at its position (blueprint §4). Names are not required
