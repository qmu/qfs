...
fn rewrite_pipe_op(op: &mut PipeOp, binds: &NewBindings) {
    match op {
        ...
        // SWITCH (blueprint §18): each arm's continuation ops and RETURNING projections carry
        // rewritable expressions.
        PipeOp::Switch(s) => {
            for arm in &mut s.arms {
                for op in &mut arm.ops {
                    rewrite_pipe_op(op, binds);
                }
                if let Some(w) = &mut arm.write {
                    if let Some(projs) = &mut w.returning {
                        for p in projs {
                            rewrite_projection(p, binds);
                        }
                    }
        ...
        // TRANSFORM names a declared definition; OF (§5.6) carries only a type reference — neither
        // has a bindable expression payload here.
        PipeOp::Limit(_)
        | PipeOp::Distinct
        | PipeOp::As(_)
        | PipeOp::Expand(_)
...
fn rewrite_expr(e: &mut Expr, binds: &NewBindings) {
    match e {
        ...
        // t92 composite constructors: substitute `NEW.*` slots inside each element/field.
        Expr::Array(elems) => {
            for el in elems {
                rewrite_expr(el, binds);
            }
        }
        Expr::Struct(fields) => {
            for (_, v) in fields {
                rewrite_expr(v, binds);
            }
        }
