//! The owned, vendor-free **event DTO** (t34, blueprint §11): the normalized cause an inbound webhook
...
/// What kind of change produced the event (blueprint §10). `#[non_exhaustive]` so a future kind does
...
/// A normalized event (blueprint §10/§11). The `new` row exposes the `NEW.*` fields a handler binds; the
