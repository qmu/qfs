---
name: qfs-slack
description: Use when a task needs Slack through qfs — read the latest messages in a channel, list the files shared in a channel or DM (newest first), detach (delete) one by its id, and post a message over /slack, as an append log. Covers creating the Slack app, the bot-token scopes it needs, and connecting a Slack workspace.
---
